---
title: "pdd.virtual.mobile.charge.notify - 虚拟类目发货通知接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.virtual.mobile.charge.notify"
menu_path:
  - "虚拟类目API"
  - "pdd.virtual.mobile.charge.notify"
captured_at: "2026-04-09T15:14:59.184Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| charge\_certi | OBJECT\[\] | 非必填 | 直充附加信息对象数组 |
| charge\_certi\_amount | LONG | 非必填 | 直充充值金额（单位：分） |
| charge\_certi\_date | STRING | 非必填 | 直充充值成功时间 （yyyy-MM-dd HH:mm:ss格式） |
| charge\_certi\_mobile | STRING | 非必填 | 直充充值号码 |
| charge\_certi\_mobile\_tail | STRING | 非必填 | 充值卡号尾号 |
| charge\_certi\_order\_sn | STRING | 非必填 | 直充充值单号 |
| charge\_certi\_text | STRING | 非必填 | 直充短信原文 |
| merchant\_outer\_id | STRING | 非必填 | 代理商(渠道)编号 |
| ele\_invoice | INTEGER | 非必填 | 电子发票信息 0-不支持开具 1-支持开具 |
| order\_sn | STRING | 必填 | 拼多多订单编码 |
| outer\_order\_sn | STRING | 必填 | 11122dafa 外部系统订单编码 |
| status | STRING | 必填 | 虚拟系统充值结果，SUCCESS-充值成功，FAIL-充值失败 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| mobile\_charge\_notify\_response | OBJECT |  | 虚拟充值回调通知响应对象 |
| success | BOOLEAN |  | 回调通知结果，true-成功，false-失败 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddVirtualMobileChargeNotifyRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddVirtualMobileChargeNotifyResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddVirtualMobileChargeNotifyRequest.ChargeCertiItem;
...
public class PopClientDemo {
 ...
        PddVirtualMobileChargeNotifyRequest request = new PddVirtualMobileChargeNotifyRequest();
        List<ChargeCertiItem> chargeCerti = new ArrayList<ChargeCertiItem>();

        ChargeCertiItem item = new ChargeCertiItem();
        item.setChargeCertiAmount(0L);
        item.setChargeCertiDate("str");
        item.setChargeCertiMobile("str");
        item.setChargeCertiMobileTail("str");
        item.setChargeCertiOrderSn("str");
        item.setChargeCertiText("str");
        item.setMerchantOuterId("str");
        chargeCerti.add(item);
        request.setChargeCerti(chargeCerti);
        request.setEleInvoice(1);
        request.setOrderSn("str");
        request.setOuterOrderSn("str");
        request.setStatus("str");
        PddVirtualMobileChargeNotifyResponse response = client.syncInvoke(request, accessToken);

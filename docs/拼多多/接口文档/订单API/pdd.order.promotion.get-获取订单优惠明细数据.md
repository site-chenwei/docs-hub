---
title: "pdd.order.promotion.get - 获取订单优惠明细数据"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.order.promotion.get"
menu_path:
  - "订单API"
  - "pdd.order.promotion.get"
captured_at: "2026-04-09T15:13:10.349Z"
---
...
更新时间：2024-07-22 11:23:11

¥免费API

必须用户授权

查询订单优惠信息（只能获取到三个月以内的多多国际订单）。 ①.订单存在商家优惠（seller\_discount）再通过此接口获取明细。
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| order\_sn\_list | STRING\[\] | 必填 | 订单号列表，最多10个 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| order\_promotion\_get\_response | OBJECT |  |  |
| order\_promotion\_list | OBJECT\[\] |  | 订单优惠信息列表 |
| order\_sn | STRING |  | 订单号 |
| seller\_discount\_details | OBJECT\[\] |  | 商家优惠信息 |
| discount | DOUBLE |  | 优惠金额。单位：元 |
| discount\_desc | STRING |  | 优惠描述 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOrderPromotionGetRequest;
...
public class PopClientDemo {
 ...
        PddOrderPromotionGetRequest request = new PddOrderPromotionGetRequest();
        List<String> orderSnList = new ArrayList<String>();
        orderSnList.add("str");
        request.setOrderSnList(orderSnList);
        PddOrderPromotionGetResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "order_promotion_get_response": {
    "order_promotion_list": [
      {
        "order_sn": "str",
        "seller_discount_details": [
          {
            "discount": 0.0,
            "discount_desc": "str"
          }

---
title: "pdd.order.review.info - 订单审核信息查询接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.order.review.info"
menu_path:
  - "订单API"
  - "pdd.order.review.info"
captured_at: "2026-04-09T15:13:13.446Z"
---
...
更新时间：2025-03-14 17:08:45

¥免费API

必须用户授权

查询单个订单审核信息（只能获取到成交时间三个月以内，状态为待发货且审核中的订单的审核信息）
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| order\_list | OBJECT\[\] | 必填 | 订单列表，目前仅支持单个查询 |
...
| --- | --- | --- | --- |
| response | OBJECT |  |  |
| order\_review\_info\_list | OBJECT\[\] |  | 订单审核信息列表 |
| estimated\_review\_time | INTEGER |  | 预计审核时间，单位:小时。0/null：平台会在订单成交后尽快完成审核；具体数值X：平台预计在订单成交后X小时内完成审核。 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOrderReviewInfoRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddOrderReviewInfoResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddOrderReviewInfoRequest.OrderListItem;
...
public class PopClientDemo {
 ...
        PddOrderReviewInfoRequest request = new PddOrderReviewInfoRequest();
 ...
        PddOrderReviewInfoResponse response = client.syncInvoke(request, accessToken);

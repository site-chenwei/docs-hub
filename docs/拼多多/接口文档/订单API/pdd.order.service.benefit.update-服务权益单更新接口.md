---
title: "pdd.order.service.benefit.update - 服务权益单更新接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.order.service.benefit.update"
menu_path:
  - "订单API"
  - "pdd.order.service.benefit.update"
captured_at: "2026-04-09T15:13:19.665Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| operate\_type | INTEGER | 必填 | 操作类型； 添加服务对象编号:1； 修改服务对象编号:2 |
| order\_sn | STRING | 必填 | 拼多多订单号 |
| out\_service\_order\_sn | STRING | 必填 | 外部服务单号 |
| period\_unit | INTEGER | 必填 | 服务有效期单位； 年:1 |
| service\_effect\_end\_time | LONG | 必填 | 服务有效止期，时间戳（毫秒） |
| service\_effect\_start\_time | LONG | 必填 | 服务有效起期，时间戳（毫秒） |
| service\_object | INTEGER | 必填 | 服务对象； 整机:1； 核心部件:2 |
| service\_object\_sn | STRING | 必填 | 服务对象编号 |
| service\_period | INTEGER | 必填 | 服务有效期 |
| service\_type | INTEGER | 必填 | 服务单类型; 设备延保:1 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| service\_benefit\_update\_response | OBJECT |  |  |
| success | BOOLEAN |  | 更新成功:true；更新失败:false |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOrderServiceBenefitUpdateRequest;
...
public class PopClientDemo {
 ...
        PddOrderServiceBenefitUpdateRequest request = new PddOrderServiceBenefitUpdateRequest();
        request.setOperateType(1);
        request.setOrderSn("str");
        request.setOutServiceOrderSn("str");
        request.setPeriodUnit(1);
        request.setServiceEffectEndTime(1729651209000L);
        request.setServiceEffectStartTime(1729651209000L);
        request.setServiceObject(1);
        request.setServiceObjectSn("str");
        request.setServicePeriod(1);
        request.setServiceType(1);
        PddOrderServiceBenefitUpdateResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "service_benefit_update_response": {
    "success": false
  }
}
```
...
| 50001 | 业务服务错误 | 2000000 | 业务异常 | 检查入参，确认无问题后联系拼多多技术 |
| 2000001 | 服务有效时间错误 | 检查服务有效时间 |
| 2000002 | 服务对象错误 | 检查服务对象 |

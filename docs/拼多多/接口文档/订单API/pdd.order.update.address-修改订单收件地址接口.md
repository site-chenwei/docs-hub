---
title: "pdd.order.update.address - 修改订单收件地址接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.order.update.address"
menu_path:
  - "订单API"
  - "pdd.order.update.address"
captured_at: "2026-04-09T15:13:41.787Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| address | STRING | 非必填 | 收件详细地址 |
| city | STRING | 必填 | 收件地城市 |
| city\_id | INTEGER | 必填 | 城市编码 |
| order\_sn | STRING | 必填 | 订单编号 |
| province | STRING | 必填 | 收件地省份 |
| province\_id | INTEGER | 必填 | 省份编码 |
| receiver\_name | STRING | 非必填 | 收件人姓名 |
| receiver\_phone | STRING | 非必填 | 收件人电话，明文 |
| town | STRING | 必填 | 收件地区县 |
| town\_id | INTEGER | 必填 | 区县编码 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOrderUpdateAddressRequest;
...
public class PopClientDemo {
 ...
        PddOrderUpdateAddressRequest request = new PddOrderUpdateAddressRequest();
 ...
        request.setTown("str");
        request.setTownId(0);
        PddOrderUpdateAddressResponse response = client.syncInvoke(request, accessToken);
...
| --- | --- |
| 订单编辑权限包 | 打单、进销存、虚拟商家后台系统、企业ERP、商家后台系统、订单处理、电子凭证商家后台系统、跨境企业ERP报关版 |

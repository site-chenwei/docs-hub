---
title: "pdd.order.upload.delivery.feature - 订单发货内容信息上传"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.order.upload.delivery.feature"
menu_path:
  - "订单API"
  - "pdd.order.upload.delivery.feature"
captured_at: "2026-04-09T15:13:44.899Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| device\_sn\_image\_list | STRING\[\] | 非必填 | 设备序列号图片列表 |
| device\_sn\_list | STRING\[\] | 非必填 | 设备序列号列表 |
| imei2\_list | STRING\[\] | 非必填 | imei2（手机串号2）列表 |
| imei\_list | STRING\[\] | 非必填 | imei（手机串号）列表 |
| order\_sn | STRING | 必填 | 订单号。形如：20150909-452750051 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| upload\_delivery\_feature\_response | OBJECT |  | response |
| is\_success | BOOLEAN |  | result |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOrderUploadDeliveryFeatureRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddOrderUploadDeliveryFeatureResponse;
...
public class PopClientDemo {
 ...
        PddOrderUploadDeliveryFeatureRequest request = new PddOrderUploadDeliveryFeatureRequest();
        List<String> deviceSnImageList = new ArrayList<String>();
        deviceSnImageList.add("str");
        request.setDeviceSnImageList(deviceSnImageList);
        List<String> deviceSnList = new ArrayList<String>();
        deviceSnList.add("str");
        request.setDeviceSnList(deviceSnList);
        List<String> imei2List = new ArrayList<String>();
        imei2List.add("str");
        request.setImei2List(imei2List);
        List<String> imeiList = new ArrayList<String>();
        imeiList.add("str");
        request.setImeiList(imeiList);
        request.setOrderSn("str");
        PddOrderUploadDeliveryFeatureResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "upload_delivery_feature_response": {
    "is_success": false
  }
}
```
...
| --- | --- |
| 物流发货权限包 | 打单、进销存、虚拟商家后台系统、企业ERP、商家后台系统、订单处理、跨境企业ERP报关版 |

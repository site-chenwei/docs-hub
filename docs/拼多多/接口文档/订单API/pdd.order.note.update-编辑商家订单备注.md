---
title: "pdd.order.note.update - 编辑商家订单备注"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.order.note.update"
menu_path:
  - "订单API"
  - "pdd.order.note.update"
captured_at: "2026-04-09T15:13:00.873Z"
---
...
| --- | --- | --- | --- |
| note | STRING | 必填 | 订单备注 |
| tag | INTEGER | 非必填 | 备注标记：1-红色，2-黄色，3-绿色，4-蓝色，5-紫色，tag与tag\_name关联，都入参或都不入参 |
| tag\_name | STRING | 非必填 | 标记名称；长度最大为3个字符，tag与tag\_name关联，都入参或都不入参 |
...
| error\_code | INTEGER | 10001 | 错误码 |
| error\_msg | STRING | 系统异常 | 错误信息 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOrderNoteUpdateRequest;
...
public class PopClientDemo {
 ...
        request.setNote("用户要求发顺丰快递");
        request.setTag(1);
        request.setTagName("发顺丰");
        request.setOrderSn("str");
        PddOrderNoteUpdateResponse response = client.syncInvoke(request, accessToken);

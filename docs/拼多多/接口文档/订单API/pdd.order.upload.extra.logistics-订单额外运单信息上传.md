---
title: "pdd.order.upload.extra.logistics - 订单额外运单信息上传"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.order.upload.extra.logistics"
menu_path:
  - "订单API"
  - "pdd.order.upload.extra.logistics"
captured_at: "2026-04-09T15:13:48.032Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| extra\_track\_list | OBJECT\[\] | 必填 | 订单多包裹发货时使用的其他发货快递信息 |
| shipping\_id | INTEGER | 必填 | 快递公司id |
| tracking\_number | STRING | 必填 | 快递单号 |
| order\_sn | STRING | 必填 | 订单号 |
| extra\_track\_type | INTEGER | 非必填 | 额外运单类型，1=分包发货，2=补发商品，3=发放赠品 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| upload\_extra\_logistics\_response | OBJECT |  |  |
| success | BOOLEAN | true | 是否成功，false-失败，true-成功 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOrderUploadExtraLogisticsRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddOrderUploadExtraLogisticsResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddOrderUploadExtraLogisticsRequest.ExtraTrackListItem;
...
public class PopClientDemo {
 ...
        PddOrderUploadExtraLogisticsRequest request = new PddOrderUploadExtraLogisticsRequest();
        List<ExtraTrackListItem> extraTrackList = new ArrayList<ExtraTrackListItem>();

        ExtraTrackListItem item = new ExtraTrackListItem();
        item.setShippingId(0);
        item.setTrackingNumber("str");
        extraTrackList.add(item);
        request.setExtraTrackList(extraTrackList);
        request.setOrderSn("str");
        request.setExtraTrackType(1);
        PddOrderUploadExtraLogisticsResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "upload_extra_logistics_response": {
    "success": true
  }
}
```

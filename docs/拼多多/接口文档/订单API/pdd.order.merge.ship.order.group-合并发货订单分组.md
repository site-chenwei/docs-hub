---
title: "pdd.order.merge.ship.order.group - 合并发货订单分组"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.order.merge.ship.order.group"
menu_path:
  - "订单API"
  - "pdd.order.merge.ship.order.group"
captured_at: "2026-04-09T15:12:57.779Z"
---
...
更新时间：2024-01-21 15:48:12

¥免费API

必须用户授权

合并发货订单分组。用于订单合并发货分组，确保相同收货信息的订单合并到相同组。 ①.普通订单相同open\_address\_id可直接合单，不同open\_address\_id可调用本接口进行判定。同一天的订单open\_address\_id不同不可合单，无需再调用本接口 ②.没有open\_address\_id的订单可调用本接口进行判定 ③.收件地址发生更新后需重新调用本接口。
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| order\_list | OBJECT\[\] | 必填 | 订单列表，最多支持100个 |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| merge\_ship\_order\_group\_response | OBJECT |  |  |
| order\_group\_list | OBJECT\[\] |  | 订单分组结果 |
| order\_sn\_list | STRING\[\] |  | 可以合并发货的订单号列表。不同列表之间不能合并发货 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOrderMergeShipOrderGroupRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddOrderMergeShipOrderGroupResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddOrderMergeShipOrderGroupRequest.OrderListItem;
...
public class PopClientDemo {
 ...
        PddOrderMergeShipOrderGroupRequest request = new PddOrderMergeShipOrderGroupRequest();
 ...
        PddOrderMergeShipOrderGroupResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "merge_ship_order_group_response": {
    "order_group_list": [
      {
        "order_sn_list": [
          "str"

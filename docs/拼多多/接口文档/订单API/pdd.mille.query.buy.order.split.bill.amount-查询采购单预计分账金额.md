---
title: "pdd.mille.query.buy.order.split.bill.amount - 查询采购单预计分账金额"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.mille.query.buy.order.split.bill.amount"
menu_path:
  - "订单API"
  - "pdd.mille.query.buy.order.split.bill.amount"
captured_at: "2026-04-09T15:12:42.229Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| request | OBJECT | 必填 | 批量采购单预计分账金额查询请求 |
| orderSnList | STRING\[\] | 非必填 | 订单号列表 |
...
| resultList | OBJECT\[\] |  | 订单详情对象集合 |
| orderAmount | LONG |  | 采购订单金额，单位分 |
| orderSn | STRING |  | 订单号 |
| orderType | INTEGER |  | 订单类型 2：分销订单， 7：分账分销订单。非2、7 代表非分销订单 |
| resellerMallId | LONG |  | 分销商店铺编号 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddMilleQueryBuyOrderSplitBillAmountRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddMilleQueryBuyOrderSplitBillAmountResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddMilleQueryBuyOrderSplitBillAmountRequest.Request;
...
public class PopClientDemo {
 ...
        PddMilleQueryBuyOrderSplitBillAmountRequest request = new PddMilleQueryBuyOrderSplitBillAmountRequest();

        Request request1 = new Request();
        List<String> orderSnList = new ArrayList<String>();
        orderSnList.add("str");
        request1.setOrderSnList(orderSnList);
        request.setRequest(request1);
        PddMilleQueryBuyOrderSplitBillAmountResponse response = client.syncInvoke(request, accessToken);
...
| 50001 | 业务服务错误 | 50001 | 越权查询，查询订单不属于此店铺 | 调用方修改入参 |

---
title: "pdd.order.submall.information.get - 以旧换新国补订单详情分店铺查询接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.order.submall.information.get"
menu_path:
  - "订单API"
  - "pdd.order.submall.information.get"
captured_at: "2026-04-09T15:13:29.116Z"
---
...
更新时间：2025-12-22 21:26:21

¥免费API

必须用户授权

分店铺查询单个以旧换新国补订单详情（只能获取到成交时间三个月以内的交易信息）
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| order\_info\_get\_response | OBJECT |  | response |
| order\_info | OBJECT |  | 订单详情对象 |
...
| extra\_delivery\_list | OBJECT\[\] |  | 额外的发货快递信息 |
| logistics\_id | INTEGER |  | 快递公司编号 |
| tracking\_number | STRING |  | 快递运单号 |
| free\_sf | INTEGER |  | 是否顺丰包邮 1表示是 0表示否 |
...
| sku\_id | LONG |  | 赠品规格编码 |
| goods\_amount | DOUBLE |  | 商品金额（元）商品金额=商品销售价格\*商品数量-订单改价折扣金额 |
...
| outer\_id | STRING |  | 商家外部编码（sku），注意：编辑商品后必须等待商品审核通过后方可生效，订单中商品信息为交易快照的商品信息。 |
| sku\_id | LONG |  | 商品规格编码 |
...
| pay\_amount | DOUBLE |  | 支付金额（元）支付金额=商品金额-折扣金额+邮费+服务费 |
| pay\_time | STRING |  | 支付时间 |
| platform\_discount | DOUBLE |  | 平台优惠金额 |
| postage | DOUBLE |  | 邮费 |
...
加密
 ...
 | STRING |  | 收件人地址，不拼接省市区。订单状态为待发货状态，且订单未在审核中的情况下返回密文数据；其余情况返回空字符串。 |
| receiver\_name

加密

 | STRING |  | 收件人姓名。订单状态为待发货状态，且订单未在审核中的情况下返回密文数据；其余情况返回空字符串。 |
| receiver\_phone

加密

 | STRING |  | 收件人电话。订单状态为待发货状态，且订单未在审核中的情况下返回密文数据；其余情况返回空字符串。 |
...
| seller\_discount | DOUBLE |  | 店铺优惠金额 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOrderSubmallInformationGetRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddOrderSubmallInformationGetResponse;
...
public class PopClientDemo {
 ...
        PddOrderSubmallInformationGetRequest request = new PddOrderSubmallInformationGetRequest();
        request.setOrderSn("str");
        PddOrderSubmallInformationGetResponse response = client.syncInvoke(request, accessToken);

---
title: "pdd.open.msg.service.send.express.msg - 根据运单号发短信"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.open.msg.service.send.express.msg"
menu_path:
  - "短信服务API"
  - "pdd.open.msg.service.send.express.msg"
captured_at: "2026-04-09T15:23:34.170Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| out\_id | STRING | 非必填 | 业务请求唯一标识 |
| sign\_name | STRING | 必填 | 短信签名名称 |
| sms\_up\_extend\_code | STRING | 非必填 | 上行短信扩展码 |
| template\_code | LONG | 必填 | 短信模板CODE |
| template\_param\_json | MAP\[\] | 非必填 | 短信模板变量JSON集合(与手机号对应)与按照手机号发短信一致key变量名 value变量值,示例："${param}","aaa"，注意${}符号勿遗漏 |
| $key | STRING | 非必填 | 模板变量key |
| $value | STRING | 非必填 | 模板变量value |
| waybill\_codes | STRING\[\] | 必填 | 物流单号集合 |
| wp\_code | STRING | 必填 | 快递公司编码 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| biz\_id | STRING | 123 | 回执id |
| code | INTEGER | 123 | 状态码 |
| message | STRING |  | 状态原因 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOpenMsgServiceSendExpressMsgRequest;
...
public class PopClientDemo {
 ...
        PddOpenMsgServiceSendExpressMsgRequest request = new PddOpenMsgServiceSendExpressMsgRequest();
        request.setOutId("str");
        request.setSignName("短信签名名称");
        request.setSmsUpExtendCode("str");
        request.setTemplateCode(152550005L);
        List<Map<String, String>> templateParamJson = new ArrayList<Map<String, String>>();
        Map<String, String> item = new HashMap<String, String>();
        item.put("", "str");
        templateParamJson.add(item);
        request.setTemplateParamJson(templateParamJson);
        List<String> waybillCodes = new ArrayList<String>();
        waybillCodes.add("PDD112233");
        request.setWaybillCodes(waybillCodes);
        request.setWpCode("XF1001");
        PddOpenMsgServiceSendExpressMsgResponse response = client.syncInvoke(request);

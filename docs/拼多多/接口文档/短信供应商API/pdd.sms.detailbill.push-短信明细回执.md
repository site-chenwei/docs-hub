---
title: "pdd.sms.detailbill.push - 短信明细回执"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.sms.detailbill.push"
menu_path:
  - "短信供应商API"
  - "pdd.sms.detailbill.push"
captured_at: "2026-04-09T15:23:52.818Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| account | STRING | 必填 | cmpp账号名 |
| batch\_version | LONG | 必填 | 批次版本，每天数据必须属于同一个批次，如果重传可以批次号增加，平台以最大批次号为准。一般情况下，批次号固定数字，只有当某天上传数据错误需要弃用时，使用增加后的新批次号。 |
| date | STRING | 必填 | 数据日期(格式yyyy-MM-dd) |
| details | OBJECT\[\] | 必填 | 短信明细，detail的列表，list最大100 |
| deliver\_time | STRING | 必填 | 短信下发时间(yyyy-MM-dd HH:mm:ss) |
| error\_code | STRING | 必填 | 回执状态码，发送成功传DELIVRD |
| msg\_id | LONG | 必填 | 短信id,即SubmitResp.msgId,十进制表示 |
| submit\_time | STRING | 必填 | 短信提交时间(yyyy-MM-dd HH:mm:ss) |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| sms\_detailbill\_push\_resposne | OBJECT |  | response |
...
import com.pdd.pop.sdk.http.api.pop.request.PddSmsDetailbillPushRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddSmsDetailbillPushResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddSmsDetailbillPushRequest.DetailsItem;
...
public class PopClientDemo {
 ...
        PddSmsDetailbillPushRequest request = new PddSmsDetailbillPushRequest();
        request.setAccount("str");
        request.setBatchVersion(0L);
        request.setDate("str");
        List<DetailsItem> details = new ArrayList<DetailsItem>();

        DetailsItem item = new DetailsItem();
        item.setDeliverTime("str");
        item.setErrorCode("str");
        item.setMsgId(0L);
        item.setSubmitTime("str");
        details.add(item);
        request.setDetails(details);
        PddSmsDetailbillPushResponse response = client.syncInvoke(request);

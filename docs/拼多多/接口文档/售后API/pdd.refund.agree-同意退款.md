---
title: "pdd.refund.agree - 同意退款"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.refund.agree"
menu_path:
  - "售后API"
  - "pdd.refund.agree"
captured_at: "2026-04-09T15:14:09.588Z"
---
...
| operate\_desc | STRING | 非必填 | 退款备注，商家留言 |
...
| message | STRING | 操作成功 | 退款操作成功 退款操作结果信息。如果退款失败，会返回失败原因 |
| order\_sn | STRING |  |  |
| succ | BOOLEAN | true | 接口调用成功 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddRefundAgreeRequest.Request;
...
public class PopClientDemo {
 ...
        request1.setOperateDesc("str");
        request1.setOrderSn("str");
        request.setRequest(request1);
        PddRefundAgreeResponse response = client.syncInvoke(request, accessToken);

---
title: "pdd.refund.status.check - 售后校验接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.refund.status.check"
menu_path:
  - "售后API"
  - "pdd.refund.status.check"
captured_at: "2026-04-09T15:14:28.225Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| order\_sns | STRING | 必填 | 20150909-452750051,20150909-452750134 用逗号分开 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| refund\_status\_check\_response | OBJECT |  | response |
| order\_sns\_exists\_refund | OBJECT\[\] |  | 存在售后的订单 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddRefundStatusCheckRequest;
...
public class PopClientDemo {
 ...
        PddRefundStatusCheckRequest request = new PddRefundStatusCheckRequest();
        request.setOrderSns("str");
        PddRefundStatusCheckResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "refund_status_check_response": {
    "order_sns_exists_refund": [
      {
        "order_sn": "str"
      }
    ]

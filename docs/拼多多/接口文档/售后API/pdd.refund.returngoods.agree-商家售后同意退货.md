---
title: "pdd.refund.returngoods.agree - 商家售后同意退货"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.refund.returngoods.agree"
menu_path:
  - "售后API"
  - "pdd.refund.returngoods.agree"
captured_at: "2026-04-09T15:14:25.181Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| request | OBJECT | 必填 | 请求入参 |
| after\_sales\_id | LONG | 必填 | 售后id |
| operate\_desc | STRING | 必填 | 给用户留言 |
| order\_sn | STRING | 必填 | 订单号 |
| return\_address\_id | STRING | 必填 | 退货地址列表中已有的退货地址id |
...
| error\_code | INTEGER |  | 接口调用错误编码 |
| error\_msg | STRING |  | 接口调用错误描述 |
| result | OBJECT |  | 返回内容 |
| after\_sales\_id | LONG |  | 售后id |
| message | STRING |  | 退货是否成功描述 |
| order\_sn | STRING |  | 订单号 |
| success | BOOLEAN |  | 退货是否成功 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddRefundReturngoodsAgreeRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddRefundReturngoodsAgreeResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddRefundReturngoodsAgreeRequest.Request;
...
public class PopClientDemo {
 ...
        PddRefundReturngoodsAgreeRequest request = new PddRefundReturngoodsAgreeRequest();

        Request request1 = new Request();
        request1.setAfterSalesId(0L);
        request1.setOperateDesc("给用户留言");
        request1.setOrderSn("订单号");
        request1.setReturnAddressId("退货地址列表中已有的退货地址id");
        request.setRequest(request1);
        PddRefundReturngoodsAgreeResponse response = client.syncInvoke(request, accessToken);
...
| --- | --- |
| 售后退款退货权限包 | 打单、进销存、虚拟商家后台系统、企业ERP、商家后台系统、订单处理、电子凭证商家后台系统 |
...
| 50001 | 业务服务错误 | 2000000 | 操作失败 | 根据错误提示处理 |
| 3000000 | 参数错误 | 按照错误提示处理，不可重试 |
| 4000000 | 执行出错 | 重试，无法恢复则联系平台处理 |

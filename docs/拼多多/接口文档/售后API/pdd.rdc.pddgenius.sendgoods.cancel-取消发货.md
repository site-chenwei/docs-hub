---
title: "pdd.rdc.pddgenius.sendgoods.cancel - 取消发货"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.rdc.pddgenius.sendgoods.cancel"
menu_path:
  - "售后API"
  - "pdd.rdc.pddgenius.sendgoods.cancel"
captured_at: "2026-04-09T15:14:03.430Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| param | OBJECT | 必填 | param |
| fail\_reason\_code | INTEGER | 非必填 | 错误码：1001 错误场景：该订单未同步，无法取消发货 实际含义：订单未同步到isv; 错误码：1002 错误场景：该订单已发货，无法取消发货 实际含义：订单已经发货; 错误码：1003 错误场景：该订单已发货，无法取消发货 实际含义：订单已打印电子面单 |
| msg | STRING | 非必填 | 描述 |
| operate\_time | LONG | 非必填 | 操作时间戳（毫秒） |
| refund\_fee | INTEGER | 非必填 | 退款金额 单位 分 |
| refund\_id | LONG | 必填 | 退款单ID |
| status | STRING | 必填 | 状态SUCCESS、FAIL |
| tid | STRING | 必填 | 订单号 |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| rdc\_pddgenius\_sendgoods\_cancel\_response | OBJECT |  | response |
| result | OBJECT |  | result |
| result\_data | OBJECT |  | result\_data |
| refund\_id | LONG |  | 退款单ID |
...
import com.pdd.pop.sdk.http.api.pop.request.PddRdcPddgeniusSendgoodsCancelRequest.Param;
...
public class PopClientDemo {
 ...
        param.setFailReasonCode(0);
        param.setMsg("str");
        param.setOperateTime(0L);
        param.setRefundFee(0);
        param.setRefundId(0L);
        param.setStatus("str");
        param.setTid("str");
...
```
{
  "rdc_pddgenius_sendgoods_cancel_response": {
    "result": {
      "result_data": {
        "refund_id": 0
      }
...
| 50001 | 业务服务错误 | isv.cancel-fail | 拦截失败 | 拦截失败 |
| isv.cancel-strategy-disable | 不存在已经配置生效的PG任务模板 | 不存在已经配置生效的PG任务模板 |
| isv.cancel-strategy-limit | 策略限额，无法退款 | 策略限额，无法退款 |
| isv.refund-fail | 退款失败 | 退款失败 |
| isv.service-error | 服务异常 | 服务异常,稍后重试 |

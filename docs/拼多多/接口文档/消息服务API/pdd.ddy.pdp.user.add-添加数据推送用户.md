---
title: "pdd.ddy.pdp.user.add - 添加数据推送用户"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ddy.pdp.user.add"
menu_path:
  - "pdd.ddy.pdp.user.add"
captured_at: "2026-04-15T03:32:29.766Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| history\_days | INTEGER | 非必填 | 推送历史数据天数，只能为30天内，包含30天。当此参数不填时，表示以页面中应用配置的历史天数为准；如果为0表示这个用户不推送历史数据；其它表示推送的历史天数 |
| rds\_id | STRING | 必填 | rds实例编号 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| ddy\_pdp\_user\_add\_response | OBJECT |  | 返回结果 |
| is\_success | BOOLEAN | true | 是否添加成功 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddDdyPdpUserAddRequest;
...
public class PopClientDemo {
 ...
        PddDdyPdpUserAddRequest request = new PddDdyPdpUserAddRequest();
        request.setHistoryDays(7);
        request.setRdsId("DE02BC0B0E64AC89");
        PddDdyPdpUserAddResponse response = client.syncInvoke(request, accessToken);
...
| 50001 | 业务服务错误 | isv.invalid-parameter:rds\_id | 数据库实例不存在或尚未绑定 | 请检查数据库实例编号，并确认该实例已绑定到应用 |

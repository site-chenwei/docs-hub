---
title: "pdd.ddy.pdp.users.get - 获取开通订单同步服务的用户"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ddy.pdp.users.get"
menu_path:
  - "pdd.ddy.pdp.users.get"
captured_at: "2026-04-15T03:32:36.122Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| owner\_id | LONG | 非必填 | 如果传了owner\_id，表示单条查询 |
| start\_modified | STRING | 非必填 | 查询最后更改时间在某个时间段内的用户 |
| end\_modified | STRING | 非必填 | 查询最后更改时间在某个时间段内的用户 |
| page\_no | INTEGER | 非必填 | 当前页数 |
| page\_size | INTEGER | 非必填 | 每页记录数，默认200，最大200 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| ddy\_pdp\_users\_get\_response | OBJECT |  | 接口返回结果 |
| total\_results | LONG | 1 | 总记录数 |
| users | OBJECT\[\] |  | 用户列表 |
| owner\_id | LONG | 6041267 | 用户id |
| rds\_id | STRING | 19E534A8C61F30F1 | rds数据库的实例id |
| status | INTEGER | 1 | 状态，0: 暂停，1: 正常，2: 商家授权已过期 |
| history\_days | INTEGER | 30 | 推送历史数据天数 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddDdyPdpUsersGetRequest;
...
public class PopClientDemo {
 ...
        PddDdyPdpUsersGetRequest request = new PddDdyPdpUsersGetRequest();
        request.setOwnerId(6041267L);
        request.setStartModified("2019-06-01 00:00:00");
        request.setEndModified("2019-06-07 12:00:00");
        request.setPageNo(1);
        request.setPageSize(100);
        PddDdyPdpUsersGetResponse response = client.syncInvoke(request);
...
```
{
  "ddy_pdp_users_get_response": {
    "total_results": 1,
    "users": [
      {
        "history_days": 30,
        "owner_id": 6041267,
        "rds_id": "19E534A8C61F30F1",
        "status": 1
      }
    ]
  }
...
| --- | --- |
| 数据推送权限包 | 打单、进销存、企业ERP、商家后台系统、订单处理、跨境企业ERP报关版 |

---
title: "pdd.pmc.user.get - 获取用户已开通消息"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.pmc.user.get"
menu_path:
  - "消息服务API"
  - "pdd.pmc.user.get"
captured_at: "2026-04-09T15:22:43.857Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| owner\_id | STRING | 必填 | 用户唯一id |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| pmc\_user\_get\_response | OBJECT |  | response |
| pmc\_user | OBJECT |  | 开通的用户数据 |
| created | STRING |  | 用户首次开通时间 |
| is\_expire | INTEGER |  | 用户授权是否有效，0表示授权有效，1表示授权过期 |
| modified | STRING |  | 用户最后开通时间 |
| owner\_id | STRING |  | 用户ID |
| topics | STRING\[\] |  | 用户开通的消息类型列表。如果为空表示应用开通的所有类型 |
...
| 50001 | 业务服务错误 | isv.service-error-1 | 系统错误 | 稍后重试 |
| isv.service-error-2 | 系统错误 | 稍后重试 |

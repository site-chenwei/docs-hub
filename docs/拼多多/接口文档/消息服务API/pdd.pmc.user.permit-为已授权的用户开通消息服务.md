---
title: "pdd.pmc.user.permit - 为已授权的用户开通消息服务"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.pmc.user.permit"
menu_path:
  - "消息服务API"
  - "pdd.pmc.user.permit"
captured_at: "2026-04-09T15:22:46.963Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| topics | STRING | 非必填 | 消息主题列表，用半角逗号分隔。当用户订阅的topic是应用订阅的子集时才需要设置，不设置表示继承应用所订阅的所有topic，一般情况建议不要设置。 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| pmc\_user\_permit\_response | OBJECT |  | response |
| is\_success | BOOLEAN |  | 是否成功 |
...
| --- | --- |
| 消息服务权限包 | 多多客联盟、打单、进销存、商品优化分析、虚拟商家后台系统、电子面单、企业ERP、商家后台系统、仓储管理系统、订单处理、厂商代发货后台系统、电子凭证商家后台系统、跨境企业ERP报关版 |
...
| 50001 | 业务服务错误 | isv.app-apply-none-topics | 应用没有订阅任何消息，无法给用户开通 | 应用先订阅消息 |
| isv.app-not-apply-this-topic | 应用没有订阅指定的消息，无法给用户开通 | 应用先订阅指定的消息 |
| isv.invalid-parameter:topics | 消息名称不合法 | 使用正确的消息名称 |
| isv.service-error-1 | 系统错误 | 稍后重试 |

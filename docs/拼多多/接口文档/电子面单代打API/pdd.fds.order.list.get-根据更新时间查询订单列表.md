---
title: "pdd.fds.order.list.get - 根据更新时间查询订单列表"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.fds.order.list.get"
menu_path:
  - "电子面单代打API"
  - "pdd.fds.order.list.get"
captured_at: "2026-04-09T15:23:59.029Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| param\_fds\_order\_list\_get\_request | OBJECT | 必填 | 入参信息 |
| end\_updated\_time | LONG | 必填 | 必填，更新时间结束时间的时间戳，指格林威治时间 1970 年 01 月 01 日 00 时 00 分 00 秒(北京时间 1970 年 01 月 01 日 08 时 00 分 00 秒)起至现在的总毫秒数 PS：开始时间结束时间间距不超过半小时 |
| page | INTEGER | 必填 | 返回页码，页码从 1 开始 PS：当前采用分页返回，数量和页数会一起传 |
| page\_size | INTEGER | 必填 | 返回数量，最大 100 |
| start\_updated\_time | LONG | 必填 | 必填，更新时间开始时间的时间戳，指格林威治时间 1970 年 01 月 01 日 00 时 00 分 00 秒(北京时间 1970 年 01 月 01 日 08 时 00 分 00 秒)起至现在的总毫秒数 PS：开始时间结束时间间距不超过半小时 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| pdd\_fds\_order\_list\_get\_response | OBJECT |  | response |
| order\_list | OBJECT\[\] |  | 订单列表 |
...
| out\_sku\_sn | STRING | A11 | 商家配置的sku编码 |
...
| sf\_only | INTEGER | 1 | 加价发顺丰状态 0：不是 1：是 |
| status | INTEGER | 1 | 分配状态 0：取消分配 1：已分配 |
| total | INTEGER | 1000 | 订单总数 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddFdsOrderListGetRequest.ParamFdsOrderListGetRequest;
...
public class PopClientDemo {
 ...
        ParamFdsOrderListGetRequest paramFdsOrderListGetRequest = new ParamFdsOrderListGetRequest();
        paramFdsOrderListGetRequest.setEndUpdatedTime(1577809800000L);
        paramFdsOrderListGetRequest.setPage(1);
        paramFdsOrderListGetRequest.setPageSize(100);
        paramFdsOrderListGetRequest.setStartUpdatedTime(1577808000000L);
        request.setParamFdsOrderListGetRequest(paramFdsOrderListGetRequest);
        PddFdsOrderListGetResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "pdd_fds_order_list_get_response": {
    "order_list": [
      {

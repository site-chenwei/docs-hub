---
title: "pdd.fds.waybill.return.slave - 电子面单回传额外运单号"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.fds.waybill.return.slave"
menu_path:
  - "电子面单代打API"
  - "pdd.fds.waybill.return.slave"
captured_at: "2026-04-09T15:24:14.669Z"
---
...
更新时间：2021-10-24 20:26:11

¥免费API

必须用户授权

1、代发管理支持厂家回传额外运单号给商家 2、商家可以在MMS后台的代发管理查看额外运单号字段 3、1个订单最多支持上传10个额外运单号，额外的从运单号不能包括主运单号
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| request | OBJECT | 必填 | 回传从运单号请求 |
| mall\_mask\_id | STRING | 必填 | 代打店铺id |
| order\_mask\_sn | STRING | 必填 | 代打订单号 |
| waybill\_codes | STRING\[\] | 非必填 | 从运单号列表，最多传递十条从运单号 |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| pdd\_fds\_waybill\_return\_slave\_response | OBJECT |  | response |
| result | BOOLEAN |  | 回传结果true：成功 false：失败 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddFdsWaybillReturnSlaveRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddFdsWaybillReturnSlaveResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddFdsWaybillReturnSlaveRequest.Request;
...
public class PopClientDemo {
 ...
        PddFdsWaybillReturnSlaveRequest request = new PddFdsWaybillReturnSlaveRequest();

        Request request1 = new Request();
        request1.setMallMaskId("str");
        request1.setOrderMaskSn("str");
        List<String> waybillCodes = new ArrayList<String>();
        waybillCodes.add("str");
        request1.setWaybillCodes(waybillCodes);
        request1.setWpCode("str");
        request.setRequest(request1);
        PddFdsWaybillReturnSlaveResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "pdd_fds_waybill_return_slave_response": {
    "result": false
  }
}
```
...
| 50001 | 业务服务错误 | 1004 | 订单不存在或者已经取消分配 | 请传入正确的数据 |
| 1009 | 快递编码错误 | 传入正确的快递编码 |
| 1323 | 查询失败 | 请重试查询 |
| 1327 | 更新从运单号失败 | 重新传入从运单号 |
| 1328 | 从运单号包含主运单，请重传从运单号 | 从运单号包含主运单，请重传从运单号 |

---
title: "pdd.ddk.oauth.goods.pid.generate - 多多进宝推广位创建接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ddk.oauth.goods.pid.generate"
menu_path:
  - "多多客工具API"
  - "pdd.ddk.oauth.goods.pid.generate"
captured_at: "2026-04-09T15:19:45.027Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| media\_id | LONG | 非必填 | 媒体id |
| number | LONG | 必填 | 要生成的推广位数量，默认为10，范围为：1~100 |
| p\_id\_name\_list | STRING\[\] | 非必填 | 推广位名称，例如\["1","2"\] |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| p\_id\_generate\_response | OBJECT |  | response |
| p\_id\_list | OBJECT\[\] |  | 多多进宝推广位对象列表 |
| create\_time | LONG |  | 推广位创建时间 |
| media\_id | LONG |  | 媒体id |
| pid\_name | STRING |  | 推广位名称 |
| p\_id | STRING |  | 调用方推广位ID |
| remain\_pid\_count | INTEGER |  | PID剩余数量 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddDdkOauthGoodsPidGenerateRequest;
...
public class PopClientDemo {
 ...
        PddDdkOauthGoodsPidGenerateRequest request = new PddDdkOauthGoodsPidGenerateRequest();
        request.setMediaId(0L);
        request.setNumber(0L);
        List<String> pIdNameList = new ArrayList<String>();
        pIdNameList.add("str");
        request.setPIdNameList(pIdNameList);
        PddDdkOauthGoodsPidGenerateResponse response = client.syncInvoke(request, accessToken);

---
title: "pdd.ddk.oauth.cms.prom.url.generate - 生成商城推广链接接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ddk.oauth.cms.prom.url.generate"
menu_path:
  - "多多客工具API"
  - "pdd.ddk.oauth.cms.prom.url.generate"
captured_at: "2026-04-09T15:19:38.491Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| channel\_type | INTEGER | 非必填 | 0, "1.9包邮"；1, "今日爆款"； 2, "品牌清仓"； 4,"PC端专属商城(已下线,会生成默认商城)" |
| custom\_parameters | STRING | 非必填 | 自定义参数，为链接打上自定义标签；自定义参数最长限制64个字节；格式为： {"uid":"11111","sid":"22222"} ，其中 uid 用户唯一标识，可自行加密后传入，每个用户仅且对应一个标识，必填； sid 上下文信息标识，例如sessionId等，非必填。该json字符串中也可以加入其他自定义的key |
...
| mobile\_short\_url | STRING |  | 唤醒拼多多app短链 |
| mobile\_url | STRING |  | 唤醒拼多多app链接 |
| multi\_group\_mobile\_short\_url | STRING |  | 多人团唤醒拼多多app链接 |
| multi\_group\_mobile\_url | STRING |  | 多人团唤醒拼多多app长链接 |
| multi\_group\_short\_url | STRING |  | 多人团短链，唤起H5页面 |
| multi\_group\_url | STRING |  | 多人团长链，唤起H5页面 |
| multi\_url\_list | OBJECT |  | 双人团链接列表 |
| mobile\_short\_url | STRING |  | 双人团唤醒拼多多app短链接 |
| mobile\_url | STRING |  | 双人团唤醒拼多多app长链接 |
| schema\_url | STRING |  | schema链接，用户安装拼多多APP的情况下会唤起APP（需客户端支持schema跳转协议） |
| short\_url | STRING |  | 双人团短链接 |
...
| mobile\_short\_url | STRING |  | 唤醒拼多多app短链接 |
| mobile\_url | STRING |  | 唤醒拼多多app长链接 |
| schema\_url | STRING |  | schema链接，用户安装拼多多APP的情况下会唤起APP（需客户端支持schema跳转协议） |
...
import com.pdd.pop.sdk.http.api.pop.request.PddDdkOauthCmsPromUrlGenerateRequest;
...
public class PopClientDemo {
 ...
        PddDdkOauthCmsPromUrlGenerateRequest request = new PddDdkOauthCmsPromUrlGenerateRequest();
 ...
        PddDdkOauthCmsPromUrlGenerateResponse response = client.syncInvoke(request, accessToken);

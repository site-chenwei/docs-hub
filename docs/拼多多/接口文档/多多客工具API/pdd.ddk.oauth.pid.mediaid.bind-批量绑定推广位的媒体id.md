---
title: "pdd.ddk.oauth.pid.mediaid.bind - 批量绑定推广位的媒体id"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ddk.oauth.pid.mediaid.bind"
menu_path:
  - "多多客工具API"
  - "pdd.ddk.oauth.pid.mediaid.bind"
captured_at: "2026-04-09T15:20:13.177Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| media\_id | LONG | 必填 | 媒体id |
| pid\_list | STRING\[\] | 必填 | 推广位列表，例如：\["60005\_612"\]，最多支持同时传入1000个 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| p\_id\_bind\_response | OBJECT |  | response |
| result | OBJECT |  | 绑定结果 |
| msg | STRING |  | 绑定结果信息提示 |
| result | BOOLEAN |  | 绑定结果 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddDdkOauthPidMediaidBindRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddDdkOauthPidMediaidBindResponse;
...
public class PopClientDemo {
 ...
        PddDdkOauthPidMediaidBindRequest request = new PddDdkOauthPidMediaidBindRequest();
        request.setMediaId(0L);
        List<String> pidList = new ArrayList<String>();
        pidList.add("str");
        request.setPidList(pidList);
        PddDdkOauthPidMediaidBindResponse response = client.syncInvoke(request, accessToken);

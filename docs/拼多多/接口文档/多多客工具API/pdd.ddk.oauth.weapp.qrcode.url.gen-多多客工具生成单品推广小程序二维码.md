---
title: "pdd.ddk.oauth.weapp.qrcode.url.gen - 多多客工具生成单品推广小程序二维码"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ddk.oauth.weapp.qrcode.url.gen"
menu_path:
  - "多多客工具API"
  - "pdd.ddk.oauth.weapp.qrcode.url.gen"
captured_at: "2026-04-09T15:20:22.562Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| custom\_parameters | STRING | 非必填 | 自定义参数，为链接打上自定义标签。自定义参数最长限制64个字节。 |
| goods\_id\_list | LONG\[\] | 非必填 | 商品ID，仅支持单个查询 |
| goods\_sign\_list | STRING\[\] | 非必填 | 商品goodsSign列表，支持通过goodsSign查询商品。goodsSign是加密后的goodsId, goodsId已下线，请使用goodsSign来替代。使用说明：https://jinbao.pinduoduo.com/qa-system?questionId=252 |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| weapp\_qrcode\_generate\_response | OBJECT |  | response |
| url | STRING |  | 单品推广小程序二维码url |
...
import com.pdd.pop.sdk.http.api.pop.request.PddDdkOauthWeappQrcodeUrlGenRequest;
...
public class PopClientDemo {
 ...
        PddDdkOauthWeappQrcodeUrlGenRequest request = new PddDdkOauthWeappQrcodeUrlGenRequest();
 ...
        request.setPId("str");
        request.setZsDuoId(1L);
        PddDdkOauthWeappQrcodeUrlGenResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "weapp_qrcode_generate_response": {
    "url": "str"
  }
}
```

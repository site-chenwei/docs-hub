---
title: "pdd.ddk.oauth.goods.pid.query - 多多客已生成推广位信息查询"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ddk.oauth.goods.pid.query"
menu_path:
  - "多多客工具API"
  - "pdd.ddk.oauth.goods.pid.query"
captured_at: "2026-04-09T15:19:48.118Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| page | INTEGER | 非必填 | 返回的页数 |
| page\_size | INTEGER | 非必填 | 返回的每页推广位数量 |
| pid\_list | STRING\[\] | 非必填 | 推广位列表，例如：\["60005\_612"\] |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| p\_id\_query\_response | OBJECT |  | response |
| p\_id\_list | OBJECT\[\] |  | 多多进宝推广位对象列表 |
| create\_time | LONG |  | 推广位生成时间 |
| media\_id | LONG |  | 媒体id |
| pid\_name | STRING |  | 推广位名称 |
| p\_id | STRING |  | 推广位ID |
| total\_count | LONG |  | 返回推广位总数 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddDdkOauthGoodsPidQueryRequest;
...
public class PopClientDemo {
 ...
        PddDdkOauthGoodsPidQueryRequest request = new PddDdkOauthGoodsPidQueryRequest();
        request.setPage(0);
        request.setPageSize(0);
        List<String> pidList = new ArrayList<String>();
        pidList.add("str");
        request.setPidList(pidList);
        PddDdkOauthGoodsPidQueryResponse response = client.syncInvoke(request, accessToken);

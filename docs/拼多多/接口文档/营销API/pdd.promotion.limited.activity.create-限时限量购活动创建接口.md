---
title: "pdd.promotion.limited.activity.create - 限时限量购活动创建接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.promotion.limited.activity.create"
menu_path:
  - "营销API"
  - "pdd.promotion.limited.activity.create"
captured_at: "2026-04-09T15:20:31.952Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| request | OBJECT\[\] | 必填 | 创建请求 |
| activity\_name | STRING | 必填 | 活动名称 |
| activity\_type | INTEGER | 必填 | 活动类型 |
| discount | LONG | 非必填 | 折扣比例，实际折扣为：discount/1000。例：300，表示3折 |
| end\_time | LONG | 非必填 | 限时折扣必填。结束时间（单位：s） |
| goods\_id | LONG | 必填 | 商品id |
| quantity | LONG | 非必填 | 活动库存的数量（限量活动时，必填） |
| sku\_price\_list | OBJECT\[\] | 必填 | 参与活动的sku数据列表 |
| activity\_price | LONG | 必填 | sku活动价格 |
| sku\_id | LONG | 必填 | skuid |
| start\_time | LONG | 非必填 | 限时折扣必填。开始时间（单位：s） |
| user\_activity\_limit | LONG | 非必填 | 用户限购数量(0:不限购) |
...
| fail\_reason | STRING |  | 创建失败时的失败原因 |
| goods\_id | STRING |  | 商品id |
| status | INTEGER |  | 商品是否创建成功。0代表成功，非0代表失败 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddPromotionLimitedActivityCreateRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddPromotionLimitedActivityCreateResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddPromotionLimitedActivityCreateRequest.RequestItem;
import com.pdd.pop.sdk.http.api.pop.request.PddPromotionLimitedActivityCreateRequest.RequestItemSkuPriceListItem;
...
public class PopClientDemo {
 ...
        PddPromotionLimitedActivityCreateRequest request = new PddPromotionLimitedActivityCreateRequest();
        List<RequestItem> request1 = new ArrayList<RequestItem>();

        RequestItem item = new RequestItem();
        item.setActivityName("str");
        item.setActivityType(0);
        item.setDiscount(0L);
        item.setEndTime(0L);
        item.setGoodsId(0L);
        item.setQuantity(0L);
        List<RequestItemSkuPriceListItem> skuPriceList = new ArrayList<RequestItemSkuPriceListItem>();

        RequestItemSkuPriceListItem item1 = new RequestItemSkuPriceListItem();
        item1.setActivityPrice(0L);
        item1.setSkuId(0L);
        skuPriceList.add(item1);
        item.setSkuPriceList(skuPriceList);
        item.setStartTime(0L);
        item.setUserActivityLimit(0L);
        request1.add(item);
        request.setRequest(request1);
        PddPromotionLimitedActivityCreateResponse response = client.syncInvoke(request, accessToken);

---
title: "pdd.promotion.goods.coupon.list.get - 商品优惠券批次列表查询"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.promotion.goods.coupon.list.get"
menu_path:
  - "营销API"
  - "pdd.promotion.goods.coupon.list.get"
captured_at: "2026-04-09T15:20:25.680Z"
---
...
| goods\_id | LONG | 非必填 | 商品ID |
| query\_range | INTEGER | 非必填 | 查询范围 0 全部，1 多多进宝券，2 无门槛商品券；默认1 |
| batch\_status | INTEGER | 非必填 | 批次状态 1 领取中，2 已领完，3 已结束，4 已暂停 |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_coupon\_batch\_list\_response | OBJECT |  | 返回商品优惠券批次表 |
| coupon\_batch\_list | OBJECT\[\] |  | 返回商品优惠券对象 |
| id | LONG |  | 批次ID |
| batch\_desc | STRING |  | 批次描述 |
| discount\_param | LONG |  | 折扣参数，为请求中传入的discountAmount，表示折扣金额，单位: 分 |
| min\_order\_amount | LONG |  | 最小可用订单金额 |
...
| source\_type | INTEGER |  | 券来源类型，45 店铺多多进宝商品券，54 无门槛商品券，87 店铺多多进宝大淘客定向商品券，88 店铺多多果园商品券 |
| goods\_id | LONG |  | 商品ID |
| goods\_name | STRING |  | 商品名 |
| image\_url | STRING |  | 图片URL |
| status | INTEGER |  | 批次状态，1 领取中，2 已领完，3 已结束，4 已暂停 |
| created\_at | LONG |  | 批次创建时间 |
| total\_size | INTEGER |  | 返回的优惠券总数 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddPromotionGoodsCouponListGetRequest;
...
public class PopClientDemo {
 ...
        PddPromotionGoodsCouponListGetRequest request = new PddPromotionGoodsCouponListGetRequest();
        request.setPage(0);
        request.setPageSize(0);
        request.setGoodsId(0L);
        request.setQueryRange(0);
        request.setBatchStatus(0);
        request.setSortBy(0);
        PddPromotionGoodsCouponListGetResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "goods_coupon_batch_list_response": {
    "coupon_batch_list": [
      {
        "batch_desc": "str",
        "batch_end_time": 0,

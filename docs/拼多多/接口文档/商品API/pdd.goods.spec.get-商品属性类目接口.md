---
title: "pdd.goods.spec.get - 商品属性类目接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.spec.get"
menu_path:
  - "商品API"
  - "pdd.goods.spec.get"
captured_at: "2026-04-09T15:17:29.506Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| cat\_id | LONG | 必填 | 叶子类目ID，必须入参level=3时的cat\_id,否则无法返回正确的参数 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_spec\_get\_response | OBJECT |  | response |
| goods\_spec\_list | OBJECT\[\] |  | 规格列表对象 |
| cat\_id | LONG |  | 规格所属的叶子类目ID |
| parent\_spec\_id | LONG |  | 商品规格对应的ID |
| parent\_spec\_name | STRING |  | 商品规格ID对应的规格名称 |
...
public class PopClientDemo {
 ...
        request.setCatId(0L);
        PddGoodsSpecGetResponse response = client.syncInvoke(request, accessToken);

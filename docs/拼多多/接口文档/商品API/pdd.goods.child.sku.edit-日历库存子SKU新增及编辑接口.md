---
title: "pdd.goods.child.sku.edit - 日历库存子SKU新增及编辑接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.child.sku.edit"
menu_path:
  - "商品API"
  - "pdd.goods.child.sku.edit"
captured_at: "2026-04-09T15:15:27.373Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| goods\_commit\_id | LONG | 非必填 | 草稿id（未填写则新建一条商品草稿） |
| goods\_id | LONG | 必填 | 商品id |
| skus | OBJECT\[\] | 必填 | 日历库存型商品sku信息列表 |
| child\_skus | OBJECT\[\] | 必填 | 日历库存商品子sku信息列表 |
| date | STRING | 必填 | 售卖日期（“yyyy-MM-dd”） |
| group\_price | LONG | 必填 | 团购价 |
| quantity\_delta | LONG | 必填 | 库存增减 |
| single\_price | LONG | 必填 | 单买价 |
| is\_onsale | INTEGER | 非必填 | 上架状态。0=已下架，1=已上架。不传表示不做修改 |
| out\_sku\_sn | STRING | 非必填 | 与sku\_id必填其一，用于确定编辑的sku，当有多个sku的out\_sku\_sn一样时会编辑失败。 |
| sku\_id | LONG | 非必填 | 日历库存商品父skuId |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| child\_sku\_edit\_response | OBJECT |  | 子sku新增或提交结果 |
| goods\_commit\_id | LONG | 1 | 草稿id |
| goods\_id | LONG | 1 | 商品id |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsChildSkuEditRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddGoodsChildSkuEditResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsChildSkuEditRequest.SkusItem;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsChildSkuEditRequest.SkusItemChildSkusItem;
...
public class PopClientDemo {
 ...
        PddGoodsChildSkuEditRequest request = new PddGoodsChildSkuEditRequest();
        request.setGoodsCommitId(1L);
        request.setGoodsId(1L);
        List<SkusItem> skus = new ArrayList<SkusItem>();

        SkusItem item = new SkusItem();
        List<SkusItemChildSkusItem> childSkus = new ArrayList<SkusItemChildSkusItem>();

        SkusItemChildSkusItem item1 = new SkusItemChildSkusItem();
        item1.setDate("2020-04-06");
        item1.setGroupPrice(150L);
        item1.setQuantityDelta(1L);
        item1.setSinglePrice(300L);
        childSkus.add(item1);
        item.setChildSkus(childSkus);
        item.setIsOnsale(1);
        item.setOutSkuSn("str");
        item.setSkuId(0L);
        skus.add(item);
        request.setSkus(skus);
        request.setSyncGoodsOperate(1);
        PddGoodsChildSkuEditResponse response = client.syncInvoke(request, accessToken);

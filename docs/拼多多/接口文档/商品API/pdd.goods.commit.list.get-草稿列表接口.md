---
title: "pdd.goods.commit.list.get - 草稿列表接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.commit.list.get"
menu_path:
  - "商品API"
  - "pdd.goods.commit.list.get"
captured_at: "2026-04-09T15:15:33.645Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| check\_status | INTEGER | 必填 | 草稿状态（0:编辑中,1:审核中,2:审核通过,3:审核驳回） |
| goods\_id | LONG | 非必填 | 商品id |
| page | INTEGER | 必填 | 页码，最多不超过100 |
| page\_size | INTEGER | 必填 | 每页数量，最多不超过100 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_commit\_list\_get\_response | OBJECT |  | res |
| list | OBJECT\[\] |  | list |
| check\_status | INTEGER |  | 草稿状态 0:编辑中,1:审核中,2:审核通过,3:审核驳回 |
| checked\_time | INTEGER |  | 审核时间 |
| commit\_id | LONG |  | 草稿id |
| goods\_id | LONG |  | 商品id |
| goods\_name | STRING |  | 商品标题 |
| is\_shop | INTEGER |  | 是否新增 0:新增，1：修改 |
| outer\_goods\_id | STRING |  | 商家编码 |
| reject\_comment | STRING |  | 驳回原因 |
| submit\_time | INTEGER |  | 提交时间 |
| total | LONG |  | total |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsCommitListGetRequest;
...
public class PopClientDemo {
 ...
        PddGoodsCommitListGetResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "goods_commit_list_get_response": {
    "list": [
      {
        "check_status": 0,
        "checked_time": 0,

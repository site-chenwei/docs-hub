---
title: "pdd.goods.material.create - 商品素材创建接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.material.create"
menu_path:
  - "商品API"
  - "pdd.goods.material.create"
captured_at: "2026-04-09T15:17:09.136Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| content | STRING | 必填 | 素材内容（一般为图片链接） |
| file\_id | LONG | 必填 | 图片空间文件id |
| goods\_id | LONG | 必填 | 商品id |
| material\_type | INTEGER | 必填 | 素材类型（1：白底图；3：长图） |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsMaterialCreateRequest;
...
public class PopClientDemo {
 ...
        PddGoodsMaterialCreateRequest request = new PddGoodsMaterialCreateRequest();
        request.setContent("str");
        request.setFileId(0L);
        request.setGoodsId(0L);
        request.setMaterialType(0);
        PddGoodsMaterialCreateResponse response = client.syncInvoke(request, accessToken);

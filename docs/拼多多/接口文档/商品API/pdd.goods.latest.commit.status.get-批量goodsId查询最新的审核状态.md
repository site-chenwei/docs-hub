---
title: "pdd.goods.latest.commit.status.get - 批量goodsId查询最新的审核状态"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.latest.commit.status.get"
menu_path:
  - "商品API"
  - "pdd.goods.latest.commit.status.get"
captured_at: "2026-04-09T15:16:30.195Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| goods\_id\_list | LONG\[\] | 必填 | 商品id(不超过100个) |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_latest\_commit\_status\_get\_response | OBJECT |  | response |
| list | OBJECT\[\] |  | list |
| check\_status | INTEGER |  | 草稿状态 |
| goods\_id | LONG |  | 商品id |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsLatestCommitStatusGetRequest;
...
public class PopClientDemo {
 ...
        PddGoodsLatestCommitStatusGetRequest request = new PddGoodsLatestCommitStatusGetRequest();
        List<Long> goodsIdList = new ArrayList<Long>();
        goodsIdList.add(0L);
        request.setGoodsIdList(goodsIdList);
        PddGoodsLatestCommitStatusGetResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "goods_latest_commit_status_get_response": {
    "list": [
      {
        "check_status": 0,
        "goods_id": 0,

---
title: "pdd.goods.spec.id.get - 生成商家自定义的规格"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.spec.id.get"
menu_path:
  - "商品API"
  - "pdd.goods.spec.id.get"
captured_at: "2026-04-09T15:17:32.599Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| parent\_spec\_id | LONG | 必填 | 拼多多标准规格ID，可以通过pdd.goods.spec.get接口获取 |
| spec\_name | STRING | 必填 | 商家编辑的规格值，如颜色规格下设置白色属性 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_spec\_id\_get\_response | OBJECT |  | 生成规格响应对象 |
| parent\_spec\_id | LONG |  | 自定义规格所属的规格ID |
| spec\_name | STRING |  | 生成的自定义规格名称 |
...
public class PopClientDemo {
 ...
        request.setParentSpecId(0L);
        request.setSpecName("str");
        PddGoodsSpecIdGetResponse response = client.syncInvoke(request, accessToken);

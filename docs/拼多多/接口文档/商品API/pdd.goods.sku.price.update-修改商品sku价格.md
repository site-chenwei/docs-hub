---
title: "pdd.goods.sku.price.update - 修改商品sku价格"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.sku.price.update"
menu_path:
  - "商品API"
  - "pdd.goods.sku.price.update"
captured_at: "2026-04-09T15:17:23.313Z"
---
...
| market\_price | LONG | 非必填 | 参考价 （单位分） |
| market\_price\_in\_yuan | STRING | 非必填 | 参考价 （单位元） |
| sku\_price\_list | OBJECT\[\] | 必填 | 待修改的sku价格 |
| group\_price | LONG | 非必填 | 拼团购买价格（单位分） |
| is\_onsale | INTEGER | 非必填 | sku上架状态，0-已下架，1-上架中 |
| single\_price | LONG | 非必填 | 单独购买价格（单位分） |
| sku\_id | LONG | 必填 | sku标识 |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_update\_sku\_price\_response | OBJECT |  | response |
| goods\_commit\_id | LONG |  |  |
| is\_success | BOOLEAN | true | 是否成功 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSkuPriceUpdateRequest.SkuPriceListItem;
...
public class PopClientDemo {
 ...
        request.setMarketPriceInYuan("1.0");
        List<SkuPriceListItem> skuPriceList = new ArrayList<SkuPriceListItem>();

        SkuPriceListItem item = new SkuPriceListItem();
        item.setGroupPrice(0L);
        item.setIsOnsale(0);
        item.setSinglePrice(0L);
        item.setSkuId(0L);
        skuPriceList.add(item);
        request.setSkuPriceList(skuPriceList);

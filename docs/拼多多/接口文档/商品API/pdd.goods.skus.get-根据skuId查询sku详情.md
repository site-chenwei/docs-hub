---
title: "pdd.goods.skus.get - 根据skuId查询sku详情"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.skus.get"
menu_path:
  - "商品API"
  - "pdd.goods.skus.get"
captured_at: "2026-04-09T15:17:26.394Z"
---
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_sku\_get\_response | OBJECT |  | sku信息 |
| goods\_id | LONG |  | 该sku所在的商品ID |
| is\_onsale | INTEGER |  | 上下架状态，1=上架，0=下架 |
| multi\_price | LONG |  | 团购价，单位分 |
| out\_sku\_sn | STRING |  | SKU维度商家外部编码 |
| quantity | LONG |  | 库存 |
| reserve\_quantity | LONG |  | 预扣库存 |
| single\_price | LONG |  | 单买价，单位分 |
| sku\_id | LONG |  | skuId |
| spec | OBJECT\[\] |  | 规格列表 |
| parent\_spec\_id | LONG |  | 规格项ID |
| parent\_spec\_name | STRING |  | 规格项 |
| spec\_id | LONG |  | 规格ID |
| spec\_name | STRING |  | 规格值 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSkusGetRequest;
...
public class PopClientDemo {
 ...
        PddGoodsSkusGetRequest request = new PddGoodsSkusGetRequest();
        request.setGoodsId(123L);
        request.setSkuId(12333L);
        PddGoodsSkusGetResponse response = client.syncInvoke(request, accessToken);

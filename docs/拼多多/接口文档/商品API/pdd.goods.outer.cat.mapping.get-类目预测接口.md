---
title: "pdd.goods.outer.cat.mapping.get - 类目预测接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.outer.cat.mapping.get"
menu_path:
  - "商品API"
  - "pdd.goods.outer.cat.mapping.get"
captured_at: "2026-04-09T15:17:01.548Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| outer\_cat\_id | LONG | 必填 | 外部叶子类目id |
| outer\_cat\_name | STRING | 必填 | 外部叶子类目名称 |
| outer\_goods\_name | STRING | 必填 | 外部商品名称 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| outer\_cat\_mapping\_get\_response | OBJECT |  | response |
| cat\_id1 | LONG | 1 | 一级类目 |
| cat\_id2 | LONG | 2 | 二级类目 |
| cat\_id3 | LONG | 3 | 三级类目 |
| cat\_id4 | LONG | 4 | 四级类目 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsOuterCatMappingGetRequest;
...
public class PopClientDemo {
 ...
        PddGoodsOuterCatMappingGetRequest request = new PddGoodsOuterCatMappingGetRequest();
        request.setOuterCatId(1L);
        request.setOuterCatName("catName");
        request.setOuterGoodsName("goodsName");
        PddGoodsOuterCatMappingGetResponse response = client.syncInvoke(request, accessToken);

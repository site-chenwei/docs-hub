---
title: "pdd.goods.submit.goods.commit - 编辑并提交草稿接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.submit.goods.commit"
menu_path:
  - "商品API"
  - "pdd.goods.submit.goods.commit"
captured_at: "2026-04-09T15:17:42.183Z"
---
...
| carousel\_video\_url | STRING | 非必填 | 轮播视频字段 |
| cat\_id | LONG | 非必填 | 叶子类目ID |
...
| ignore\_edit\_warn | BOOLEAN | 非必填 | 是否获取商品发布警告信息，默认为否 |
...
| operate\_type | INTEGER | 非必填 | 0:提交， 1：保存（默认提交） |
...
| bonded\_warehouse\_key | STRING | 非必填 | 保税仓唯一标识 |
...
| is\_onsale | INTEGER | 非必填 | sku上架状态，0-已下架，1-上架中 |
| length | LONG | 非必填 | sku送装参数：长度 |
| limit\_quantity | LONG | 非必填 | sku购买限制，只入参999 |
| multi\_price | LONG | 非必填 | 商品团购价格 |
| out\_sku\_sn | STRING | 非必填 | 商品sku外部编码 |
| out\_source\_sku\_id | STRING | 非必填 | 第三方sku Id |
| oversea\_sku | OBJECT | 非必填 | oversea\_sku |
| measurement\_code | STRING | 非必填 | 计量单位编码，从接口pdd.gooods.sku.measurement.list获取code |
| specifications | STRING | 非必填 | 规格 |
| taxation | INTEGER | 非必填 | 税费 |
| price | LONG | 非必填 | 商品单买价格 |
| quantity | LONG | 非必填 | 商品sku库存初始数量，后续库存update只使用stocks.update接口进行调用 |
| sku\_pre\_sale\_time | INTEGER | 非必填 | sku预售时间戳，单位秒；不更新传null，取消传0，更新传实际值 |
| sku\_properties | OBJECT\[\] | 非必填 | sku属性 |
| punit | STRING | 非必填 | 属性单位 |
| ref\_pid | LONG | 非必填 | 属性id |
| value | STRING | 非必填 | 属性值 |
| vid | LONG | 非必填 | 属性值id |
| spec\_id\_list | LONG\[\] | 非必填 | 商品规格列表，根据pdd.goods.spec.id.get生成的规格属性id，例如：颜色规格下商家新增白色和黑色，大小规格下商家新增L和XL，则由4种spec组合，入参一种组合即可，在skulist中需要有4个spec组合的sku |
| thumb\_url | STRING | 非必填 | sku预览图，预览图尺寸：等宽高，且高度不低于480px，现已支持1M大小，越清晰越好卖，SKU预览图格式：仅支持JPG,PNG格式 |
| weight | LONG | 非必填 | 重量，单位为g |
| sku\_type | INTEGER | 非必填 | 库存方式（0：普通型，1：日历型） |
| song\_huo\_an\_zhuang | STRING | 非必填 | 送货入户并安装模版id |
| song\_huo\_ru\_hu | STRING | 非必填 | 送货入户模版id |
| sync\_goods\_operate | INTEGER | 非必填 | 提交后上架状态，0:上架,1:保持原样 |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_update\_response | OBJECT |  | 返回response |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddGoodsSubmitGoodsCommitResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.CarouselVideoItem;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.ElecGoodsAttributes;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.GoodsPropertiesItem;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.GoodsPropertiesItemTableInfo;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.GoodsPropertiesItemTableInfoTableValueListItem;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.GoodsTradeAttr;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.GoodsTradeAttrBookingNotes;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.GoodsTravelAttr;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.OverseaGoods;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.SkuListItem;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.SkuListItemOverseaSku;
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsSubmitGoodsCommitRequest.SkuListItemSkuPropertiesItem;
...
public class PopClientDemo {
 ...
        specIdList.add(0L);
        item3.setSpecIdList(specIdList);
 ...
        PddGoodsSubmitGoodsCommitResponse response = client.syncInvoke(request, accessToken);

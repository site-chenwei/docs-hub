---
title: "pdd.goods.cps.unit.query - 查询商品推广API"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.cps.unit.query"
menu_path:
  - "商品API"
  - "pdd.goods.cps.unit.query"
captured_at: "2026-04-09T15:15:58.628Z"
---
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| coupon\_vo | OBJECT |  | 计划使用的优惠券信息。商品没有优惠券将返回null，有优惠券将返回优惠券信息，包括优惠券起始结束时间，优惠券数量，剩余数量，金额等 |
| coupon\_end\_time | STRING |  | coupon\_end\_time |
| coupon\_id | LONG |  | coupon\_id |
| coupon\_start\_time | STRING |  | coupon\_start\_time |
| discount | INTEGER |  | discount |
| init\_quantity | LONG |  | init\_quantity |
| remain\_quantity | LONG |  | remain\_quantity |
| goods\_id | LONG |  | 商品id |
| rate | INTEGER |  | 佣金比例（千分比） |
| rate\_to\_be | INTEGER |  | 即将生效的全店推广计划佣金比（千分比），若没有则为null |
| rate\_to\_be\_day | STRING |  | 通用推广计划佣金生效时间 |
| status | INTEGER |  | 商品状态，1-推广中，2-暂停，3-删除 |
| status\_to\_be | INTEGER |  | 即将生效的全店推广计划状态：1-推广中，2-暂停推广，若没有则为null |
| status\_to\_be\_day | STRING |  | 通用推广计划状态生效时间 |
| unit\_id | LONG |  | 单品推广计划单元id |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsCpsUnitQueryRequest;
...
public class PopClientDemo {
 ...
        PddGoodsCpsUnitQueryRequest request = new PddGoodsCpsUnitQueryRequest();
        request.setGoodsId(0L);
        PddGoodsCpsUnitQueryResponse response = client.syncInvoke(request, accessToken);

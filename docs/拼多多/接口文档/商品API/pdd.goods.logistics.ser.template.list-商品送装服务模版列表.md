---
title: "pdd.goods.logistics.ser.template.list - 商品送装服务模版列表"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.logistics.ser.template.list"
menu_path:
  - "商品API"
  - "pdd.goods.logistics.ser.template.list"
captured_at: "2026-04-09T15:16:39.726Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| length | INTEGER | 必填 | 查询大小 |
| query\_type | INTEGER | 必填 | 查询类型 |
| start | INTEGER | 必填 | 查询偏移量 |
| template\_type | INTEGER | 必填 | 模板类型 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_logistics\_ser\_template\_list\_response | OBJECT |  | response |
| list | OBJECT\[\] |  | 列表 |
| template\_id | STRING |  | 模版id |
| template\_name | STRING |  | 模版名称 |
| update\_time | INTEGER |  | 更新时间 |
| using | INTEGER |  | 使用情况 |
| total | INTEGER |  | 总数 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsLogisticsSerTemplateListRequest;
...
public class PopClientDemo {
 ...
        PddGoodsLogisticsSerTemplateListResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "goods_logistics_ser_template_list_response": {
    "list": [
      {
        "template_id": "str",
        "template_name": "str",

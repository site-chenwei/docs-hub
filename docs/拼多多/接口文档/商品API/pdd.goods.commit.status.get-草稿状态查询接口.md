---
title: "pdd.goods.commit.status.get - 草稿状态查询接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.commit.status.get"
menu_path:
  - "商品API"
  - "pdd.goods.commit.status.get"
captured_at: "2026-04-09T15:15:36.823Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| goods\_commit\_id\_list | LONG\[\] | 必填 | goods\_commit\_id列表 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_commit\_status\_get\_response | OBJECT |  | response |
| list | OBJECT\[\] |  | list |
| check\_status | INTEGER |  | 状态 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsCommitStatusGetRequest;
...
public class PopClientDemo {
 ...
        PddGoodsCommitStatusGetRequest request = new PddGoodsCommitStatusGetRequest();
        List<Long> goodsCommitIdList = new ArrayList<Long>();
        goodsCommitIdList.add(0L);
        request.setGoodsCommitIdList(goodsCommitIdList);
        PddGoodsCommitStatusGetResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "goods_commit_status_get_response": {
    "list": [
      {
        "check_status": 0,
        "goods_commit_id": 0,

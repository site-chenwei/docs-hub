---
title: "pdd.goods.quantity.update - 商品库存更新接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.quantity.update"
menu_path:
  - "商品API"
  - "pdd.goods.quantity.update"
captured_at: "2026-04-09T15:17:07.760Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| force\_update | BOOLEAN | 非必填 | 是否强制更新，仅update\_type=1(全量更新)时有效, 默认值false； force\_update=false时，quantity不能小于预扣库存； force\_update=true时，代表强制更新，当quantity<预扣库存时，不报错，直接将quantity清0 |
| goods\_id | LONG | 必填 | 商品id |
| outer\_id | STRING | 非必填 | sku商家编码 |
...
| sku\_id | LONG | 非必填 | sku\_id和outer\_id必填一个 |
| update\_type | INTEGER | 非必填 | 库存更新方式，可选。1为全量更新，2为增量更新。如果不填，默认为全量更新 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_quantity\_update\_response | OBJECT |  |  |
| is\_success | BOOLEAN |  |  |
...
public class PopClientDemo {
 ...
        request.setGoodsId(111111L);
        request.setOuterId("11111");
        request.setQuantity(11L);
        request.setSkuId(11111L);

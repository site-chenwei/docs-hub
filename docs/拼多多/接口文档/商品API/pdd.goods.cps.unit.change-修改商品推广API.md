---
title: "pdd.goods.cps.unit.change - 修改商品推广API"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.cps.unit.change"
menu_path:
  - "商品API"
  - "pdd.goods.cps.unit.change"
captured_at: "2026-04-09T15:15:49.267Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| coupon\_end\_time | STRING | 非必填 | 优惠券结束时间 |
| coupon\_id | LONG | 非必填 | 优惠券id |
| coupon\_sn | STRING | 非必填 | 优惠券号 |
| coupon\_start\_time | STRING | 非必填 | 优惠券开始时间 |
| discount | INTEGER | 非必填 | 优惠券面额（单位为分） |
| goods\_id | LONG | 必填 | 商品id |
| init\_quantity | LONG | 非必填 | 设置的优惠券张数 |
| rate | INTEGER | 必填 | 佣金比例（千分比） |
| remain\_quantity | LONG | 非必填 | 优惠券剩余数量 |
| duration | INTEGER | 非必填 | 优惠券领取后的有效使用时间天数 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| is\_change\_success | BOOLEAN |  | 是否修改成功 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsCpsUnitChangeRequest;
...
public class PopClientDemo {
 ...
        PddGoodsCpsUnitChangeRequest request = new PddGoodsCpsUnitChangeRequest();
 ...
        request.setInitQuantity(0L);
        request.setRate(0);
        request.setRemainQuantity(0L);
        request.setDuration(0);
        PddGoodsCpsUnitChangeResponse response = client.syncInvoke(request, accessToken);

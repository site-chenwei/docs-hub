---
title: "pdd.goods.material.query - 商品素材列表查询"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.goods.material.query"
menu_path:
  - "商品API"
  - "pdd.goods.material.query"
captured_at: "2026-04-09T15:16:55.296Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| goods\_id\_list | LONG\[\] | 必填 | 商品id列表 |
| type\_list | LONG\[\] | 必填 | 素材类型列表 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| material\_list | OBJECT\[\] |  |  |
| appeal\_check\_comment | STRING |  | 申诉审核信息 |
| check\_comment | STRING |  | 审核信息 |
| check\_status | INTEGER |  | 审核状态（1：待审核；2：通过；3：驳回；101：申诉待审核；102：申诉通过；103：申诉驳回） |
| content | STRING |  | 素材内容 |
| goods\_id | LONG |  | 商品id |
| material\_id | LONG |  | 素材id |
| online\_material | STRING |  | 线上素材 |
| type | INTEGER |  | 素材类型（1：白底图；3：长图） |
...
import com.pdd.pop.sdk.http.api.pop.request.PddGoodsMaterialQueryRequest;
...
public class PopClientDemo {
 ...
        PddGoodsMaterialQueryRequest request = new PddGoodsMaterialQueryRequest();
 ...
        PddGoodsMaterialQueryResponse response = client.syncInvoke(request, accessToken);
...
| --- | --- |
| 图片空间管理权限包 | 进销存、商品优化分析、虚拟商家后台系统、企业ERP、商家后台系统 |

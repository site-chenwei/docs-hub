---
title: "pdd.invoice.application.query - 开票申请单查询"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.invoice.application.query"
menu_path:
  - "发票服务API"
  - "pdd.invoice.application.query"
captured_at: "2026-04-09T15:21:22.399Z"
---

# pdd.invoice.application.query

## 开票申请单查询

更新时间：2026-02-10 22:13:19

¥免费API

必须用户授权

当消费者在拼多多平台申请开票之后，第三方ERP通过此接口获取开票申请信息

### 公共参数

#### 请求地址（目前只提供正式环境，暂无沙箱环境）

| 环境 | HTTP地址 | HTTPS地址 |
| --- | --- | --- |
| 正式环境 | http://gw-api.pinduoduo.com/api/router | https://gw-api.pinduoduo.com/api/router |

#### 公共请求参数

| 参数名称 | 参数类型 | 是否必填 | 参数描述 |
| --- | --- | --- | --- |
| type | String | 必填 | API接口名称 |
| client\_id | String | 必填 | POP分配给应用的client\_id |
| access\_token | String | 非必填 | 通过code获取的access\_token |
| timestamp | String | 必填 | UNIX时间戳，单位秒，需要与拼多多服务器时间差值在10分钟内 |
| data\_type | String | 非必填 | 响应格式，即返回数据的格式，JSON或者XML（二选一），默认JSON，注意是大写 |
| version | String | 非必填 | API协议版本号，默认为V1，可不填 |
| sign | String | 必填 | API输入参数签名结果，签名算法参考开放平台接入指南第三部分底部。 |

### 请求参数说明

| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| order\_sn | STRING | 非必填 | 订单号；订单号和申请时间必填其一 |
| page | INTEGER | 非必填 | 页码，默认1 |
| page\_size | INTEGER | 非必填 | 每页返回数目，默认50 |
| quality\_goods\_invoice | INTEGER | 非必填 | 是否正品发票 0=非正品发票 1=是正品发票 |
| status | INTEGER | 非必填 | 申请状态：0-已拒绝，1-申请中，2-已同意 |
| update\_end\_time | LONG | 非必填 | 申请结束时间, 时间戳（单位毫秒，查询时间间隔不可超过15天） |
| update\_start\_time | LONG | 非必填 | 申请开始时间, 时间戳（单位毫秒，查询时间间隔不可超过15天） |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| invoice\_application\_query\_response | OBJECT |  | response |
| invoice\_application\_list | OBJECT\[\] |  | 发票申请列表 |
| application\_status | INTEGER | 1 | 申请状态：0-已拒绝，1-申请中，2-已同意 |
| apply\_time | INTEGER |  | 申请时间 |
| business\_type | INTEGER | 1 | 抬头类型：0-个人，1-企业 |
| invoice\_amount | STRING | 100 | 开票金额，单位：分 |
| invoice\_kind | INTEGER | 0 | 发票种类：0-电子，1-纸质，2-专票；目前只支持0和2 |
| invoice\_type | INTEGER | 0 | 发票类型：0-蓝票，1-红票 |
| invoice\_way | INTEGER | 0 | 开票方式 0=手动开票,1=自动开票 |
| mall\_id | LONG | 6040724 | 店铺id |
| memo | STRING |  | 备注 |
| order\_sn | STRING | 200310-357983846491793 | 订单号 |
| payer\_account | STRING |  | （企业抬头）开户账号 |
| payer\_address | STRING |  | （企业抬头）地址 |
| payer\_bank | STRING |  | （企业抬头）开户银行 |
| payer\_name | STRING | 中环有限公司 | 发票抬头 |
| payer\_phone | STRING |  | （企业抬头）电话 |
| payer\_register\_no | STRING | 32532232432543 | 企业税号，抬头为企业类型必填 |
| quality\_goods\_invoice | INTEGER | 0 | 是否正品发票：0=非正品发票，1=正品发票 |
| reason | STRING |  | 驳回原因 |
| sum\_price | STRING |  | 不含税金额，暂为null |
| sum\_tax | STRING |  | 总税额，暂为null |
| tax\_rate | STRING |  | 税率，暂为null |
| trigger\_status | INTEGER | 1 | 开票申请触发类型：1-申请开票，2-改抬头 |

### 请求示例

JAVA

CURL

```
package com.pdd.pop.sdk.http.demo;

import com.pdd.pop.sdk.common.util.JsonUtil;
import com.pdd.pop.sdk.http.api.pop.request.PddInvoiceApplicationQueryRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddInvoiceApplicationQueryResponse;
import com.pdd.pop.sdk.http.PopClient;
import com.pdd.pop.sdk.http.PopHttpClient;

public class PopClientDemo {

    public static void main(String[] args) throws Exception {

        String clientId = "your clientId";
        String clientSecret = "your clientSecret";
        String accessToken = "your accessToken";
        PopClient client = new PopHttpClient(clientId, clientSecret);

        PddInvoiceApplicationQueryRequest request = new PddInvoiceApplicationQueryRequest();
        request.setOrderSn("str");
        request.setPage(1);
        request.setPageSize(10);
        request.setQualityGoodsInvoice(1);
        request.setStatus(1);
        request.setUpdateEndTime(1651824077510L);
        request.setUpdateStartTime(1651824077510L);
        PddInvoiceApplicationQueryResponse response = client.syncInvoke(request, accessToken);
        System.out.println(JsonUtil.transferToJson(response));
    }
}
```

### 响应示例

```
{
  "invoice_application_query_response": {
    "invoice_application_list": [
      {
        "application_status": 1,
        "apply_time": 0,
        "business_type": 1,
        "invoice_amount": "100",
        "invoice_kind": 0,
        "invoice_type": 0,
        "invoice_way": 0,
        "mall_id": 6040724,
        "memo": "str",
        "order_sn": "200310-357983846491793",
        "payer_account": "str",
        "payer_address": "str",
        "payer_bank": "str",
        "payer_name": "中环有限公司",
        "payer_phone": "str",
        "payer_register_no": "32532232432543",
        "quality_goods_invoice": 0,
        "reason": "str",
        "sum_price": "str",
        "sum_tax": "str",
        "tax_rate": "str",
        "trigger_status": 1
      }
    ]
  }
}
```

### 异常示例

JSON

XML

```

{
  "error_response": {
    "error_msg": "公共参数错误:type",
    "sub_msg": "",
    "sub_code": null,
    "error_code": 10001,
    "request_id": "15440104776643887"
  }
}
```

### 相关权限包

| 拥有此接口的权限包 | 可获得/可申请此权限包的应用类型 |
| --- | --- |
| 电子发票权限包 | 虚拟商家后台系统、企业ERP、商家后台系统、跨境企业ERP报关版 |

### 返回错误码说明

| 主错误码 | 主错误描述 | 子错误码 | 子错误描述 | 解决办法 |
| --- | --- | --- | --- | --- |
| 10000 | 参数错误 | 10000 | 参数错误 | 参数值有误，按照文档要求填写请求参数 |
| 10001 | 公共参数错误 | 10001 | 公共参数错误 | 请检查请求的公共参数 |
| 10016 | client下线或者clientId不正确 | 10016 | client下线或者clientId不正确 | 请核查您的client\_id是否正确 |
| 10017 | type不正确 | 10017 | type不正确 | 检查type是否正确 |
| 10018 | target\_client\_id下线或者target\_client\_id不正确 | 10018 | target\_client\_id下线或者target\_client\_id不正确 | 检查target\_client\_id 是否正确 |
| 10019 | access\_token已过期 | 10019 | access\_token已过期 | 刷新access\_token或者重新授权再次获取access\_token |
| 20004 | 签名sign校验失败 | 20004 | 签名sign校验失败 | 请按照接入指南第三部分指导，生成签名 |
| 20005 | ip无权访问接口，请加入ip白名单 | 20005 | ip无权访问接口，请加入ip白名单 | 把ip白名单加入白名单 |
| 20031 | 用户没有授权访问此接口 | 20031 | 用户没有授权访问此接口 | 您创建的应用中不包含此接口，请查看API文档，了解相关权限包 |
| 20032 | access\_token或client\_id错误 | 20032 | access\_token或client\_id错误 | 检查access\_token或client\_id |
| 20034 | 接口处于下线状态 | 20034 | 接口处于下线状态 | 检查接口状态 |
| 20035 | 接口不属于当前网关 | 20035 | 接口不属于当前网关 | 判断调用网关url 是否正确 |
| 21001 | 请求参数错误 | 21001 | 请求参数错误 | 业务参数输入错误 |
| 21002 | 请求参数不能为空 | 21002 | 请求参数不能为空 | 检查业务参数必填是否已填 |
| 30000 | 没有调用此target接口权限 | 30000 | 没有调用此target接口权限 | 检查是否获得调用此target接口权限 |
| 30001 | client\_id和partner\_id不匹配 | 30001 | client\_id和partner\_id不匹配 | 检查partner\_id是否正确 |
| 50000 | 系统内部错误 | 50000 | 系统内部错误 | 系统内部错误，请加群联系相关负责人 |
| 50002 | 业务系统内部异常 | 50002 | 业务系统内部异常 | 请加群联系相关负责人 |
| 52001 | 网关业务服务错误 | 52001 | 网关业务服务错误 | 联系技术支持解决 |
| 52002 | 网关系统内部异常 | 52002 | 网关系统内部异常 | 联系技术支持解决 |
| 52004 | 请求body 太大 | 52004 | 请求body 太大 | 检查请求体是否过大 |
| 52101 | 当前接口被限流，请稍后重试 | 52101 | 当前接口被限流，请稍后重试 | 当前接口被限流，请稍后重试 |
| 52102 | 当前接口暂时不可用，请稍后重试 | 52102 | 当前接口暂时不可用，请稍后重试 | 当前接口被降级，请稍后重试 |
| 52103 | 服务暂时不可用，请稍后重试 | 52103 | 服务暂时不可用，请稍后重试 | 当前接口被降级，请稍后重试 |
| 70031 | 调用过于频繁，请调整调用频率 | 70031 | 调用过于频繁，请调整调用频率 | 调用过于频繁，请调整调用频率 |
| 70032 | 当前请求被禁止调用 | 70032 | 当前请求被禁止调用 | 当前请求被禁止调用 |
| 70033 | 当前接口因系统维护，暂时下线，请稍后再试！ | 70033 | 当前接口因系统维护，暂时下线，请稍后再试！ | 当前接口因系统维护，暂时下线，请稍后再试！ |
| 70034 | 当前用户或应用存在风险，禁止调用！ | 70034 | 当前用户或应用存在风险，禁止调用！ | 当前用户或应用存在风险，禁止调用！ |
| 70035 | 当前用户或应用存在风险，禁止调用。请联系ddjb@pinduoduo.com | 70035 | 当前用户或应用存在风险，禁止调用。请联系ddjb@pinduoduo.com | 当前用户或应用存在风险，禁止调用。请联系ddjb@pinduoduo.com |
| 70036 | 应用处于测试状态，调用次数被限制 | 70036 | 应用处于测试状态，调用次数被限制 | 应用处于测试状态，调用次数达到上限被限制 |
| 50001 | 业务服务错误 | 2000000 | 错误code对应的errorMsg报错描述 | 按errorMsg提示传入正确参数 |
| 4000000 | 系统错误 | 稍后重试，多次重试不成功联系开发 |

### 限流规则

接口总限流频次：7500次/1秒

商家限流频次：10000次/60秒

### API工具

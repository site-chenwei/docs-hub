---
title: "pdd.ticket.order.create.notifycation - 旅游门票订单创建异步回调接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ticket.order.create.notifycation"
menu_path:
  - "旅游门票API"
  - "pdd.ticket.order.create.notifycation"
captured_at: "2026-04-09T15:25:24.853Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| code\_type | INTEGER | 非必填 | 码类型。status=2时必填。1.无凭证(身份证/手机号) 2. 数字码 3.QR图片 4.外链 |
| failed\_code | INTEGER | 非必填 | 失败错误码。status=3时必填 |
| failed\_reason | STRING | 非必填 | 失败原因。 status=3时必填 |
| order\_no | STRING | 必填 | 拼多多制票号 |
| out\_order\_sn | STRING | 必填 | isv订单号 |
| status | INTEGER | 必填 | 制码状态。 2.制作成功 3.制作失败 |
| tickets | OBJECT\[\] | 非必填 | 凭证信息列表。status=2 且 code\_type!=1 时必填 |
| additional | STRING | 非必填 | 辅助凭证，有辅助凭证时返回 |
| code | STRING | 非必填 | 主凭证，code\_type=2时返回 |
| file | STRING | 非必填 | 文件base64流，code\_type=3时返回，大小小于800KB |
| url | STRING | 非必填 | 外链，code\_type=4时返回 |
| ticket\_type | INTEGER | 非必填 | 凭证类型。status=2时必填。1.一人一码 2.一人多码 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddTicketOrderCreateNotifycationRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddTicketOrderCreateNotifycationResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddTicketOrderCreateNotifycationRequest.TicketsItem;
...
public class PopClientDemo {
 ...
        PddTicketOrderCreateNotifycationRequest request = new PddTicketOrderCreateNotifycationRequest();
        request.setCodeType(0);
        request.setFailedCode(0);
        request.setFailedReason("str");
        request.setOrderNo("str");
        request.setOutOrderSn("str");
        request.setStatus(0);
        List<TicketsItem> tickets = new ArrayList<TicketsItem>();

        TicketsItem item = new TicketsItem();
        item.setAdditional("str");
        item.setCode("str");
        item.setFile("str");
        item.setUrl("str");
        tickets.add(item);
 ...
        PddTicketOrderCreateNotifycationResponse response = client.syncInvoke(request, accessToken);

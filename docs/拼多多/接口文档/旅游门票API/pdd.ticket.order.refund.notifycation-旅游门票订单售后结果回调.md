---
title: "pdd.ticket.order.refund.notifycation - 旅游门票订单售后结果回调"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ticket.order.refund.notifycation"
menu_path:
  - "旅游门票API"
  - "pdd.ticket.order.refund.notifycation"
captured_at: "2026-04-09T15:25:08.165Z"
---
...
| refund\_amount | LONG | 非必填 | 退款金额（分） status=2时必传 |
| reject\_reason | STRING | 非必填 | 驳回原因 status=3时必传 |
| serial\_no | STRING | 必填 | 退款流水号 |
| status | INTEGER | 必填 | 受理状态。2.已通过 3.已驳回 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddTicketOrderRefundNotifycationRequest;
...
public class PopClientDemo {
 ...
        PddTicketOrderRefundNotifycationRequest request = new PddTicketOrderRefundNotifycationRequest();
        request.setOrderNo("str");
        request.setRefundAmount(0L);
        request.setRejectReason("str");
        request.setSerialNo("str");
        request.setStatus(0);
        PddTicketOrderRefundNotifycationResponse response = client.syncInvoke(request, accessToken);

---
title: "pdd.ticket.scenic.get - 旅游门票拼多多景区编码查询"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ticket.scenic.get"
menu_path:
  - "旅游门票API"
  - "pdd.ticket.scenic.get"
captured_at: "2026-04-09T15:25:11.239Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| city\_code | LONG | 非必填 | 城市编码 |
| location\_type | INTEGER | 必填 | 定位类型 1.百度 2.google |
| scenic\_id | LONG | 非必填 | 拼多多景区 ID |
| scenic\_name | STRING | 必填 | 景区简称（至少两个字） |
...
| area\_list | OBJECT\[\] |  |  |
| latitude | DOUBLE |  | 纬度 |
| location\_type | INTEGER |  | 定位类型 1.百度 2.google |
| longitude | DOUBLE |  | 经度 |
| scenic\_id | LONG |  | 拼多多景区编码 |
| scenic\_name | STRING |  | 拼多多景区名称 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddTicketScenicGetRequest;
...
public class PopClientDemo {
 ...
        PddTicketScenicGetRequest request = new PddTicketScenicGetRequest();
        request.setCityCode(0L);
        request.setLocationType(0);
        request.setScenicId(0L);
        request.setScenicName("str");
        PddTicketScenicGetResponse response = client.syncInvoke(request, accessToken);

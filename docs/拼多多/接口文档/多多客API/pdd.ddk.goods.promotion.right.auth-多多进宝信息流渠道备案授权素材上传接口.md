---
title: "pdd.ddk.goods.promotion.right.auth - 多多进宝信息流渠道备案授权素材上传接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ddk.goods.promotion.right.auth"
menu_path:
  - "多多客API"
  - "pdd.ddk.goods.promotion.right.auth"
captured_at: "2026-04-09T15:18:38.645Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| demo\_url | STRING | 必填 | 推广商品视频素材url |
| duo\_id | LONG | 必填 | 渠道duoId |
| goods\_id | LONG | 必填 | 商品GoodsId |
| mall\_certificate\_url | STRING\[\] | 必填 | 商家资质证明图片url列表，1到3张图 |
| promotion\_code\_url | STRING | 必填 | 推广视频预览码url |
| promotion\_end\_time | LONG | 必填 | 推广结束时间戳，毫秒 |
| promotion\_start\_time | LONG | 必填 | 推广开始时间戳，毫秒 |
| thumb\_pic\_url | STRING\[\] | 非必填 | 商品图片素材url列表，0到3张图 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| goods\_promotion\_right\_auth\_response | OBJECT |  | 返回结果 |
| reason | STRING |  | 备案失败原因 |
| result | BOOLEAN |  | 备案结果 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddDdkGoodsPromotionRightAuthRequest;
...
public class PopClientDemo {
 ...
        PddDdkGoodsPromotionRightAuthRequest request = new PddDdkGoodsPromotionRightAuthRequest();
        request.setDemoUrl("str");
        request.setDuoId(123456L);
        request.setGoodsId(324734634444L);
        List<String> mallCertificateUrl = new ArrayList<String>();
        mallCertificateUrl.add("str");
        request.setMallCertificateUrl(mallCertificateUrl);
        request.setPromotionCodeUrl("str");
        request.setPromotionEndTime(1663227237000L);
        request.setPromotionStartTime(1663227237000L);
        List<String> thumbPicUrl = new ArrayList<String>();
        thumbPicUrl.add("str");
        request.setThumbPicUrl(thumbPicUrl);
        PddDdkGoodsPromotionRightAuthResponse response = client.syncInvoke(request);
...
```
{
  "goods_promotion_right_auth_response": {
    "reason": "str",
    "result": false
  }
}

---
title: "pdd.ddk.promotion.goods.query - 多多进宝信息流投放商品报备进度查询"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ddk.promotion.goods.query"
menu_path:
  - "多多客API"
  - "pdd.ddk.promotion.goods.query"
captured_at: "2026-04-09T15:18:44.944Z"
---
...
| mall\_id | LONG | 非必填 | 店铺id |
| page\_number | INTEGER | 非必填 | 分页查询页数 |
| page\_size | INTEGER | 非必填 | 分页查询页大小 |
| status\_list | INTEGER\[\] | 非必填 | 查询状态列表 |
| update\_end\_time | LONG | 非必填 | 最后更新开始时间 |
| update\_start\_time | LONG | 非必填 | 最后更新结束时间（最长支持30天） |
...
| --- | --- | --- | --- |
| response | OBJECT |  |  |
| application\_list | OBJECT\[\] |  | 报备记录列表 |
| comment | STRING |  | 审核信息 |
| commit\_time | LONG |  | 报备提交时间 |
| goods\_id | LONG |  | 商品id |
| mall\_id | LONG |  | 店铺id |
| promotion\_end\_time | LONG |  | 推广结束时间 |
| promotion\_start\_time | LONG |  | 推广开始时间 |
| status | INTEGER |  | 报备状态。0-已创建，1-已提交，2-已通过，3-已驳回 |
| updated\_at | LONG |  | 最后更新时间 |
| total | INTEGER |  | 报备记录总数 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddDdkPromotionGoodsQueryRequest;
...
public class PopClientDemo {
 ...
        PddDdkPromotionGoodsQueryRequest request = new PddDdkPromotionGoodsQueryRequest();
        request.setGoodsId(330698928L);
        request.setMallId(418468305787L);
        request.setPageNumber(1);
        request.setPageSize(20);
        List<Integer> statusList = new ArrayList<Integer>();
        statusList.add(0);
        request.setStatusList(statusList);
        request.setUpdateEndTime(1690819200000L);
        request.setUpdateStartTime(1688140800000L);
        PddDdkPromotionGoodsQueryResponse response = client.syncInvoke(request);

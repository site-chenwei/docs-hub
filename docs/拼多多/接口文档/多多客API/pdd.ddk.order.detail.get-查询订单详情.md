---
title: "pdd.ddk.order.detail.get - 查询订单详情"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.ddk.order.detail.get"
menu_path:
  - "多多客API"
  - "pdd.ddk.order.detail.get"
captured_at: "2026-04-09T15:19:22.732Z"
---
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| order\_detail\_response | OBJECT |  | order\_detail\_response |
...
| goods\_price | LONG |  | 商品价格（分） |
| goods\_quantity | LONG |  | 商品数量 |
...
| group\_id | LONG |  | 成团编号 |
| in\_ten\_million\_subsidy\_quota | INTEGER |  | 计入千万补贴额度(仅top渠道享受) 值为0时不计入 其他为null |
...
| not\_in\_ten\_million\_subsidy\_quota\_reason | STRING |  | 不计入千万补贴额度原因 |
| order\_amount | LONG |  | 订单价格（分） |
| order\_create\_time | LONG |  | 订单创建时间（UNIX时间戳） |
| order\_group\_success\_time | LONG |  | 订单成团时间（UNIX时间戳） |
| order\_modify\_at | LONG |  | 订单最后更新时间（UNIX时间戳） |
| order\_pay\_time | LONG |  | 订单支付时间（UNIX时间戳） |
| order\_receive\_time | LONG |  | 订单确认收货时间（UNIX时间戳） |
| order\_settle\_time | LONG |  | 订单结算时间（UNIX时间戳） |
| order\_sn | STRING |  | 订单编号 |
| order\_status | INTEGER |  | 订单状态 |
| order\_status\_desc | STRING |  | 订单状态：0-已支付；1-已成团；2-确认收货；3-审核成功；4-审核失败（不可提现）；5-已经结算 ;10-已处罚 |
| order\_verify\_time | LONG |  | 订单审核时间（UNIX时间戳） |
| pid | STRING |  | 推广位id |
| platform\_discount | LONG |  | 平台券金额，表示该订单使用的平台券金额，单位分 |
| point\_time | LONG |  | 打点时间 |
...
| return\_status | INTEGER |  | 售后状态：0：无，1：售后中，2：售后完成 |
...
| subsidy\_order\_remark | STRING |  | 补贴订单备注 |
...
| url\_last\_generate\_time | LONG |  | 链接最后一次生产时间 |
...
public class PopClientDemo {
 ...
        request.setOrderSn("211019-1231231231211");
...
```
{
 ...
    "group_id": 0,
    "in_ten_million_subsidy_quota": 0,
    "is_direct": 0,
    "mall_id": 0,
    "mall_name": "str",
    "no_subsidy_reason": "str",
    "not_in_ten_million_subsidy_quota_reason": "str",
    "order_amount": 0,
    "order_create_time": 0,

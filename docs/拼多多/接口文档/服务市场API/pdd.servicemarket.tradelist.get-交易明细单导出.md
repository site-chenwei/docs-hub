---
title: "pdd.servicemarket.tradelist.get - 交易明细单导出"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.servicemarket.tradelist.get"
menu_path:
  - "服务市场API"
  - "pdd.servicemarket.tradelist.get"
captured_at: "2026-04-09T15:23:46.565Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| begin\_time | INTEGER | 必填 | 查询起始时间，精确到秒，起止时间间隔最大31天 |
| end\_time | INTEGER | 必填 | 查询结束时间，精确到秒，起止时间间隔最大31天 |
| group\_type | INTEGER | 非必填 | 收支类型，空-全部 1-收入 2-支出 |
| page | INTEGER | 必填 | 分页页码，最大100 |
| page\_size | INTEGER | 必填 | 分页大小，最大100 |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| mall\_balance\_flow\_search\_response | OBJECT |  | response |
| data | OBJECT\[\] |  | 交易流水列表 |
| amount | LONG | 1000 | 交易金额，单位分 |
| class\_name | STRING | 交易收入 | 流水类目名称，1-交易收入，2-优惠券结算，3-退款，4-提现，5-技术服务费，7-扣款 |
| created\_at | INTEGER | 1578583502 | 流水创建时间，单位秒 |
| group\_type | INTEGER | 1 | 资金流向，1-收入，2-支出 |
| service\_order\_sn | STRING | 200109-137080340852553 | 服务订单号 |
| trade\_note | STRING | 订单收入-服务名称 | 交易备注 |
...
public class PopClientDemo {
 ...
        request.setBeginTime(1577863063);
        request.setEndTime(1578986263);
        request.setGroupType(1);
        request.setPage(1);
        request.setPageSize(20);
        request.setServiceOrderSn("200109-137080340852553");
        PddServicemarketTradelistGetResponse response = client.syncInvoke(request);
...
```
{
  "mall_balance_flow_search_response": {
    "data": [
      {
        "amount": 1000,
        "class_name": "交易收入",
        "created_at": 1578583502,
        "group_type": 1,
        "service_order_sn": "200109-137080340852553",
        "trade_note": "订单收入-服务名称"
      }
    ],
    "total_count": 1
  }

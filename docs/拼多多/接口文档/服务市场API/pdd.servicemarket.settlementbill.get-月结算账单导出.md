---
title: "pdd.servicemarket.settlementbill.get - 月结算账单导出"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.servicemarket.settlementbill.get"
menu_path:
  - "服务市场API"
  - "pdd.servicemarket.settlementbill.get"
captured_at: "2026-04-09T15:23:43.504Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| page | INTEGER | 必填 | 分页页码，最大不能超过1000 |
| page\_size | INTEGER | 必填 | 分页大小，最大不能超过100 |
| service\_order\_sn | STRING | 非必填 | 服务订单号 |
| settle\_month | STRING | 必填 | 结算月份 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| settlement\_bill\_search\_response | OBJECT |  | response |
| data | OBJECT\[\] |  | 结算明细列表 |
| service\_order\_sn | STRING | 20200109-000004180146624 | 服务订单号 |
| settle\_amount | LONG | 994 | 当期结算金额，单位分 |
| settle\_month | STRING | 2020-01 | 结算月份 |
| settle\_time | STRING | 2020-01 | 结算时间 |
| unsettle\_amount | LONG | 0 | 待结算金额，单位分 |
| service\_name | STRING | 测试模板 | 服务名称 |
| order\_type | INTEGER | 3 | 订单类型：0-应用服务、1-客服外包、2-电子发票、3-店铺装修、4-推广托管、5-代运营服务 |
| total\_count | INTEGER | 1 | 当前查询条件下所有订单总数 |
...
public class PopClientDemo {
 ...
        request.setServiceOrderSn("20200109-000004180146624");
        request.setSettleMonth("2020-01");
        PddServicemarketSettlementbillGetResponse response = client.syncInvoke(request);
...
```
{
  "settlement_bill_search_response": {
    "data": [
      {
        "order_type": 3,
        "service_name": "测试模板",
        "service_order_sn": "20200109-000004180146624",
        "settle_amount": 994,
        "settle_month": "2020-01",
        "settle_time": "2020-01",
        "unsettle_amount": 0
      }
    ],

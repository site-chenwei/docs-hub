---
title: "pdd.express.add.depot - 增加仓库"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.express.add.depot"
menu_path:
  - "仓储API"
  - "pdd.express.add.depot"
captured_at: "2026-04-09T15:21:50.321Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| contact\_name | STRING | 非必填 | 联系人姓名 |
| depot\_address | STRING | 必填 | 仓库详细地址5-20字 |
| depot\_alias | STRING | 必填 | 别名 |
| depot\_city\_id | INTEGER | 必填 | 所在市id |
| depot\_code | STRING | 必填 | 仓库编码 |
| depot\_district\_id | INTEGER | 必填 | 所在区id |
| depot\_name | STRING | 必填 | 仓库名称 |
| depot\_province\_id | INTEGER | 必填 | 所在省id |
| depot\_region | MAP | 必填 | 仓库区域（ 省->市->区id列表）例如：{"34":{"396":\[3383\]}}Map<Integer, Map<Integer, List<Integer>>> { 1:{ 2:\[3,4\] } } |
| key | STRING | 必填 | 省份id |
| value | MAP | 必填 | 市 -> 区id列表 |
| key | STRING | 必填 | 城市id |
| value | STRING\[\] | 必填 | 区id列表 |
| telephone | STRING | 非必填 | 联系人电话 |
| zip\_code | STRING | 非必填 | 邮编 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| open\_api\_response | STRING | success | 操作成功 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddExpressAddDepotRequest;
...
public class PopClientDemo {
 ...
        PddExpressAddDepotRequest request = new PddExpressAddDepotRequest();
        request.setContactName("联系人");
        request.setDepotAddress("北京市朝阳区");
        request.setDepotAlias("仓库");
        request.setDepotCityId(1);
        request.setDepotCode("1");
        request.setDepotDistrictId(1);
        request.setDepotName("仓库");
        request.setDepotProvinceId(1);
        Map<String, Map<String, List<String>>> depotRegion = new HashMap<String, Map<String, List<String>>>();
        Map<String, List<String>> value = new HashMap<String, List<String>>();
        List<String> value1 = new ArrayList<String>();
        value1.add("1");
        value.put("str", value1);
        depotRegion.put("str", value);
        request.setDepotRegion(depotRegion);
        request.setTelephone("13711111111");
        request.setZipCode("710000");
        PddExpressAddDepotResponse response = client.syncInvoke(request, accessToken);

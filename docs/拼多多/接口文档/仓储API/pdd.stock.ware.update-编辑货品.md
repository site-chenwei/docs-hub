---
title: "pdd.stock.ware.update - 编辑货品"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.stock.ware.update"
menu_path:
  - "仓储API"
  - "pdd.stock.ware.update"
captured_at: "2026-04-09T15:22:31.136Z"
---
...
| --- | --- | --- | --- |
| id | LONG | 必填 | 货品id |
| ware\_type | INTEGER | 必填 | 类型 0:单独货品。1:组合货品 |
| ware\_infos | OBJECT\[\] | 非必填 | 组合货品中子货品的关联关系, ware\_type为1时必填； |
| ware\_id | LONG | 必填 | 子货品id |
| ware\_quantity | INTEGER | 必填 | 子货品数量 |
| ware\_sn | STRING | 必填 | 货品编码 |
| ware\_name | STRING | 必填 | 货品名称 |
| note | STRING | 非必填 | 备注 |
| service\_quality | INTEGER | 非必填 | 高低值服务 |
| volume | INTEGER | 非必填 | 体积：立方毫米，只精确到100（即：最末两位为0） |
| length | INTEGER | 非必填 | 长：毫米，精确到1 |
| width | INTEGER | 非必填 | 宽：毫米，精确到1 |
| height | INTEGER | 非必填 | 高：毫米，精确到1 |
| weight | INTEGER | 必填 | 重量：g，精确到10（即：末位为0） |
| gross\_weight | INTEGER | 非必填 | 毛重：g，精确到10（即：末位为0） |
| net\_weight | INTEGER | 非必填 | 净重：g，精确到10（即：末位为0） |
| tare\_weight | INTEGER | 非必填 | 皮重：g，精确到10（即：末位为0） |
| price | INTEGER | 非必填 | 单价：分，精确到10（即：末位为0） |
| color | STRING | 非必填 | 颜色 |
| packing | STRING | 非必填 | 包材 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| open\_api\_response | LONG |  | 货品id |
...
import com.pdd.pop.sdk.http.api.pop.request.PddStockWareUpdateRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddStockWareUpdateResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddStockWareUpdateRequest.WareInfosItem;
...
public class PopClientDemo {
 ...
        PddStockWareUpdateRequest request = new PddStockWareUpdateRequest();
        request.setId(0L);
        request.setWareType(0);
        List<WareInfosItem> wareInfos = new ArrayList<WareInfosItem>();

        WareInfosItem item = new WareInfosItem();
        item.setWareId(0L);
        item.setWareQuantity(0);
        wareInfos.add(item);
        request.setWareInfos(wareInfos);
        request.setWareSn("str");
        request.setWareName("str");
        request.setNote("str");
        request.setServiceQuality(0);
        request.setVolume(0);
        request.setLength(0);
        request.setWidth(0);
        request.setHeight(0);
        request.setWeight(0);
        request.setGrossWeight(0);
        request.setNetWeight(0);
        request.setTareWeight(0);
        request.setPrice(0);
        request.setColor("str");
        request.setPacking("str");
        PddStockWareUpdateResponse response = client.syncInvoke(request, accessToken);

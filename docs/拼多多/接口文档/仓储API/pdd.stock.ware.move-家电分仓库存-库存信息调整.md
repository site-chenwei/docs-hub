---
title: "pdd.stock.ware.move - 家电分仓库存-库存信息调整"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.stock.ware.move"
menu_path:
  - "仓储API"
  - "pdd.stock.ware.move"
captured_at: "2026-04-09T15:22:24.984Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| stock\_move\_order\_action\_dto | OBJECT | 必填 |  |
| move\_direction | INTEGER | 必填 | 调整方向。1, "入库"；2, "出库"；3, "库存同步" |
| order\_note | STRING | 非必填 | 调整单备注 |
| business\_type | INTEGER | 必填 | 业务类型。1, "采购"；2, "调拨"；3, "退货"；4, "盘点"；5, "发货"；6, "库存同步" |
| warehouse\_sn | STRING | 必填 | 仓库编码 |
| move\_time | LONG | 必填 | 调整时间 |
| move\_order\_sn | STRING | 必填 | 调整单号 |
| stock\_move\_record\_action\_dto\_list | OBJECT\[\] | 必填 | List<JsonObject>的json string, 一次传入StockMoveRecordActionDTO list size不超过30个 |
| note | STRING | 非必填 | 备注 |
| move\_num | LONG | 必填 | 调整数量 |
| ware\_sn | STRING | 必填 | 货品sn |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| open\_api\_response | LONG |  | 调整单ID |
...
import com.pdd.pop.sdk.http.api.pop.request.PddStockWareMoveRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddStockWareMoveResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddStockWareMoveRequest.StockMoveOrderActionDto;
import com.pdd.pop.sdk.http.api.pop.request.PddStockWareMoveRequest.StockMoveRecordActionDtoListItem;
...
public class PopClientDemo {
 ...
        PddStockWareMoveRequest request = new PddStockWareMoveRequest();

        StockMoveOrderActionDto stockMoveOrderActionDto = new StockMoveOrderActionDto();
        stockMoveOrderActionDto.setMoveDirection(0);
        stockMoveOrderActionDto.setOrderNote("str");
        stockMoveOrderActionDto.setBusinessType(0);
        stockMoveOrderActionDto.setWarehouseSn("str");
        stockMoveOrderActionDto.setMoveTime(0L);
        stockMoveOrderActionDto.setMoveOrderSn("str");
        request.setStockMoveOrderActionDto(stockMoveOrderActionDto);
        List<StockMoveRecordActionDtoListItem> stockMoveRecordActionDtoList = new ArrayList<StockMoveRecordActionDtoListItem>();

        StockMoveRecordActionDtoListItem item = new StockMoveRecordActionDtoListItem();
        item.setNote("str");
        item.setMoveNum(0L);
        item.setWareSn("str");
        stockMoveRecordActionDtoList.add(item);
        request.setStockMoveRecordActionDtoList(stockMoveRecordActionDtoList);
        PddStockWareMoveResponse response = client.syncInvoke(request, accessToken);

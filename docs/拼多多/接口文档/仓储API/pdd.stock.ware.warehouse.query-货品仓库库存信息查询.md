---
title: "pdd.stock.ware.warehouse.query - 货品仓库库存信息查询"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.stock.ware.warehouse.query"
menu_path:
  - "仓储API"
  - "pdd.stock.ware.warehouse.query"
captured_at: "2026-04-09T15:22:34.315Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| request | OBJECT | 必填 | 请求对象 |
| page | INTEGER | 必填 | 当前页数 |
| page\_size | INTEGER | 必填 | 页显示数据条数 |
| ware\_sn\_list | STRING\[\] | 必填 | 货品编码列表 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| result | OBJECT | 查询结果 | 查询结果 |
| total | LONG | 1 | 总数 |
| ware\_sn\_warehouse\_info | MAP | 查询结果 | 查询结果 |
| $key | STRING | wareSn01 | 货品编码 |
| $value | OBJECT\[\] | 库存信息列表 | 库存信息列表 |
| quantity | LONG | 100 | 库存数量 |
| warehouse\_name | STRING | 上海仓库01 | 仓库名称 |
| warehouse\_sn | STRING | warehouse01 | 仓库编码 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddStockWareWarehouseQueryRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddStockWareWarehouseQueryResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddStockWareWarehouseQueryRequest.Request;
...
public class PopClientDemo {
 ...
        PddStockWareWarehouseQueryRequest request = new PddStockWareWarehouseQueryRequest();

        Request request1 = new Request();
        request1.setPage(1);
        request1.setPageSize(100);
        List<String> wareSnList = new ArrayList<String>();
        wareSnList.add("wareSn01");
        request1.setWareSnList(wareSnList);
        request.setRequest(request1);
        PddStockWareWarehouseQueryResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "result": {
    "total": 1,
    "ware_sn_warehouse_info": {
      "wareSn01": [
        {
          "quantity": 100,
          "warehouse_name": "上海仓库01",
          "warehouse_sn": "warehouse01"
        }
      ]
    }
  }

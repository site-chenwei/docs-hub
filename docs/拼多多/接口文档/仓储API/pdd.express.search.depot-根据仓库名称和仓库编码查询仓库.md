---
title: "pdd.express.search.depot - 根据仓库名称和仓库编码查询仓库"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.express.search.depot"
menu_path:
  - "仓储API"
  - "pdd.express.search.depot"
captured_at: "2026-04-09T15:22:02.769Z"
---
...
| --- | --- | --- | --- |
| code | STRING | 必填 | 仓库编码 |
| length | INTEGER | 必填 | 10 分页数据size |
| name | STRING | 必填 | 仓库名称 |
| start | INTEGER | 必填 | 0 分页数据起始位置 |
...
| province\_map | MAP |  | 覆盖信息，省：全覆盖 |
| key | STRING | 1 | 省份id |
| value | STRING | 1 | 1 半覆盖， 2 全覆盖 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddExpressSearchDepotRequest;
...
public class PopClientDemo {
 ...
        PddExpressSearchDepotRequest request = new PddExpressSearchDepotRequest();
        request.setCode("1");
        request.setLength(100);
        request.setName("仓库");
        request.setStart(0);
        PddExpressSearchDepotResponse response = client.syncInvoke(request, accessToken);

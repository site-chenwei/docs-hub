---
title: "pdd.stock.goods.id.to.sku.query - 根据商品id查询sku信息"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.stock.goods.id.to.sku.query"
menu_path:
  - "仓储API"
  - "pdd.stock.goods.id.to.sku.query"
captured_at: "2026-04-09T15:22:05.997Z"
---
...
| need\_offsale | BOOLEAN | 非必填 | 是否需要查询下架商品和sku，默认不需要 |
| ware\_id | LONG | 非必填 | 货品id |
...
| goods\_id | LONG |  | 商品id |
| is\_onsale | BOOLEAN |  | 上下架状态，true表示上架，false表示下架 |
| sku\_id | LONG |  | sku\_id |
| specs | OBJECT\[\] |  | 规格信息 |
| spec\_id | LONG |  | 规格id |
...
import com.pdd.pop.sdk.http.api.pop.request.PddStockGoodsIdToSkuQueryRequest;
...
public class PopClientDemo {
 ...
        PddStockGoodsIdToSkuQueryRequest request = new PddStockGoodsIdToSkuQueryRequest();
        request.setGoodsId(100L);
        request.setNeedOffsale(true);
        request.setWareId(200L);
        PddStockGoodsIdToSkuQueryResponse response = client.syncInvoke(request, accessToken);

---
title: "pdd.express.depot.list.get - 仓库列表"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.express.depot.list.get"
menu_path:
  - "仓储API"
  - "pdd.express.depot.list.get"
captured_at: "2026-04-09T15:22:19.670Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| length | LONG | 必填 | 分页数据size |
| start | LONG | 必填 | 分页数据起始位置 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| open\_api\_response | OBJECT |  | 返回结果 |
| count | INTEGER | 1 | 仓库数量 |
| depot\_list | OBJECT\[\] |  | 仓库列表 |
| address | STRING | 北京市朝阳区 | 仓库详细地址 |
| alias | STRING | 1 | 仓库别名 |
| city | INTEGER | 1 | 仓库所在市 |
| code | STRING | 1 | 仓库编码 |
| contact\_name | STRING | 联系人 | 联系人名字 |
| contact\_tel | STRING | 13711111111 | 联系人电话 |
| depot\_id | LONG | 1 | 仓库Id |
| district | INTEGER | 1 | 仓库所在区 |
| id\_str | STRING | 1 | 仓库Id2 |
| name | STRING | 仓库 | 仓库名称 |
| province | INTEGER | 1 | 仓库所在省 |
| province\_map | MAP |  | 覆盖信息,key是覆盖的省id;value中1表示局部，2表示全部覆盖 |
| key | STRING | 1 | 省份id |
| value | INTEGER | 1 | 1表示局部，2表示全部覆盖 |
| type | INTEGER | 1 | 仓库类型 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddExpressDepotListGetRequest;
...
public class PopClientDemo {
 ...
        PddExpressDepotListGetRequest request = new PddExpressDepotListGetRequest();
        request.setLength(100L);
        request.setStart(1L);
        PddExpressDepotListGetResponse response = client.syncInvoke(request, accessToken);

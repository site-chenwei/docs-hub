---
title: "pdd.cloud.security.event.tracking.login - 多多云ISV用户登录事件上报接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.cloud.security.event.tracking.login"
menu_path:
  - "pdd.cloud.security.event.tracking.login"
captured_at: "2026-04-15T03:32:26.735Z"
---
...
更新时间：2020-03-31 20:04:25

¥免费API

不需用户授权

多多云安全，用于ISV用户登录事件上报记录
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| loginMessage | STRING | 非必填 | 如果登录成功，可以填"success", 否则可以填写登录失败的原因 |
| loginResult | BOOLEAN | 必填 | 登录结果 |
| mallIdList | LONG\[\] | 必填 | 店铺id列表 |
| pati | STRING | 必填 | cookie中的\_pati值 |
| timestamp | LONG | 必填 | 事件时间（毫秒级别的时间戳） |
...
import com.pdd.pop.sdk.http.api.pop.request.PddCloudSecurityEventTrackingLoginRequest;
...
public class PopClientDemo {
 ...
        PddCloudSecurityEventTrackingLoginRequest request = new PddCloudSecurityEventTrackingLoginRequest();
        request.setLoginMessage("str");
        request.setLoginResult(false);
        List<Long> mallIdList = new ArrayList<Long>();
        mallIdList.add(1234L);
        request.setMallIdList(mallIdList);
        request.setPati("str");
        request.setTimestamp(0L);
        request.setUserId("str");
        request.setUserIp("str");
        PddCloudSecurityEventTrackingLoginResponse response = client.syncInvoke(request);

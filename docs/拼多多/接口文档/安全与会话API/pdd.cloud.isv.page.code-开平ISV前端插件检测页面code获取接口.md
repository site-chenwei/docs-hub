---
title: "pdd.cloud.isv.page.code - 开平ISV前端插件检测页面code获取接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.cloud.isv.page.code"
menu_path:
  - "pdd.cloud.isv.page.code"
captured_at: "2026-04-15T03:32:20.736Z"
---

# pdd.cloud.isv.page.code

## 开平ISV前端插件检测页面code获取接口

更新时间：2022-07-28 17:41:25

¥免费API

不需用户授权

获取植入ISV的前端检测插件的初始化code。 用于ISV前端插件检测 状态及接口管理信息
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| httpReferer | STRING | 必填 | 用户前端请求的HTTP referer, 需要满足标准URL格式. 如果为C/S架构客户端,填client, 如果为移动端,填mobile |
| mallIdList | STRING\[\] | 必填 | 用户关联的拼多多商家ID列表 |
| userId | STRING | 必填 | ISV权限体系里的商家用户ID |
...
| --- | --- | --- | --- |
| response | OBJECT | 无 |  |
| errorCode | INTEGER | 50001 | 错误码 |
| errorMessage | STRING | "server error" | 服务端错误 |
| pageCode | STRING | 123123111 | 插件token |

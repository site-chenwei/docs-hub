---
title: "pdd.finance.balance.daily.bill.url.get - 商家货款日账单下载链接查询接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.finance.balance.daily.bill.url.get"
menu_path:
  - "财务API"
  - "pdd.finance.balance.daily.bill.url.get"
captured_at: "2026-04-09T15:23:31.081Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| bill\_date | STRING | 必填 | 账单日期（形如yyyy-MM-dd）；例如入参为“2019-03-24”，则返回2019年3月24日的商家货款日账单的下载链接 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| finance\_balance\_daily\_bill\_url\_get\_response | OBJECT |  | response |
| download\_url | STRING | http:// | 货款账单下载 URL |
...
public class PopClientDemo {
 ...
        request.setBillDate("2019-03-24");
...
```
{
  "finance_balance_daily_bill_url_get_response": {
    "download_url": "http://"
  }
}
```
...
| --- | --- |
| 账单查询权限包 | 虚拟商家后台系统、商家后台系统、电子凭证商家后台系统 |
...
| 50001 | 业务服务错误 | pdd.finance.balance.daily.bill.exist.error | 货款日账单不存在 | 输入日期的账单文件不存在，可能是因为： 1、账单日期在参加日账单下载计划之前，可在MMS手动下载历史数据 2、账单日期尚未到来 3、日账单文件尚未生成，请在账单日期次日上午10点以后重试 |
| pdd.finance.balance.daily.bill.expire.error | 货款日账单已过期 | 输入日期为90天前 |

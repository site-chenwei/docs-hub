---
title: "pdd.live.video.mall.create - 多多视频商家发布视频"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.live.video.mall.create"
menu_path:
  - "自媒体API"
  - "pdd.live.video.mall.create"
captured_at: "2026-04-09T15:25:33.071Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| request | OBJECT | 必填 | 请求 |
| cover | STRING | 必填 | 视频封面，通过图片上传接口得到 |
| desc | STRING | 非必填 | 视频标题，可以带话题 |
| goods\_id | LONG | 必填 | 视频关联的商品，注意只能是当前商家的商品 |
| vid | STRING | 必填 | 通过视频上传接口得到，注意每次请求发布接口需要重新生成vid |
...
| result | OBJECT |  |  |
| video\_id | STRING |  | 发布成功得到的videoId |
...
import com.pdd.pop.sdk.http.api.pop.request.PddLiveVideoMallCreateRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddLiveVideoMallCreateResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddLiveVideoMallCreateRequest.Request;
...
public class PopClientDemo {
 ...
        PddLiveVideoMallCreateRequest request = new PddLiveVideoMallCreateRequest();

        Request request1 = new Request();
        request1.setCover("str");
        request1.setDesc("title#话题1 #话题2");
        request1.setGoodsId(0L);
        request1.setVid("str");
        request.setRequest(request1);
        PddLiveVideoMallCreateResponse response = client.syncInvoke(request, accessToken);

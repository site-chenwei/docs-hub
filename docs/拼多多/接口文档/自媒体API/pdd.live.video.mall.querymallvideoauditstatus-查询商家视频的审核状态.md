---
title: "pdd.live.video.mall.querymallvideoauditstatus - 查询商家视频的审核状态"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.live.video.mall.querymallvideoauditstatus"
menu_path:
  - "自媒体API"
  - "pdd.live.video.mall.querymallvideoauditstatus"
captured_at: "2026-04-09T15:25:36.151Z"
---
...
| error\_msg | STRING |  | 错误原因 |
| result | OBJECT |  |  |
| auditStatus | INTEGER |  | 0:审核中 1:审核通过 2:审核驳回 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddLiveVideoMallQuerymallvideoauditstatusRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddLiveVideoMallQuerymallvideoauditstatusResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddLiveVideoMallQuerymallvideoauditstatusRequest.Request;
...
public class PopClientDemo {
 ...
        PddLiveVideoMallQuerymallvideoauditstatusRequest request = new PddLiveVideoMallQuerymallvideoauditstatusRequest();

        Request request1 = new Request();
        request1.setFeedId("str");
        request.setRequest(request1);
        PddLiveVideoMallQuerymallvideoauditstatusResponse response = client.syncInvoke(request, accessToken);
...
| 50001 | 业务服务错误 | 400000001 | 入参错误 | 检查入参是否有效、是否缺少必填项 |
| 400000002 | 系统错误 | 系统错误，稍后重试 |
| 400000009 | 视频封面地址错误 | 检查封面地址是否有效 |
| 400000010 | 账号异常，暂时不支持发布视频 | 账号异常，暂时不支持发布视频 |
| 400000012 | 视频关联的商品错误 | 检查商品是否在售且是当前商家的商品 |
| 400000014 | 超出当日发布数上限 | 合理创建内容 |
| 400000015 | 同一vid重复发布视频 | 重新请求视频上传接口新生成vid |
| 400000016 | 无效vid | 检查vid是否有效 |
| 400000017 | 视频标题超过长度限制 | 精简标题 |
| 400000018 | 非开放平台上传的带货视频 | 确认数据来源 |
| 400000019 | 非商家自己的feed | 确认数据来源 |

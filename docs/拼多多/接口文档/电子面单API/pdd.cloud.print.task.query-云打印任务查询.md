---
title: "pdd.cloud.print.task.query - 云打印任务查询"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.cloud.print.task.query"
menu_path:
  - "电子面单API"
  - "pdd.cloud.print.task.query"
captured_at: "2026-04-09T15:22:53.600Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| cloud\_print\_task\_query | OBJECT | 必填 | 云打印任务查询请求 |
| print\_sequence\_list | INTEGER\[\] | 非必填 | 打印序号，非必填，填了则只查询列表内的任务 |
| print\_task\_id | STRING | 必填 | 打印任务id |
...
| error\_msg | STRING |  |  |
| result | OBJECT |  |  |
| cloud\_print\_task\_result\_list | OBJECT\[\] |  | 云打印任务结果列表 |
| fail\_reason | STRING |  | 失败原因 |
| mall\_id | LONG |  | 请求云打印时token对应的mallId |
| print\_sequence | INTEGER |  | 打印序号 |
| print\_status | INTEGER |  | 打印状态 0:打印中 1:打印成功 2:打印失败 3:打印超时 |
| print\_task\_id | STRING |  | 打印任务id |
| pushed\_to\_printer | BOOLEAN |  | 是否推送给云打印机，只有打印失败的任务才有值，如果为true，需要重点关注是否打印完成 |
| ship\_code | STRING |  | 快递公司编码 |
| waybill\_code | STRING |  | 运单号 |
| success | BOOLEAN |  |  |
...
import com.pdd.pop.sdk.http.api.pop.request.PddCloudPrintTaskQueryRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddCloudPrintTaskQueryResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddCloudPrintTaskQueryRequest.CloudPrintTaskQuery;
...
public class PopClientDemo {
 ...
        PddCloudPrintTaskQueryRequest request = new PddCloudPrintTaskQueryRequest();

        CloudPrintTaskQuery cloudPrintTaskQuery = new CloudPrintTaskQuery();
        List<Integer> printSequenceList = new ArrayList<Integer>();
        printSequenceList.add(0);
        cloudPrintTaskQuery.setPrintSequenceList(printSequenceList);
        cloudPrintTaskQuery.setPrintTaskId("str");
        cloudPrintTaskQuery.setPrinterId("str");
        cloudPrintTaskQuery.setShareCode("str");
        request.setCloudPrintTaskQuery(cloudPrintTaskQuery);
        PddCloudPrintTaskQueryResponse response = client.syncInvoke(request, accessToken);

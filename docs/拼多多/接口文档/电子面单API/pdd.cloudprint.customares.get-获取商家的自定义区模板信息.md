---
title: "pdd.cloudprint.customares.get - 获取商家的自定义区模板信息"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.cloudprint.customares.get"
menu_path:
  - "电子面单API"
  - "pdd.cloudprint.customares.get"
captured_at: "2026-04-09T15:23:09.220Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| template\_id | INTEGER | 必填 | 即pdd.cloudprint.stdtemplates.get接口返回的standard\_template\_id |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| pdd\_cloudprint\_customares\_get\_response | OBJECT | 1 | response |
| result | OBJECT |  | result |
| datas | OBJECT\[\] |  | datas |
| custom\_area\_id | LONG |  | custom\_area\_id |
| custom\_area\_name | STRING |  | custom\_area\_name |
| custom\_area\_url | STRING |  | custom\_area\_url |
| keys | OBJECT\[\] |  | keys |
...
import com.pdd.pop.sdk.http.api.pop.request.PddCloudprintCustomaresGetRequest;
...
public class PopClientDemo {
 ...
        PddCloudprintCustomaresGetResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "pdd_cloudprint_customares_get_response": {
    "result": {
      "datas": [
        {
          "custom_area_id": 0,
          "custom_area_name": "str",
          "custom_area_url": "str",
          "keys": [
            {}
          ]

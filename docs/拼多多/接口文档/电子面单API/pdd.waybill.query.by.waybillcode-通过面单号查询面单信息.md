---
title: "pdd.waybill.query.by.waybillcode - 通过面单号查询面单信息"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.waybill.query.by.waybillcode"
menu_path:
  - "电子面单API"
  - "pdd.waybill.query.by.waybillcode"
captured_at: "2026-04-09T15:23:21.744Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| param\_list | OBJECT\[\] | 必填 | 系统自动生成 |
| object\_id | STRING | 必填 | 请求id |
| waybill\_code | STRING | 必填 | 电子面单号 |
| wp\_code | STRING | 必填 | 快递公司code |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| pdd\_waybill\_query\_by\_waybillcode\_response | OBJECT |  | response |
| modules | OBJECT\[\] |  | 查询返回值 |
| waybill\_cloud\_print\_response | OBJECT |  | 面单查询结构体 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddWaybillQueryByWaybillcodeRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddWaybillQueryByWaybillcodeResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddWaybillQueryByWaybillcodeRequest.ParamListItem;
...
public class PopClientDemo {
 ...
        item.setWaybillCode("str");
        item.setWpCode("str");
        paramList.add(item);
        request.setParamList(paramList);
        PddWaybillQueryByWaybillcodeResponse response = client.syncInvoke(request, accessToken);
...
| 50001 | 业务服务错误 | isv.invalid-parameter | 非法参数 | 非法参数 |
| isv.service-error | 系统异常 | 系统异常 |
| isv.waybill-not-exist | 该面单号不存在 | 该面单号不存在 |
| server.busy | 系统繁忙，请稍后再试 | 系统繁忙，请稍后再试 |

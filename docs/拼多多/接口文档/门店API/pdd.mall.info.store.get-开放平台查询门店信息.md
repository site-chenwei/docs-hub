---
title: "pdd.mall.info.store.get - 开放平台查询门店信息"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.mall.info.store.get"
menu_path:
  - "门店API"
  - "pdd.mall.info.store.get"
captured_at: "2026-04-09T15:24:33.635Z"
---
...
| --- | --- | --- | --- |
| city | STRING | 非必填 | 市 |
| district | STRING | 非必填 | 区 |
| page\_number | INTEGER | 必填 | 分页 |
| page\_size | INTEGER | 必填 | 分页大小 |
| province | STRING | 非必填 | 省 |
| store\_id | LONG | 非必填 | 门店Id |
| store\_name | STRING | 非必填 | 门店名称 |
...
| --- | --- | --- | --- |
| res | OBJECT |  | 返回结构体 |
| mallStoreVOList | OBJECT\[\] |  | 返回结构体 |
| audit\_status | INTEGER |  | 审核状态 |
| city | STRING |  | 市 |
| district | STRING |  | 区 |
| mall\_id | LONG |  | 店铺id |
| poi\_latitude | DOUBLE |  | 门店纬度 |
| poi\_longitude | DOUBLE |  | 门店经度 |
| province | STRING |  | 省 |
| store\_address | STRING |  | 门店详细地址 |
| store\_id | LONG |  | 门店id |
| store\_name | STRING |  | 门店名称 |
| store\_number | STRING |  | 门店自有编号 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddMallInfoStoreGetRequest;
...
public class PopClientDemo {
 ...
        PddMallInfoStoreGetRequest request = new PddMallInfoStoreGetRequest();
        request.setCity("杭州市");
        request.setDistrict("江干区");
        request.setPageNumber(1);
        request.setPageSize(50);
        request.setProvince("浙江省");
        request.setStoreId(574625L);
        request.setStoreName("长宁途虎门店");
        request.setStoreNumber("654门店");
        PddMallInfoStoreGetResponse response = client.syncInvoke(request, accessToken);

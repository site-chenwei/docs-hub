---
title: "pdd.mall.info.group.remove.store.get - 门店组删除门店"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.mall.info.group.remove.store.get"
menu_path:
  - "门店API"
  - "pdd.mall.info.group.remove.store.get"
captured_at: "2026-04-09T15:24:27.301Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| group\_id | LONG | 必填 | 店铺ID |
| store\_id\_list | LONG\[\] | 必填 | 门店ID列表 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| mall\_info\_group\_remove\_store\_get\_response | OBJECT |  |  |
| is\_success | BOOLEAN |  |  |
...
import com.pdd.pop.sdk.http.api.pop.request.PddMallInfoGroupRemoveStoreGetRequest;
...
public class PopClientDemo {
 ...
        PddMallInfoGroupRemoveStoreGetRequest request = new PddMallInfoGroupRemoveStoreGetRequest();
        request.setGroupId(0L);
        List<Long> storeIdList = new ArrayList<Long>();
        storeIdList.add(0L);
        request.setStoreIdList(storeIdList);
        PddMallInfoGroupRemoveStoreGetResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "mall_info_group_remove_store_get_response": {
    "is_success": false
  }
}
```

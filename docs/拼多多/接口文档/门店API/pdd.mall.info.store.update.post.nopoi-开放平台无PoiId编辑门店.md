---
title: "pdd.mall.info.store.update.post.nopoi - 开放平台无PoiId编辑门店"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.mall.info.store.update.post.nopoi"
menu_path:
  - "门店API"
  - "pdd.mall.info.store.update.post.nopoi"
captured_at: "2026-04-09T15:24:36.727Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| business\_status | INTEGER | 必填 | 门店营业状态 |
| business\_week\_list | INTEGER\[\] | 必填 | 一周营业时间 |
| city | STRING | 必填 | 市 |
| district | STRING | 必填 | 区 |
| end\_business\_hour | STRING | 必填 | 结束营业时间 |
| poi\_latitude | DOUBLE | 必填 | 门店纬度 |
| poi\_longitude | DOUBLE | 必填 | 门店经度 |
| province | STRING | 必填 | 省 |
| start\_business\_hour | STRING | 必填 | 起始营业时间 |
| store\_address | STRING | 必填 | 详细地址 |
| store\_id | LONG | 必填 | 门店id |
| store\_name | STRING | 必填 | 门店名称 |
| store\_number | STRING | 非必填 | 门店自有编号 |
| store\_phone | STRING | 必填 | 门店电话 |
| trade\_type | INTEGER | 必填 | 门店行业（1-男女装，2-运动户外，3-服饰配件，4-厨具电器，5-汽车，8-全屋定制） |
...
import com.pdd.pop.sdk.http.api.pop.request.PddMallInfoStoreUpdatePostNopoiRequest;
...
public class PopClientDemo {
 ...
        PddMallInfoStoreUpdatePostNopoiRequest request = new PddMallInfoStoreUpdatePostNopoiRequest();
 ...
        request.setStoreId(5412369L);
        request.setStoreName("长宁门店");
        request.setStoreNumber("654门店");
        request.setStorePhone("36547896");
        request.setTradeType(1);
        PddMallInfoStoreUpdatePostNopoiResponse response = client.syncInvoke(request, accessToken);

---
title: "pdd.qrpay.payee.register - 交易二维码-参数注册接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.qrpay.payee.register"
menu_path:
  - "门店API"
  - "pdd.qrpay.payee.register"
captured_at: "2026-04-09T15:24:59.953Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| payee\_list | OBJECT\[\] | 必填 | 参数列表 |
| payee | STRING | 必填 | 参数名，用于注册到名单，并生成对应URL |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| qrpay\_payee\_register\_response | OBJECT |  | response |
| payee\_result\_list | OBJECT\[\] |  | 参数注册结果 |
| payee | STRING |  | 参数名 |
| payee\_id | LONG |  | 参数id |
| qr\_image\_url | STRING |  | 交易二维码图片UR |
| qr\_pay\_page\_url | STRING |  | 交易二维码页面URL |
...
import com.pdd.pop.sdk.http.api.pop.request.PddQrpayPayeeRegisterRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddQrpayPayeeRegisterResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddQrpayPayeeRegisterRequest.PayeeListItem;
...
public class PopClientDemo {
 ...
        PddQrpayPayeeRegisterRequest request = new PddQrpayPayeeRegisterRequest();
        List<PayeeListItem> payeeList = new ArrayList<PayeeListItem>();

        PayeeListItem item = new PayeeListItem();
        item.setPayee("str");
        payeeList.add(item);
        request.setPayeeList(payeeList);
        PddQrpayPayeeRegisterResponse response = client.syncInvoke(request, accessToken);
...
| 50001 | 业务服务错误 | pdd.qrpay.payee.register.system.error | 系统错误 | 系统错误 |

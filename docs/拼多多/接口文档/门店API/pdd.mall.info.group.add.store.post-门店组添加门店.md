---
title: "pdd.mall.info.group.add.store.post - 门店组添加门店"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.mall.info.group.add.store.post"
menu_path:
  - "门店API"
  - "pdd.mall.info.group.add.store.post"
captured_at: "2026-04-09T15:24:17.776Z"
---
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| mall\_info\_group\_add\_store\_post\_response | OBJECT |  |  |
| is\_success | BOOLEAN |  |  |
...
import com.pdd.pop.sdk.http.api.pop.request.PddMallInfoGroupAddStorePostRequest;
...
public class PopClientDemo {
 ...
        PddMallInfoGroupAddStorePostRequest request = new PddMallInfoGroupAddStorePostRequest();
        request.setGroupId(0L);
        List<Long> storeIdList = new ArrayList<Long>();
        storeIdList.add(0L);
        request.setStoreIdList(storeIdList);
        PddMallInfoGroupAddStorePostResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "mall_info_group_add_store_post_response": {
    "is_success": false
  }
}
```

---
title: "pdd.logistics.online.send - 订单发货通知接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.logistics.online.send"
menu_path:
  - "物流API"
  - "pdd.logistics.online.send"
captured_at: "2026-04-09T15:14:46.757Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| feature | STRING | 非必填 | 发货个性内容. |
| logistics\_id | LONG | 必填 | 快递公司编号 |
| order\_sn | STRING | 必填 | 订单号。形如：20150909-452750051 |
| redelivery\_type | INTEGER | 非必填 | 修改发货模式：不传则默认为首次发货 1=首次发货：用于订单首次发货，仅待发货订单可传入； 2=修改发货：用于订单修改发货，调用成功后将会覆盖原发货信息，仅已发货订单可传入 |
| refund\_address\_id | STRING | 非必填 | 退货地址的id，不填则取商品默认退货地址（可在“拼多多-商家后台/售后工作台/售后设置”为商品绑定默认退货地址，若未设置则取店铺默认退货地址） |
| sequence\_id | INTEGER | 非必填 | 发货批次：普通订单无需传入，当前仅限 分批发货订单(考研图书等场景) 传入 |
| ship\_info\_list | OBJECT\[\] | 非必填 | 发货凭证信息列表。以发货物流底单举例，形如：\[{ "type":1, "url":"XXXXXX" }\]，其中url需要通过【发货底单图片上传接口】上传对应图片获取。 |
| type | INTEGER | 非必填 | 发货凭证类型：1=发货物流底单 |
| url | STRING | 非必填 | 发货凭证链接，需要通过【发货底单图片上传接口】上传对应图片获取。 |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| logistics\_online\_send\_response | OBJECT |  | 发货通知响应对象 |
| is\_success | BOOLEAN |  | 是否成功，false-失败，true-成功 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddLogisticsOnlineSendRequest.ShipInfoListItem;

---
title: "pdd.logistics.isv.trace.notify.sub - ISV物流轨迹推送消息订阅接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.logistics.isv.trace.notify.sub"
menu_path:
  - "物流API"
  - "pdd.logistics.isv.trace.notify.sub"
captured_at: "2026-04-09T15:14:43.690Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| ship\_code | STRING | 必填 | 快递公司编码 |
| tel | STRING | 必填 | 收件人手机 |
| track\_no | STRING | 必填 | 快递单号 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| logistics\_isv\_trace\_notify\_sub | OBJECT |  | 订阅响应对象 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddLogisticsIsvTraceNotifySubRequest;
...
public class PopClientDemo {
 ...
        PddLogisticsIsvTraceNotifySubRequest request = new PddLogisticsIsvTraceNotifySubRequest();
        request.setShipCode("ZTO");
        request.setTel("13023245678");
        request.setTrackNo("str");
        PddLogisticsIsvTraceNotifySubResponse response = client.syncInvoke(request);
...
```
{
  "logistics_isv_trace_notify_sub": {
    "is_success": false
  }
}
```
...
| --- | --- |
| 物流轨迹订阅权限包 | 打单、进销存、电子面单、企业ERP、订单处理、跨境企业ERP报关版 |
...
| 50001 | 业务服务错误 | 1000 | 非法业务参数 | 确认手机号格式是否正确 |
| 1001 | 快递公司编码有误 | 确认快递公司编码是否有误 |
| 1200 | 快递公司轨迹不支持订阅 | 快递公司轨迹不支持订阅 |
| 1203 | 订阅失败 | 重新订阅 |
| 9990 | 业务系统异常 | 稍后重新订阅 |

### 限流规则

应用限流频次：4000000次/86400秒

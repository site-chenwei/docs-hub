---
title: "pdd.oversea.clearance.get - 获取多多国际清关材料"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.oversea.clearance.get"
menu_path:
  - "多多国际API"
  - "pdd.oversea.clearance.get"
captured_at: "2026-04-09T15:24:49.229Z"
---
...
| need\_image | BOOLEAN | 非必填 | 是否需要身份证图片（目前仅限清关服务商应用获取CC行邮订单的身份证照片） |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| clearance\_response | OBJECT |  |  |
| id\_card\_name
加密
...
加密
 ...
 | STRING |  | 支付单号 |
| pay\_type | STRING |  | 支付方式，枚举值：WEIXIN,ALIPAY,DUODUOPAY |
| id\_card\_back\_url

加密

 | STRING |  | 身份证反面 |
| id\_card\_front\_url
...
import com.pdd.pop.sdk.http.api.pop.request.PddOverseaClearanceGetRequest;
...
public class PopClientDemo {
 ...
        PddOverseaClearanceGetRequest request = new PddOverseaClearanceGetRequest();
        request.setOrderSn("2020-123456");
        request.setNeedImage(false);
        PddOverseaClearanceGetResponse response = client.syncInvoke(request, accessToken);

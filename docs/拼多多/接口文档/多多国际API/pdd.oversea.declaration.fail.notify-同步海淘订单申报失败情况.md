---
title: "pdd.oversea.declaration.fail.notify - 同步海淘订单申报失败情况"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.oversea.declaration.fail.notify"
menu_path:
  - "多多国际API"
  - "pdd.oversea.declaration.fail.notify"
captured_at: "2026-04-09T15:24:52.357Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| fail\_reason | INTEGER | 必填 | 1-超过购买额度，4-用户未核验导致清关失败，2-其他清关异常（如重量超标、退运、扣留等） |
...
| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| notify\_exceeded\_response | OBJECT |  |  |
| result | BOOLEAN | 是否处理成功 | true=处理成功 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddOverseaDeclarationFailNotifyRequest;
...
public class PopClientDemo {
 ...
        PddOverseaDeclarationFailNotifyRequest request = new PddOverseaDeclarationFailNotifyRequest();
        request.setFailReason(1);
        request.setOrderSn("201111-1234567");
        PddOverseaDeclarationFailNotifyResponse response = client.syncInvoke(request, accessToken);

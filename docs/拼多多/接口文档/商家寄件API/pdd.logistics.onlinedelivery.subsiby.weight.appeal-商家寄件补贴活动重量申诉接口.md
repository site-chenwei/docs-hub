---
title: "pdd.logistics.onlinedelivery.subsiby.weight.appeal - 商家寄件补贴活动重量申诉接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.logistics.onlinedelivery.subsiby.weight.appeal"
menu_path:
  - "商家寄件API"
  - "pdd.logistics.onlinedelivery.subsiby.weight.appeal"
captured_at: "2026-04-09T15:26:32.478Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| acceptPddWeight | BOOLEAN | 必填 | 是否接受pdd侧推荐重量，不进行申诉 |
| appealImgUrlList | STRING\[\] | 非必填 | 图片URL列表。上传包裹称重图片，图片中需要有清晰的包裹运单号，以及承重数据号，支持上传2-5张图片（acceptPddWeight=false时必传） |
| appealReason | STRING | 非必填 | 快递公司描述举证原因，最多200字（acceptPddWeight=false时必传） |
| appealWeight | INTEGER | 非必填 | 快递公司认为合理的重量，单位：g（acceptPddWeight=false时必传） |
| billDate | STRING | 必填 | 账期 |
| bizBillType | INTEGER | 必填 | 账单类型。方便后续扩展 1：拼多多商家寄件补贴活动 |
| deliveryReceiptSn | STRING | 必填 | 寄件单号 |
| shipCode | STRING | 必填 | 快递公司编码 |
| waybillCode | STRING | 必填 | 运单号 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| response | OBJECT |  | 返回结果实体 |
| errorCode | INTEGER | 1234 | 请求失败错误码 |
| errorMsg | STRING | \*\*\*\*\* | 请求失败错误详情 |
| result | OBJECT |  | 申诉结果实体 |
| errorMsg | STRING | 长河落日圆 | 申诉请求受理失败的原因 |
| result | BOOLEAN | true | 申诉请求受理结果 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddLogisticsOnlinedeliverySubsibyWeightAppealRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddLogisticsOnlinedeliverySubsibyWeightAppealResponse;
...
public class PopClientDemo {
 ...
        PddLogisticsOnlinedeliverySubsibyWeightAppealRequest request = new PddLogisticsOnlinedeliverySubsibyWeightAppealRequest();
        request.setAcceptPddWeight(true);
        List<String> appealImgUrlList = new ArrayList<String>();
        appealImgUrlList.add("https://pics0.baidu.com/feed/279759ee3d6d55fbcfd4565bf4b1455a21a4dde4.jpeg?token=6a13dc21688d2025fb5bb80f32298c68");
        request.setAppealImgUrlList(appealImgUrlList);
        request.setAppealReason("大漠孤烟直");
 ...
        PddLogisticsOnlinedeliverySubsibyWeightAppealResponse response = client.syncInvoke(request);

---
title: "pdd.logistics.onlinedelivery.receipt.detail - 寄件单详情接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.logistics.onlinedelivery.receipt.detail"
menu_path:
  - "商家寄件API"
  - "pdd.logistics.onlinedelivery.receipt.detail"
captured_at: "2026-04-09T15:26:16.781Z"
---
...
查询寄件单状态，详情信息，包括寄件地址、包裹信息、包括支付状态以及支付失败原因等
...
| deliveryReceiptSn | STRING | 必填 | 寄件单号 |
...
| courierMobile | STRING | \*\*\* | 快递员电话（脱敏） |
| courierName | STRING | 小\* | 快递员姓名（脱敏） |
| shipCode | STRING | 极兔速递 | 快递公司名称 |
| shipName | STRING | JTSD | 快递公司code |
| deliveryReceiptSn | STRING | OD123456 | 寄件单号 |
| orderSn | STRING | 20251011-123456789 | 订单号 |
| packageHintCode | STRING | 666 | 包裹码 |
| payFailReason | STRING |  | 支付失败原因 |
| payStatus | INTEGER | 0 | 支付状态 |
| payStatusDesc | STRING | 未支付 | 支付状态描述 |
| predictPrice | LONG | 100 | 预估价格（单位：分） |
| predictWeight | INTEGER | 10 | 预估重量（单位：克） |
| printData | STRING | 123adas9k2ddf | 打印信息 |
| realPrice | LONG | 100 | 实际价格（单位：分） |
| realWeight | INTEGER | 10 | 实际重量（单位：克） |
| receiver | OBJECT |  | receiver |
| addressDetail | STRING | 天山路街道555号 | 详细地址（脱敏） |
| cityCode | STRING | 2507 | 市编码 |
| cityName | STRING | 上海市 | 市名称 |
| contactMobile | STRING | \*\*\* | 电话（脱敏） |
| contactName | STRING | 小明\* | 姓名（脱敏） |
| districtCode | STRING | 2507 | 区编码 |
| districtName | STRING | 长宁区 | 区名称 |
| provinceCode | STRING | 25 | 省编码 |
| provinceName | STRING | 上海市 | 省名称 |
| townCode | STRING | 250701 | 镇编码 |
| townName | STRING |  | 镇名称 |
| sender | OBJECT |  | sender |
| addressDetail | STRING | \*\*\* | 详细地址（脱敏） |
| cityCode | STRING | 2501 | 市编码 |
| cityName | STRING | 上海市 | 市名称 |
| contactMobile | STRING | \*\*\* | 电话（脱敏） |
| contactName | STRING | 小\* | 姓名（脱敏） |
| districtCode | STRING | 2507 | 区id |
| districtName | STRING | 长宁区 | 区名称 |
| provinceCode | STRING | 25 | 省编码 |
| provinceName | STRING | 上海市 | 省名称 |
| townCode | STRING | 251101 | 镇编码 |
| townName | STRING |  | 镇名称 |
| status | INTEGER | 0 | 运单状态枚举值（0待取件，100待揽收、200配送中、300已完成、400已取消） |
| statusDesc | STRING |  | statusDesc |
| waybillCode | STRING | 123456 | 运单号 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddLogisticsOnlinedeliveryReceiptDetailRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddLogisticsOnlinedeliveryReceiptDetailResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddLogisticsOnlinedeliveryReceiptDetailRequest.Request;
...
public class PopClientDemo {
 ...
        PddLogisticsOnlinedeliveryReceiptDetailRequest request = new PddLogisticsOnlinedeliveryReceiptDetailRequest();

        Request request1 = new Request();
        request1.setDeliveryReceiptSn("OD123456");
        request.setRequest(request1);
        PddLogisticsOnlinedeliveryReceiptDetailResponse response = client.syncInvoke(request, accessToken);
...
```
{
 ...
      "predictPrice": 100,
      "predictWeight": 10,
      "printData": "123adas9k2ddf",
      "realPrice": 100,
      "realWeight": 10,
      "receiver": {
        "addressDetail": "天山路街道555号",
        "cityCode": "2507",
        "cityName": "上海市",
        "contactMobile": "***",
        "contactName": "小明*",

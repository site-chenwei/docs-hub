---
title: "pdd.reserved.medicine.logistics.upload - 预留药物流信息上传"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.reserved.medicine.logistics.upload"
menu_path:
  - "卡券API"
  - "pdd.reserved.medicine.logistics.upload"
captured_at: "2026-04-09T15:20:47.906Z"
---
...
| logistics\_company\_id | LONG | 必填 | 物流公司id,参考接口https://open.pinduoduo.com/application/document/api?id=pdd.logistics.companies.get 取对应物流公司id字段 |
| logistics\_sn | STRING | 必填 | 物流单号 |
| order\_sn | STRING | 必填 | pdd订单号 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddReservedMedicineLogisticsUploadRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddReservedMedicineLogisticsUploadResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddReservedMedicineLogisticsUploadRequest.Request;
...
public class PopClientDemo {
 ...
        PddReservedMedicineLogisticsUploadRequest request = new PddReservedMedicineLogisticsUploadRequest();

        Request request1 = new Request();
        request1.setLogisticsCompanyId(0L);
        request1.setLogisticsSn("str");
        request1.setOrderSn("str");
        request.setRequest(request1);
        PddReservedMedicineLogisticsUploadResponse response = client.syncInvoke(request, accessToken);
...
| 50001 | 业务服务错误 | 600001 | 物流信息错误 | 物流单号或者物流公司id错误 |
| 600002 | 订单重复上传 | 订单信息已上传 |
| 600003 | 订单不存在 | 当前店铺没有对应的订单 |
| 600004 | 权限不足 需要pdd开通白名单 | 权限不足 需要pdd开通白名单 |

---
title: "pdd.voucher.virtual.card.verification - 卡券（电子）核销接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.voucher.virtual.card.verification"
menu_path:
  - "卡券API"
  - "pdd.voucher.virtual.card.verification"
captured_at: "2026-04-09T15:21:09.904Z"
---
...
| voucher\_data\_list | OBJECT\[\] | 必填 | 券信息列表 |
| out\_trans\_no | STRING | 必填 | 流水号 |
| voucher\_time | LONG | 必填 | 券状态更改时间 |
| voucher\_status | INTEGER | 必填 | 券状态 1：已核销；2：已销毁 |
| voucher\_no | STRING | 必填 | 券号 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| voucher\_voucher\_info\_verify\_response | OBJECT |  | response |
| order\_sn | STRING |  | 拼多多订单号 |
| goods\_id | LONG |  | 商品id |
| sku\_id | LONG |  | 商品属性id |
| mall\_id | LONG |  | 店铺id |
| voucher\_list | OBJECT\[\] |  | 券信息数组 |
| card\_no | STRING |  | 卡号 |
| card\_code | STRING |  | 卡密 |
| mark\_password | STRING |  | 标准密码 |
| status | INTEGER |  | 状态 |
| refund\_status | INTEGER |  | 返回状态 |
| verification\_at | LONG |  | 验证处 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddVoucherVirtualCardVerificationRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddVoucherVirtualCardVerificationResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddVoucherVirtualCardVerificationRequest.VoucherDataListItem;
...
public class PopClientDemo {
 ...
        PddVoucherVirtualCardVerificationRequest request = new PddVoucherVirtualCardVerificationRequest();
        request.setOrderSn("str");
        List<VoucherDataListItem> voucherDataList = new ArrayList<VoucherDataListItem>();

        VoucherDataListItem item = new VoucherDataListItem();
        item.setOutTransNo("str");
        item.setVoucherTime(0L);
        item.setVoucherStatus(0);
        item.setVoucherNo("str");
        voucherDataList.add(item);
        request.setVoucherDataList(voucherDataList);
        PddVoucherVirtualCardVerificationResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "voucher_voucher_info_verify_response": {
    "goods_id": 0,
    "mall_id": 0,
    "order_sn": "str",
    "sku_id": 0,

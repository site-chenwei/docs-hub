---
title: "pdd.voucher.virtual.card.batch.add - 批量添加卡券"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.voucher.virtual.card.batch.add"
menu_path:
  - "卡券API"
  - "pdd.voucher.virtual.card.batch.add"
captured_at: "2026-04-09T15:21:06.741Z"
---
...
| 参数接口 | 参数类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| data | OBJECT | 非必填 | 业务数据 |
| chargeAddress | STRING | 非必填 | 充值地址 |
| dataList | OBJECT\[\] | 必填 | 卡密信息列表，一次请求最多5000条卡密 |
| cardNo | STRING | 非必填 | 卡密卡号，商家卡密必填 |
| encryptPassword | STRING | 必填 | 用户核销卡密加密串，加密所使用public key向对接小二获取（加密算法"RSA"，填充方式"RSA/ECB/PKCS1Padding"） |
| goodsId | LONG | 必填 | 商品Id |
| skuId | LONG | 必填 | skuId |
...
| result | OBJECT |  | 响应信息 |
| batchId | LONG |  | 卡密批次Id |
| chargeAddress | STRING |  | 充值地址 |
| mallId | LONG |  | 店铺Id |
| totalNum | INTEGER |  | 批次添加的卡密数量 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddVoucherVirtualCardBatchAddRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddVoucherVirtualCardBatchAddResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddVoucherVirtualCardBatchAddRequest.Data;
import com.pdd.pop.sdk.http.api.pop.request.PddVoucherVirtualCardBatchAddRequest.DataDataListItem;
...
public class PopClientDemo {
 ...
        PddVoucherVirtualCardBatchAddRequest request = new PddVoucherVirtualCardBatchAddRequest();

        Data data = new Data();
        data.setChargeAddress("str");
        List<DataDataListItem> dataList = new ArrayList<DataDataListItem>();

        DataDataListItem item = new DataDataListItem();
        item.setCardNo("str");
        item.setEncryptPassword("str");
        dataList.add(item);
        data.setDataList(dataList);
        data.setGoodsId(0L);
        data.setSkuId(0L);
        request.setData(data);
        PddVoucherVirtualCardBatchAddResponse response = client.syncInvoke(request, accessToken);

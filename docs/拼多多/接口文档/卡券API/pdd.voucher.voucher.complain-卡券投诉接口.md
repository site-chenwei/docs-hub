---
title: "pdd.voucher.voucher.complain - 卡券投诉接口"
source_url: "https://open.pinduoduo.com/application/document/api?id=pdd.voucher.voucher.complain"
menu_path:
  - "卡券API"
  - "pdd.voucher.voucher.complain"
captured_at: "2026-04-09T15:21:13.004Z"
---
...
| out\_biz\_no | STRING | 必填 | 外部流水号 |
| voucher\_list | OBJECT\[\] | 非必填 | 优惠券信息列表,例子\[{"voucher\_id":"test voucher\_id","voucher\_no":"test voucher\_no"}\] |
| voucher\_id | STRING | 非必填 | 卡券ID |
| voucher\_no | STRING | 非必填 | 卡券号 |
| complain\_user | STRING | 必填 | 投诉人 |
| complain\_user\_mobile | STRING | 必填 | 投诉人电话 |
| complain\_content | STRING | 必填 | 投诉内容 |
| complain\_attachment\_list | STRING\[\] | 必填 | \["http://testimg.yangkeduo.com/pdd\_oms/2018-01-16/411068e948835ae053a86c13f8ebb5ee.jpg"\] |
| complain\_type | INTEGER | 必填 | 枚举值1、大闸蟹死蟹或者少蟹 ；2、大闸蟹重量不符；3、大闸蟹公母数量不符；4、大闸蟹产地不符；5、欺诈发货（收到的产品非大闸蟹）；6、蟹券无法提货 7、其他质量问题 |

### 返回参数说明

| 参数接口 | 参数类型 | 例子 | 说明 |
| --- | --- | --- | --- |
| voucher\_voucher\_complain\_response | OBJECT |  | response |
| is\_success | BOOLEAN |  | 是否请求成功 |
...
import com.pdd.pop.sdk.http.api.pop.request.PddVoucherVoucherComplainRequest;
import com.pdd.pop.sdk.http.api.pop.response.PddVoucherVoucherComplainResponse;
import com.pdd.pop.sdk.http.api.pop.request.PddVoucherVoucherComplainRequest.VoucherListItem;
...
public class PopClientDemo {
 ...
        PddVoucherVoucherComplainRequest request = new PddVoucherVoucherComplainRequest();
        request.setOrderSn("str");
        request.setOutBizNo("str");
        List<VoucherListItem> voucherList = new ArrayList<VoucherListItem>();

        VoucherListItem item = new VoucherListItem();
        item.setVoucherId("str");
        item.setVoucherNo("str");
        voucherList.add(item);
        request.setVoucherList(voucherList);
        request.setComplainUser("str");
        request.setComplainUserMobile("str");
        request.setComplainContent("str");
        List<String> complainAttachmentList = new ArrayList<String>();
        complainAttachmentList.add("str");
        request.setComplainAttachmentList(complainAttachmentList);
        request.setComplainType(0);
        PddVoucherVoucherComplainResponse response = client.syncInvoke(request, accessToken);
...
```
{
  "voucher_voucher_complain_response": {
    "is_success": false
  }
}
```

---
title: "如何实现文本展开收起功能"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-faqs/faqs-arkui-267"
menu_path:
  - "FAQ"
  - "应用框架开发"
  - "UI框架"
...
// Text line width
const TEXT\_WIDTH: number = 350;
// Number of lines to show when collapsed
const COLLAPSE\_LINES: number = 2;
const ELLIPSIS: string = '...';
const EXPAND\_STR: string = 'Expand';
const COLLAPSE\_STR: string = 'Collapse';
const FULL\_TEXT: string =
...
struct TextCollapseTest {
  ...
  expandText(): void {
    if (this.needProcess) {
      this.suffixStr = COLLAPSE\_STR;
      this.expanded = true;
      this.title = FULL\_TEXT;
    }
  }
  ...
  collapseText(): void {
    ...
    let expandSize: SizeOptions = this.getUIContext().getMeasureUtils().measureTextSize({
      textContent: FULL\_TEXT,
      constraintWidth: TEXT\_WIDTH,
      fontSize: 30
    });
    ...
    let clipTitle: string = FULL\_TEXT;
    this.suffixStr = EXPAND\_STR;
    ...
    while (true) {
      tempTitle = this.title.substring(0, cursor) + ELLIPSIS + EXPAND\_STR;

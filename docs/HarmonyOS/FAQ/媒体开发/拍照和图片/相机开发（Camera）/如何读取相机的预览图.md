---
title: "如何读取相机的预览图"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-faqs/faqs-camera-6"
menu_path:
  - "FAQ"
  - "媒体开发"
  - "拍照和图片"
  - "相机开发（Camera）"
  - "如何读取相机的预览图"
captured_at: "2026-04-17T02:03:18.415Z"
---
...
async function createDualChannelPreview(cameraManager: camera.CameraManager, XComponentSurfaceId: string, receiver: image.ImageReceiver): Promise<void> {
  ...
  // Get profile object
  let profiles: camera.CameraOutputCapability = cameraManager.getSupportedOutputCapability(camerasDevices\[0\], camera.SceneMode.NORMAL\_PHOTO); // 获取对应相机设备profiles

---
title: "应用切面编程设计"
source_url: "https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-application-aspect-programming-design"
menu_path:
  - "最佳实践"
  - "应用框架"
  - "ArkTS语言"
  - "应用切面编程设计"
captured_at: "2026-04-17T01:54:14.141Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/dc/v3/f1cKGw5bRYS5M4aOfvQKsw/zh-cn_image_0000002380185801.png?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=9484739EDE13913A658A965AF7BDBBEF7432B308299A380A6D50DA1DDDB575A6 "点击放大")
...
**图2** 类的实例化示意  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bc/v3/ANaqYStfS0KaXC-cagecLg/zh-cn_image_0000002346386188.png?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=054F8CEBAEEE021D6CBA7D657817BFB45A6B63D2035E97D9CAEE29FFDA1F29F2 "点击放大")

原型对象也有原型\_proto\_。类的继承通过原型实现。实例方法调用时，会在原型链上查找方法，找到后执行调用。具体如图3所示。

**图3** 类的原型与继承  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e1/v3/fYiagsl7QJyLJzTA4KmoTQ/zh-cn_image_0000002346386564.png?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=9CB4E0AE429132FE7817E81291C81C310A31D04C3DE3C20010243E3A5818E068 "点击放大")

插桩和替换的操作是将回调参数与原方法组合成新函数，再用新函数替换原方法。具体如图4所示。

**图4** 插桩和替换原理示意图  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/be/v3/vZk_34gISD6AIZt5D_xHwA/zh-cn_image_0000002346546400.png?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=F07CFA62E1E74D52DF50622B7043BFA67DE6B61D476C9C95DEE817BBAC41DBBC "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c0/v3/dT35N4jNTEOyxo-Z50vzAg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=64AF24779CFB84B7917DE5B3AB8997BD31C696F77CC160F0AE73C677DB00C343)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d5/v3/Z2XfHrueQ5ituKROBoezvg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=AC2FF54A45D0B823A2F40BE98CBE39737BE25B54475C7152C537C055EF653A9E)
...
1.  修改foo()方法中的打印日志。
    ...
    util.Aspect.replace(Test2, 'foo', false, (instance: Test2, arg: string) => {
      console.log(arg + ' \_\_replaced implementation');
    });
    new Test2().foo('123');
    // \[LOG\]: "123 \_\_replaced implementation"
...
1.  Child类继承了Base类的获取实时位置方法，但测试发现Child类的getCurrentLocation()方法在实际场景中调用非常频繁，需要控制调用频率。采取的措施是修改Child类的getCurrentLocation()方法的实现，通过缓存位置信息来减少系统接口的调用次数。具体实现为：如果从上次调用到现在的时间不足60秒，则直接返回缓存的位置信息；否则，调用系统接口获取新的位置信息并更新缓存。
    ...
    let cached\_location: Object | undefined;
    let time: number | undefined;
    util.Aspect.replace(Child, 'getCurrentLocation', false, () => {
      let newTime = new Date().getTime();
      // Real-time location can be called at most once per minute.
      if (!cached\_location || !time || newTime - time > 60000) {
        time = newTime;
        cached\_location = geoLocationManager.getCurrentLocation();
      }
      // Return cached location information
      return cached\_location;
    });
    new Child().getCurrentLocation()
    

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a9/v3/oOm0nD_uQxiftfgNrnWIXg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=BB8B7DE3F36987DABE2B1C53E53B471F186F7519C2465A5E4C70E169DDBC1CAB)
...
7.  接口不限制对系统提供的类方法进行插桩。只要类和方法在运行时是实际存在的对象，并且方法的属性描述符的writable字段为true，就可以使用对应接口进行插桩和替换。
    ...
    ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7a/v3/VKIWs4PiSzeg4tIMSIwfVA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=5837A17D63FE2059F122B6C2FE144F328CEE558A504A1782C110A440A6AD7768)

---
title: "音质切换开发实践"
source_url: "https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-sound-quality-switching"
menu_path:
  - "最佳实践"
  - "媒体"
  - "音频和视频"
  - "音质切换开发实践"
captured_at: "2026-04-17T01:54:15.416Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a2/v3/FY-E68G3Q8SC7l18PdBYvA/zh-cn_image_0000002490682592.gif?HW-CC-KV=V1&HW-CC-Date=20260417T015417Z&HW-CC-Expire=86400&HW-CC-Sign=ACA52DDDCCC5739CBA57CD6473A76AFE5F4011EFC3199EABF8512B440B05D0A0 "点击放大")
...
1.  在MediaTools类中，定义如下方法：
    ...
    export class MediaTools {
      ...
      static getMsFromByteLength(byteLength: number): number {
        return 1000 \* (byteLength / SECOND\_BUFFER\_WALK);
      }
...
2.  在AVPlayerController类中，定义如下方法：
    ...
    export class AVPlayerController {
      ...
      seek(currentTime: number): void {
        this.avPlayer?.seek(currentTime, media.SeekMode.SEEK\_NEXT\_SYNC);
      }
      // ...
    }
    ...
    export class AudioRendererController {
      ...
      public async initAudioRenderer(fd: number, offset: number, length: number): Promise<void> {
        ...
        let audioStreamInfo: audio.AudioStreamInfo = {
          samplingRate: audio.AudioSamplingRate.SAMPLE\_RATE\_48000, // Audio file sampling rate
          channels: audio.AudioChannel.CHANNEL\_2, // Number of audio file channels
          sampleFormat: audio.AudioSampleFormat.SAMPLE\_FORMAT\_S16LE, // Audio sampling format
          encodingType: audio.AudioEncodingType.ENCODING\_TYPE\_RAW // Audio encoding format
        };
        // AudioRenderer information
        let audioRendererInfo: audio.AudioRendererInfo = {
          usage: audio.StreamUsage.STREAM\_USAGE\_MUSIC, // Audio stream usage type
        ...
        await audio.createAudioRenderer(audioRendererOptions).then((data) => {
          ...
          if (this.audioRenderer !== undefined) {
            try {
              ...
              this.audioRenderer.setInterruptMode(audio.InterruptMode.INDEPENDENT\_MODE);
      ...
      public async start(): Promise<void> {
        if (this.audioRenderer !== undefined) {
          // Audio state
          let stateGroup = \[audio.AudioState.STATE\_PREPARED, audio.AudioState.STATE\_PAUSED, audio.AudioState.STATE\_STOPPED\];
      ...
      public pause(): void {
        if (this.audioRenderer !== undefined) {
          if (this.audioRenderer.state.valueOf() !== audio.AudioState.STATE\_RUNNING) {
            return;
          }
...
5.  在页面中自定义changeType()事件，通过[getRawfd()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-resource-manager#getrawfd9-1)获取resources/rawfile目录下的音频资源，将获取到的数据传入PlayerController的changeType()方法，调用playerController的seek()方法传入当前Slider进度的值，用于跳转播放。
    ...
    Menu() {
      MenuItem({
        content: $r('app.string.standard\_quality'),
      }).onClick(() => {
        if (this.index === 0) {
          return;
        }
        this.text = $r('app.string.standard\_quality');
        this.index = 0;
      ...
      // ...
      MenuItem({
        content: $r('app.string.pcm\_high\_quality'),
      })
        .onClick(() => {
          if (this.index === 2) {
            return;
          }
          this.text = $r('app.string.pcm\_high\_quality');
          this.index = 2;
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fd/v3/i5aCz9QdT3CNMoWzDSY76w/zh-cn_image_0000002522762335.gif?HW-CC-KV=V1&HW-CC-Date=20260417T015417Z&HW-CC-Expire=86400&HW-CC-Sign=3B7A4AB9DBAFD9A0B841702923E422C3775F61BDF744F8DC5F287556F7714FBC "点击放大")
...
1.  在MenuItem组件的点击事件中调用自定义changeType()方法，传入音频资源文件路径和对应音频格式audioType。
    ...
    Menu() {
      ...
      MenuItem({
        content: $r('app.string.flac\_high\_quality'),
      })
        .onClick(() => {
          if (this.index === 1) {
            return;
          }
          this.text = $r('app.string.flac\_high\_quality');
          this.index = 1;

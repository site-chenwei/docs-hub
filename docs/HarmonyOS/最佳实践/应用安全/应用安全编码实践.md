...
menu_path:
  ...
  - "应用安全"
  - "应用安全编码实践"
captured_at: "2026-04-17T01:54:17.422Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/18/v3/IQMsWOm_Q22zMq9bO9pcdQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T015419Z&HW-CC-Expire=86400&HW-CC-Sign=9BB65A44736EBC656CAEEDEDD0CE05AB29A7F7B9BCD1C8E8C38591D22E891B95)
...
function publishEventWithData() {
  let options: commonEventManager.CommonEventPublishData = {
    code: 1,
    data: 'ContactData', // Sending sensitive data
    subscriberPermissions: \['ohos.permission.READ\_CONTACTS'\], // Set the permission
  }
  commonEventManager.publish('MyCommonEvent', options, (err) => {
    if (err.code) {
      hilog.error(0x0000, 'privacy', \`publish event error: ${err.code} ${err.message} ${err.name} ${err.stack}\`);
...
function exesql(sql: string) {
  ...
  relationalStore.getRdbStore(context, STORE\_CONFIG, (err: BusinessError, rdbStore: relationalStore.RdbStore) => {
    store = rdbStore;
    if (err) {
  ...
  let SQL\_DELETE\_TABLE = 'DELETE FROM test WHERE name = ' + sql;
  if (store != undefined) {
    (store as relationalStore.RdbStore).executeSql(SQL\_DELETE\_TABLE, (err) => {
      if (err) {

...
menu_path:
  ...
  - "应用安全"
  - "应用权限申请"
captured_at: "2026-04-17T01:54:17.371Z"
---
...
-   实施策略：如果用户拒绝了某项权限申请，应用不应再次弹窗请求该权限。应在页面的适当位置添加明确的提示，指导用户开启权限或退出当前需要该权限的场景，直到用户重新触发时再引导其完成授权。例如，权限申请被拒绝后，按钮上方显示提示文字，具体效果见下图：
    ...
    ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b/v3/0QJu2mS3RK2icQTXrqmopQ/zh-cn_image_0000002194010136.png?HW-CC-KV=V1&HW-CC-Date=20260417T015419Z&HW-CC-Expire=86400&HW-CC-Sign=AFEF6EB85C2DFF8ADE4DFE0BB0EB9EFD12B689C2F818632D8DAA37D6A955BE25 "点击放大")
...
-   正例1（以示例代码为例，效果图如下）
    ...
    ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/25/v3/MFjYwk2uQpyBTyjvH-3k1A/zh-cn_image_0000002193850544.png?HW-CC-KV=V1&HW-CC-Date=20260417T015419Z&HW-CC-Expire=86400&HW-CC-Sign=5F8DCB519CAE2E1F673AFEF36063071D09AC0400016D94BF148705CD2E48130D "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/44/v3/TzE1v0TgTO-V-dI4BsyblQ/notice_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T015419Z&HW-CC-Expire=86400&HW-CC-Sign=CC4B5D50E97A2FA87500106DF85402AF7F55EE59B24F0C15477C69DE7305D54F)
...
3.  不允许：点击后应用无法获取该权限，且再次调用requestPermissionsFromUser()方法无法拉起该权限设置弹框。
    ...
    ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/68/v3/x7HXpEFgTfOxjPQcg7p5VQ/zh-cn_image_0000002194010128.png?HW-CC-KV=V1&HW-CC-Date=20260417T015419Z&HW-CC-Expire=86400&HW-CC-Sign=7F08BE5787EE24DF0E7950BEEA88FFB1D63CD1943FDA81E07E3767FFC9F38C78 "点击放大")
    

具体流程图如下：

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4c/v3/mJ7agLNTSMWSLAO8MqdSdw/zh-cn_image_0000002267532549.jpg?HW-CC-KV=V1&HW-CC-Date=20260417T015419Z&HW-CC-Expire=86400&HW-CC-Sign=298514207404C3EECEF8727C3CA000F8AD948EA5AB6606A6A1A8524D52B7DF55 "点击放大")
...
checkPermissionGrant(): void {
  ...
  try {
    ...
    let approximatelyLocation = atManager.checkAccessTokenSync(tokenId, 'ohos.permission.APPROXIMATELY\_LOCATION');
    let location = atManager.checkAccessTokenSync(tokenId, 'ohos.permission.LOCATION');
    hasPermission = approximatelyLocation === abilityAccessCtrl.GrantStatus.PERMISSION\_GRANTED &&
      location === abilityAccessCtrl.GrantStatus.PERMISSION\_GRANTED;
  } catch (error) {
    const err: BusinessError = error as BusinessError;
...
requestPermissions(): void {
  let atManager = abilityAccessCtrl.createAtManager();
  try {
    atManager.requestPermissionsFromUser(this.context, \['ohos.permission.APPROXIMATELY\_LOCATION',
      ...
      if (data.authResults\[0\] === -1 || data.authResults\[1\] === -1) {
        if (data.dialogShownResults && (data.dialogShownResults\[0\] || data.dialogShownResults\[1\])) {
          ...
          if(data.authResults\[0\] === -1){
            this.permissionsMessage = $r('app.string.obtain\_location\_permission');
          } else {
            this.permissionsMessage = $r('app.string.obtain\_precise\_positioning')
          }
        } else {
...
private openPermissionsSetting(): void {
  let atManager = abilityAccessCtrl.createAtManager();
  atManager.requestPermissionOnSetting(this.context, \['ohos.permission.APPROXIMATELY\_LOCATION',
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/33/v3/1qE0VM0pRwKlN4mSrVtU7Q/zh-cn_image_0000002267612717.jpg?HW-CC-KV=V1&HW-CC-Date=20260417T015419Z&HW-CC-Expire=86400&HW-CC-Sign=F15C29297254DADFB2304400AF2A53A3B986994BF5024F1E2E689006F419D4D0 "点击放大")
  ...
  if (!isLocationEnabled) {
    atManager.requestGlobalSwitch(this.context, abilityAccessCtrl.SwitchType.LOCATION).then((data: boolean) => {
      if (data) {
        ...
        this.locationServiceMessage = $r('app.string.open\_location\_services');
      }
    }).catch((err: BusinessError) => {
      hilog.error(0x0000, 'Index', 'data:' + JSON.stringify(err));
    });

...
menu_path:
  ...
  - "应用安全"
  - "加解密跨平台数据兼容性"
captured_at: "2026-04-17T01:54:17.401Z"
---
    ...
    public static async convertStrToPubKey(keyStr: string): Promise<cryptoFramework.PubKey | undefined> {
      ...
      try {
        const pubKeySpec: cryptoFramework.ECCPubKeySpec = {
          params: cryptoFramework.ECCKeyUtil.genECCCommonParamsSpec('NID\_sm2'),
          pk: pks,
          algName: 'SM2',
    ...
    public static async convertStrToPriKey(keyStr: string): Promise<cryptoFramework.PriKey | undefined> {
      try {
        ...
        const priKeySpec: cryptoFramework.ECCPriKeySpec = {
          ...
          sk: sks,
          algName: 'SM2',
          specType: cryptoFramework.AsyKeySpecType.PRIVATE\_KEY\_SPEC
        };
...
-   SM2密文格式转换
    ...
    i2dSM2CipherText(primal\_data: string): Uint8Array | undefined {
      // Converting a Hex String to byte array.
      const cipherBytes = DataConversion.hexStrToUint8Array(primal\_data);
      ...
      if (totalLength < c1Length + 32) {
        hilog.error(0, 'id2\_SM2\_Ciphertext', 'Non-SM2 criterion compliant.');
      }
      ...
      if (c1Bytes\[0\] !== 0x04) {
        hilog.error(0, 'id2\_SM2\_Ciphertext', 'C1 is not in uncompressed format.');
      }
    ...
    d2i\_SM2\_Ciphertext(standard\_data: string): string {
      let message: string = standard\_data;
      if (!message.startsWith('30')) {
        hilog.error(0, 'd2i\_SM2\_Ciphertext', 'Incorrect ciphertext format');
      }
      message = message.slice('30'.length, message.length);
    
      // The length of the SM2 sequence content.
      let sequence\_lexHex: string = this.getLenHex(message);
      message = message.slice(sequence\_lexHex.length, message.length);
      let sequence\_len: number = this.lenHex2number(sequence\_lexHex);
    
      if (sequence\_len !== message.length / 2) {
        hilog.error(0x0000, 'd2i\_SM2\_Ciphertext', 'Incorrect ciphertext format');
      }
    
      let sm2\_sequence = new SM2Sequence();
      message = this.readC1(sm2\_sequence, message);
      message = this.readC3(sm2\_sequence, message);
      message = this.readC2(sm2\_sequence, message);
      hilog.info(0x0000, 'sm2\_sequence', sm2\_sequence.toString());
    
      let primal\_data: string = sm2\_sequence.C1x + sm2\_sequence.C1y + sm2\_sequence.C3 + sm2\_sequence.C2;
      return primal\_data;
    }
...
-   AES指定分组模式后偏移量参数设置问题
    ...
    加解密参数add，长度为0~INT\_MAX字节，常用为16字节。

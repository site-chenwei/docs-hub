...
menu_path:
  ...
  - "后台软件资源合理使用"
  - "后台音频播放合理使用"
captured_at: "2026-04-17T01:54:18.888Z"
---
...
async function read() {
  ...
  const filePath = path + '/voice\_call\_data.wav'; // Prohibit the file from being played silently
  try {
    let file: fs.File = fs.openSync(filePath, fs.OpenMode.READ\_ONLY); // Open the file

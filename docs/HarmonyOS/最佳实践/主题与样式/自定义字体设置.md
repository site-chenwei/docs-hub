---
title: "自定义字体设置"
source_url: "https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-custom-font-settings"
menu_path:
  - "最佳实践"
  - "主题与样式"
  - "自定义字体设置"
captured_at: "2026-04-17T01:54:13.941Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8e/v3/gnlQteSdQ7G86A0bKIRSTA/zh-cn_image_0000002361481894.gif?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=95DA57437D23894DE37E61D1DBA84B13F7B869714EC47A8D3A484BC940EFA585 "点击放大")
...
1.  创建首选项工具类PreferenceUtils，在类中定义如下方法：
    ...
    export class PreferenceUtils {
      ...
      saveModifyFont(textFont: string) {
        try {
          this.preference?.putSync(TEXT\_FONT, textFont);
      ...
      getFont(): string {
        let textFont: string = '';
        try {
          textFont = this.preference?.getSync(TEXT\_FONT, '') as string;
        } catch (err) {
          let error = err as BusinessError;
    ...
    export function registerMyFont(uiContext: UIContext) {
      try {
        ...
        uiContext.getFont().registerFont({
          familyName: $r('app.string.HarmonyOS\_Italic'),
          familySrc: $rawfile('HarmonyOS\_SansItalic.ttf')
        });
        // Register HarmonyOS Condensed font through registrant Font
        uiContext.getFont().registerFont({
          familyName: $r('app.string.HarmonyOS\_Condensed'),
          familySrc: $rawfile('HarmonyOS\_Condensed.ttf')
        });
      } catch (err) {
        let error = err as BusinessError;
...
5.  在MenuItem的onChange()事件中修改fontOffset的值，并通过saveModifyFont()方法，将选择的字体数据写入首选项并持久化存储。
    ...
    MenuItem({
      content: item === '' ? $r('app.string.system\_default') : item,
...
6.  通过fontFamily属性传入变量fontOffset，使用注册的自定义字体改变字体样式。
    ...
    Column() {
      Text($r('app.string.preview\_text'))
        // ...
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1b/v3/IH80bOb1ThaYKCW8BAa8DQ/zh-cn_image_0000002395161853.gif?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=34603B30E22BD25DC568D581CF317A87C9A34F7C1D0167E8305FAEFF8472BCCB "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/33/v3/sMBi-I4qRJGUeQhgM_Yf8A/zh-cn_image_0000002361641798.gif?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=5F7472D4A37CE21984C2B7E4FDB527DE1592D2C3D8ADD8BC2A1D2621C3F414D0 "点击放大")
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/87/v3/3Lu0a1SrRaCy6nTxgqkl9Q/zh-cn_image_0000002395321741.gif?HW-CC-KV=V1&HW-CC-Date=20260417T015415Z&HW-CC-Expire=86400&HW-CC-Sign=E3E6A031C7DF0BDE109EFDC1E9F8E2070BD51632E9BC82ABFEA8791B7675C56A "点击放大")

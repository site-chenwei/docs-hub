---
title: "跨设备剪贴板常见场景"
source_url: "https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-distributed-pasteboard"
menu_path:
  - "最佳实践"
  - "自由流转"
  - "多端协同"
  - "跨设备剪贴板常见场景"
captured_at: "2026-04-17T01:54:16.674Z"
---
...
let pasteData: pasteboard.PasteData = pasteboard.createData(pasteboard.MIMETYPE\_TEXT\_URI, 'PageABuilder');
...
onPageShow(): void {
  let systemPasteboard: pasteboard.SystemPasteboard = pasteboard.getSystemPasteboard();
  try {
    let result: boolean = systemPasteboard.hasDataType(pasteboard.MIMETYPE\_TEXT\_URI);
    if (result) {
...
checkPermissionGrant(): void {
  ...
  try {
    ...
    let approximatelyLocation = atManager.checkAccessTokenSync(tokenId, 'ohos.permission.READ\_PASTEBOARD');
...
let pasteData: pasteboard.PasteData = pasteboard.createData(pasteboard.MIMETYPE\_TEXT\_PLAIN, '');
...
record.addEntry(pasteboard.MIMETYPE\_TEXT\_HTML, html);
record.addEntry(pasteboard.MIMETYPE\_TEXT\_PLAIN, styledString.getString());
...
for (let i = 0; i < htmlSpans.length; i++) {
  ...
  if (!!(span as RichEditorImageSpanResult)?.valuePixelMap) {
    ...
    record.addEntry(pasteboard.MIMETYPE\_PIXELMAP, pixelMap);
    break;
  }
}
          ...
          try {
            ...
            for (let i = 0; i < gettedData.getRecordCount(); i++) {
              ...
              let targetTypes: string\[\] = \[
                pasteboard.MIMETYPE\_TEXT\_HTML,
                pasteboard.MIMETYPE\_TEXT\_PLAIN,
                pasteboard.MIMETYPE\_PIXELMAP
              \];
              let tmpTypes: string\[\] = record.getValidTypes(targetTypes);
              for (let j = 0; j < tmpTypes.length; j++) {
                switch (tmpTypes\[j\]) {
                  case pasteboard.MIMETYPE\_TEXT\_HTML: {
                    let htmlRecord = await record.getData(pasteboard.MIMETYPE\_TEXT\_HTML);
                    htmlRecord = htmlRecord as string;
                    this.handlerPasteHtmlData(htmlRecord);
                    isPastedSuccess = true;
                    break;
                  }
                  case pasteboard.MIMETYPE\_TEXT\_PLAIN: {
                    let textRecord = await record.getData(pasteboard.MIMETYPE\_TEXT\_PLAIN);
                  ...
                  case pasteboard.MIMETYPE\_PIXELMAP: {
                    let pixelMapRecord = await record.getData(pasteboard.MIMETYPE\_PIXELMAP);
...
let file = fs.openSync(fileFullName, fs.OpenMode.READ\_WRITE | fs.OpenMode.CREATE);
let writeLen = fs.writeSync(file.fd, VALUE\_TEST\_STRING\_ELEMENT);
hilog.info(0x0000, '\[ProgressBar\]', 'The size of str is: ' + writeLen);
fs.closeSync(file);
let pasteData =
  pasteboard.createData(pasteboard.MIMETYPE\_TEXT\_URI, fileUri.getUriFromPath(fileFullName));

---
title: "多网并发网络加速"
source_url: "https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-path-network-turbo"
menu_path:
  - "最佳实践"
  - "网络"
  - "多网并发网络加速"
captured_at: "2026-04-17T01:54:14.678Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/89/v3/lJMnFdzbSAGg63o9GoX4lw/zh-cn_image_0000002456588494.png?HW-CC-KV=V1&HW-CC-Date=20260417T015416Z&HW-CC-Expire=86400&HW-CC-Sign=DD2F3EC4829D4319AC7DE0FBFDB13F4D481FD9F6C6E1C3F3A77DB708C713FB5F "点击放大")
    ...
    export function setUploadSceneDesc(enter: boolean): boolean {
      try {
        let sceneDesc: netBoost.SceneDesc = {
          scene: 'upload',
          sceneEvent: enter ? netBoost.SceneEvent.SCENE\_EVENT\_ENTER : netBoost.SceneEvent.SCENE\_EVENT\_LEAVE
        };
      ...
      return new Promise((resolve) => {
        try {
          netHandover.requestMultiPath((data: netHandover.MultiPathRequestResult) => {
            if (data && data.result === netHandover.MultiPathErrorResult.MULTIPATH\_ERROR\_NONE) {
              resolve(true);
            } else {
              resolve(false);
            }
    ...
    export function setOnMultiPathStateChange(netHandleChange: NetHandleChange) {
      try {
        netHandover.on('multiPathStateChange', (data: netHandover.MultiPathStateInfo) => {
          if (data.multiPathState === netHandover.MultiPathState.MULTIPATH\_CREATED &&
            data.netHandle.netId >= CommonConst.MIN\_USEFUL\_NET\_ID) {
            netHandleChange?.onMultiNetSuccess(data.netHandle);
          } else if (data.multiPathState === netHandover.MultiPathState.MULTIPATH\_CREATING) {
    ...
    export function getAllNetHandles(): connection.NetHandle\[\] | undefined {
      try {
        ...
        for (let i = 0; i < netHandles.length; i++) {
          ...
          if (netHandles\[i\].netId >= CommonConst.MIN\_USEFUL\_NET\_ID) {
...
3.  文件分片处理
    ...
    function getChunkSize(fileSize: number): number {
      const MB = CommonConst.MIN\_SLICE\_FILE\_SIZE;
      const fullMBs = Math.floor(fileSize / MB);
      let res = Math.max(1, Math.min(CommonConst.MAX\_SLICE\_FILE\_COUNT, fullMBs));
...
4.  多Socket并发上传不同文件片段
    ...
    async function initMultiPartUpload(host: string, port: number,
      ...
      // build request info
      const request = \`${method} ${path} HTTP/1.1\\r\\n\` + headers.join('\\r\\n') + CommonConst.HEADER\_BODY\_DELIMITER;
    ...
    async function uploadSegment(host: string, port: number, initResult: InitiateMultipartUpload, fileChunk: FileUpChunk,
      ...
      await bindSpecifiedNet(socketInstance, netId, CommonConst.SOCKET\_BIND\_UP\_PORT);
    ...
    private async uploadFileChunkWithNet(netHandle: connection.NetHandle) {
      ...
      if (fileChunkWithData) {
        const task =
          new taskpool.Task(uploadSegment, CommonConst.UP\_HOST, CommonConst.HTTP\_DEFAULT\_PORT, this.initMultipartUpload!,
...
5.  通知服务端整合分片文件
    ...
    private async mergeSegmentsOnServer() {
      ...
      const task = new taskpool.Task(completeMultiPartUploadRequest, CommonConst.UP\_HOST, CommonConst.HTTP\_DEFAULT\_PORT,
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a9/v3/Fg-3znLYRlejOxWTA4GQRQ/zh-cn_image_0000002489628085.png?HW-CC-KV=V1&HW-CC-Date=20260417T015416Z&HW-CC-Expire=86400&HW-CC-Sign=D7B152347960122E3DD819EC6BCA32F953E91B560EE305EED50D13AE8CFB0678 "点击放大")
...
public async startDownload() {
  if (this.downUrls) {
    ...
    const task = new taskpool.Task(getFileSizeWithNet, this.downUrls, CommonConst.HTTP\_DEFAULT\_PORT, netId);
    try {
      ...
      if (fileSize > 0) {
        ...
        const downTask =
          new taskpool.Task(downloadFileChunkWithNet, this.downUrls, CommonConst.HTTP\_DEFAULT\_PORT, this.fileChunk,
...
private startDownFirstFile(netHandle: connection.NetHandle) {
  ...
  this.firstFileDownloader.onError = () => {
    ...
    this.downFirstProgressColor = $r('sys.color.multi\_color\_08');
    this.taskArray.shift();
    this.downFirstFileResult = $r('app.string.down\_failed');
  }
  // init downloading status
  this.firstFileDownloading = true;
  this.downFirstProgress = 0;
  this.downFirstProgressColor = $r('sys.color.comp\_background\_emphasize');
  this.downFirstFileResult = '';
  // start download

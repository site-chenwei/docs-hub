---
title: "白盒性能测试框架使用指导"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/perftest-guideline"
menu_path:
  - "指南"
  - "应用测试"
  - "单元测试和UI测试"
  - "自动化测试框架使用指导"
  - "白盒性能测试框架使用指导"
captured_at: "2026-04-17T01:36:43.902Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/69/v3/Tw8B54nOTfmR9aaa-xK82w/zh-cn_image_0000002529421640.png?HW-CC-KV=V1&HW-CC-Date=20260417T013644Z&HW-CC-Expire=86400&HW-CC-Sign=9A1AA570200E9444CB85DA1EA96733C72944CC783CE015E113C4A4E9D6AE4D88)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8c/v3/LPSj_RoGTSmHdwDhbYD0oA/zh-cn_image_0000002560301617.png?HW-CC-KV=V1&HW-CC-Date=20260417T013644Z&HW-CC-Expire=86400&HW-CC-Sign=9C554620F58D48B192924C2AE31DFE6F14A9B0A9EE513E1D72EEA109A8372FE0)
...
1.  定义测试性能指标列表
    ...
    let metrics: Array<PerfMetric> = \[PerfMetric.DURATION, PerfMetric.CPU\_USAGE\]; // 定义待测指标
...
let res2: PerfMeasureResult = perfTest.getMeasureResult(PerfMetric.CPU\_USAGE); // 获取CPU使用率指标的测试结果
    ...
    export default function PerfTestTest() {
      describe('PerfTestTest1', () => {
        it('testExample2', Level.LEVEL3, async (done: Function) => {
          ...
          await driver.delayMs(1000);
          let metrics: Array<PerfMetric> = \[PerfMetric.LIST\_SWIPE\_FPS\]; // 指定被测指标为列表滑动帧率
          ...
          try {
            let perfTest: PerfTest = PerfTest.create(perfTestStrategy); // 创建测试任务对象PerfTest
            await perfTest.run(); // 执行测试，异步函数需使用await同步等待完成
            let res: PerfMeasureResult = perfTest.getMeasureResult(PerfMetric.LIST\_SWIPE\_FPS); // 获取列表滑动帧率指标的测试结果
            perfTest.destroy(); // 销毁PerfTest对象

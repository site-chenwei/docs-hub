---
title: "创建并使用环境资源"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkgraphics3d-resource-environment"
menu_path:
  - "指南"
  - "图形"
  ...
  - "创建并使用环境资源"
captured_at: "2026-04-17T01:36:09.204Z"
---
    ...
    function createEnvironmentPromise() : Promise<Environment> {
      return new Promise((resolve, reject) => {
        ...
        if (globalScene) {
          ...
          let sceneImageParameter: SceneResourceParameters = { name: 'image', uri: $rawfile('image/Cube\_BaseColor.png') };
          ...
          image.then(async (imageEntity: Image) => {
            ...
            env.then(async (envEntity: Environment) => {
              envEntity.backgroundType = EnvironmentBackgroundType.BACKGROUND\_EQUIRECTANGULAR;

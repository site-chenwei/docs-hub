---
title: "管理网络连接"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/net-connection-manager"
menu_path:
  - "指南"
  - "系统"
  - "网络"
  ...
  - "连接网络"
  - "管理网络连接"
captured_at: "2026-04-17T01:35:53.521Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/0a/v3/3Ay8pz5RQ4GOG9zQF2dtag/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013555Z&HW-CC-Expire=86400&HW-CC-Sign=A0EFAE99B24CFA66FA3B4157DFD7123D5453B8AA707F462BC0D8DA8513D1D404)
...
3.  调用[createNetConnection](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-net-connection#connectioncreatenetconnection)方法，指定网络能力、网络类型和超时时间(可选，如不传入代表默认网络；创建不同于默认网络时可通过指定这些参数完成)，创建一个NetConnection对象。
    ...
    // 指定超时时间为10s(默认值为0)
    let TIMEOUT = 10 \* NETWORK\_CONNECTION\_TIMEOUT;
...
4.  调用该对象的[on()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-net-connection#onnetavailable)方法，传入type和callback，订阅关心的事件。
    ...
    // 订阅网络能力变化事件，如果当前指定网络的能力发生变化，通过on\_netCapabilitiesChange通知用户
    ...
    // 订阅网络连接信息变化事件，如果当前指定网络的连接信息发生变化，通过on\_netConnectionPropertiesChange通知用户
    ...
    // 订阅网络丢失事件，如果当前处于连接状态的指定网络断开，通过on\_netLost通知用户
            ...
            connection.getNetCapabilities(netHandleInfo).then(
              (data: connection.NetCapabilities) => {
                ...
                for (let item of bearerTypesNum) {
                  if (item == 0) {
                    // 蜂窝网络
                    hilog.info(0x0000, 'testTag', JSON.stringify('BEARER\_CELLULAR'));
                  } else if (item == 1) {
                    // Wi-Fi网络
                    hilog.info(0x0000, 'testTag', JSON.stringify('BEARER\_WIFI'));
                  } else if (item == 3) {
                    // 以太网网络
                    hilog.info(0x0000, 'testTag', JSON.stringify('BEARER\_ETHERNET'));
                  }
                }
                ...
                for (let item of dataNumber) {
                  if (item == 0) {
                    ...
                    hilog.info(0x0000, 'testTag', JSON.stringify('NET\_CAPABILITY\_MMS'));
                  } else if (item == 11) {
                    // 表示网络流量未被计费
                    hilog.info(0x0000, 'testTag', JSON.stringify('NET\_CAPABILITY\_NOT\_METERED'));
                  } else if (item == 12) {
                    // 表示该网络应具有访问Internet的能力，该能力由网络提供者设置
                    hilog.info(0x0000, 'testTag', JSON.stringify('NET\_CAPABILITY\_INTERNET'));
                  } else if (item == 15) {
                    // 表示网络不使用VPN（Virtual Private Network，虚拟专用网络）
                    hilog.info(0x0000, 'testTag', JSON.stringify('NET\_CAPABILITY\_NOT\_VPN'));
                  } else if (item == 16) {
                    // 表示该网络访问Internet的能力被网络管理成功验证，该能力由网络管理模块设置
                    hilog.info(0x0000, 'testTag', JSON.stringify('NET\_CAPABILITY\_VALIDATED'));
                  }
                }
              });
          }
...
2.  代码示例
    ...
    // ...
        ...
        if (!netHandle || netHandle.netId === 0) {
          ...
          // 判断网络是否可以访问互联网。
          if (cap?.includes(connection.NetCap.NET\_CAPABILITY\_CHECKING\_CONNECTIVITY)) {
            // 正在验证网络连通性，请稍后重试。
            hilog.info(0x0000, 'testTag', 'default network is checking, please try again later');
          } else {
            if (cap?.includes(connection.NetCap.NET\_CAPABILITY\_VALIDATED)) {

---
title: "证书对象的创建、解析和校验"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/create-parse-verify-cert-object"
menu_path:
  - "指南"
  - "系统"
  - "安全"
  ...
  - "证书算法库框架"
  - "证书对象的创建、解析和校验"
captured_at: "2026-04-17T01:35:49.531Z"
---
...
function certSample(): void {
  ...
  cert.createX509Cert(encodingBlob, (err, x509Cert) => {
    ...
    // 获取证书颁发者名称。
    let issuerName = x509Cert.getIssuerName(cert.EncodingType.ENCODING\_UTF8);
    console.info(\`X509 issuerName: ${issuerName}\`);

    // 获取证书主体名称。
    let subjectNameBin = x509Cert.getSubjectName(cert.EncodingType.ENCODING\_UTF8);
    ...
    // 获取证书对象的字符串类型数据。
    let certString = x509Cert.toString(cert.EncodingType.ENCODING\_UTF8);

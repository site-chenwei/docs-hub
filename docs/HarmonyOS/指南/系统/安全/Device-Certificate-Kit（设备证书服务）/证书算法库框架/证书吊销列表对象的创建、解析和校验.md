...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/create-parse-verify-crl-object"
menu_path:
  - "指南"
  - "系统"
  - "安全"
  ...
  - "证书算法库框架"
  - "证书吊销列表对象的创建、解析和校验"
captured_at: "2026-04-17T01:35:49.538Z"
---
...
function crlSample(): void {
  ...
  cert.createX509CRL(encodingBlob, (err, x509Crl) => {
    ...
    // 获取证书吊销列表颁发者名称。
    let issuerName = x509Crl.getIssuerName(cert.EncodingType.ENCODING\_UTF8);
    console.info(\`X509 CRL issuerName: ${issuerName}\`);

    // 获取证书吊销列表对象的字符串类型数据。
    let crlString = x509Crl.toString(cert.EncodingType.ENCODING\_UTF8);
    console.info(\`X509 CRL crlString: ${crlString}\`);

    // 公钥的二进制数据需要传入@ohos.security.cryptoFramework的convertKey()方法去获取公钥对象。
    try {
      let keyGenerator = cryptoFramework.createAsyKeyGenerator('RSA1024|PRIMES\_3');

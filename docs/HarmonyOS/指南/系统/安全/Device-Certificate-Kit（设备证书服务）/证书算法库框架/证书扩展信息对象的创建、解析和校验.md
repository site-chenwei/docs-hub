...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/create-parse-verify-certextension-object"
menu_path:
  - "指南"
  - "系统"
  - "安全"
  ...
  - "证书算法库框架"
  - "证书扩展信息对象的创建、解析和校验"
captured_at: "2026-04-17T01:35:49.501Z"
---
...
function certExtensionSample(): void {
  ...
  let encodingBlob: cert.EncodingBlob = {
    data: extData,
    // 证书扩展格式，目前仅支持DER格式。
    encodingFormat: cert.EncodingFormat.FORMAT\_DER
  };
  ...
  cert.createCertExtension(encodingBlob, (err, certExtension) => {
    ...
    try {
      ...
      let entry = certExtension.getEntry(cert.ExtensionEntryType.EXTENSION\_ENTRY\_TYPE\_ENTRY, oid);

---
title: "证书CMS验签"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/create-cms-verify-object"
menu_path:
  - "指南"
  - "系统"
  - "安全"
  ...
  - "证书算法库框架"
  - "证书CMS验签"
captured_at: "2026-04-17T01:35:49.633Z"
---
...
let ECC\_256\_PUB\_ENTRY\_CERT: string =
  '-----BEGIN CERTIFICATE-----\\n' +
...
let ECC\_256\_PUB\_INTER\_CERT: string =
  '-----BEGIN CERTIFICATE-----\\n' +
...
let ECC\_256\_PUB\_ROOT\_CERT: string =
  '-----BEGIN CERTIFICATE-----\\n' +
    ...
    '-----END CERTIFICATE-----';
let ECC\_256\_PRI\_ENTRY\_KEY: string =
...
async function testCmsVerifyTest() {
  try {
    ...
    let x509CertEntry: cert.X509Cert = await createX509Cert(ECC\_256\_PUB\_ENTRY\_CERT);
    let x509CertInter: cert.X509Cert = await createX509Cert(ECC\_256\_PUB\_INTER\_CERT);
    let x509CertRoot: cert.X509Cert = await createX509Cert(ECC\_256\_PUB\_ROOT\_CERT);
    let cms: cert.CmsGenerator = cert.createCmsGenerator(cert.CmsContentType.SIGNED\_DATA);
    let signerConfig: cert.CmsSignerConfig = {
      mdName: 'SHA256',
    };
    let keyInfo: cert.PrivateKeyInfo = {
      key: ECC\_256\_PRI\_ENTRY\_KEY
    };

...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-resourceleak-events-arkts"
menu_path:
  - "指南"
  - "系统"
  - "调测调优"
  ...
  - "资源泄漏事件"
  - "订阅资源泄漏事件（ArkTS）"
captured_at: "2026-04-17T01:36:01.628Z"
---
...
2.  添加 pss leak 相关内容：
    ...
    // 读 /proc/self/smaps\_rollup 中的 PSS 字段，统计当前进程的 PSS (单位 KB)
    static int GetCurrentProcessPss()
    {
        std::ifstream smapsFile("/proc/self/smaps\_rollup");
        if (!smapsFile.is\_open()) {
            std::cerr << "Failed to open /proc/self/smaps\_rollup" << std::endl;
            return 0;
        }
        std::string line;
        int totalPss = 0;
    ...
    {
        std::ifstream fdFile("/proc/self/fd\_num");
        if (!fdFile.is\_open()) {
            std::cerr << "Failed to open /proc/self/fd\_num" << std::endl;
            return 0;
        }
        std::string line;
        int totalPss = 0;
    ...
    {
        const size\_t maxSafe = 1073741824;
    ...
    // N-API 导出方法
    static napi\_value LeakMB(napi\_env env, napi\_callback\_info info)
    {
        size\_t argc = 1;
        napi\_value args\[1\];
        napi\_get\_cb\_info(env, info, &argc, args, nullptr, nullptr);
        if (argc < 1) {
            napi\_throw\_type\_error(env, nullptr, "Expected 1 argument");
            return nullptr;
        }
        double x = 0;
        if (napi\_get\_value\_double(env, args\[0\], &x) != napi\_ok) {
            napi\_throw\_type\_error(env, nullptr, "Argument must be a number");
            return nullptr;
        }
        const size\_t kilobyte = 1024;
        StartNativeLeak(static\_cast<size\_t>(x \* kilobyte));
        napi\_value rtn;
        napi\_get\_undefined(env, &rtn);
        return rtn;
    }
    ...
    static napi\_value Init(napi\_env env, napi\_value exports)
    {
        napi\_property\_descriptor desc\[\] = {
            // ...
            { "leakMB", nullptr, LeakMB, nullptr, nullptr, nullptr, napi\_default, nullptr}
        };
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c2/v3/_lEDjdfWQgiROJ-qEoeYbQ/caution_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013601Z&HW-CC-Expire=86400&HW-CC-Sign=C97D4C167EB9DA27B0113F4B5109D963C4A82C4A05F9EAE1451775DD35753D83)

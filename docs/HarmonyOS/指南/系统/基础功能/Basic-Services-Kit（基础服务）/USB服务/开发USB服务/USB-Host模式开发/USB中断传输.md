---
title: "USB中断传输"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/interrupttransfer"
menu_path:
  - "指南"
  - "系统"
  - "基础功能"
  ...
  - "USB Host模式开发"
  - "USB中断传输"
captured_at: "2026-04-17T01:35:54.975Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2b/v3/OkIkckvvS1-pdrEYLl3uww/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013557Z&HW-CC-Expire=86400&HW-CC-Sign=9C0B0F26C46DCBD685C2A7D36DA330BAB48351760B0D95655B5757616AA9E3B8)
    ...
    for (let i = 0; i < usbConfigs?.length; i++) {
      ...
      for (let j = 0; j < usbInterfaces?.length; j++) {
        ...
        usbEndpoint = usbEndpoints?.find((value) => {
          return value.direction === 128 && value.type === usbManager.UsbEndpointTransferType.TRANSFER\_TYPE\_INTERRUPT;
        })
...
6.  传输数据。
    ...
    try {
      ...
      transferParams = {
        ...
        type: usbManager.UsbEndpointTransferType.TRANSFER\_TYPE\_INTERRUPT,
        timeout: 2000,
        length: 10,
        callback: () => {
        },

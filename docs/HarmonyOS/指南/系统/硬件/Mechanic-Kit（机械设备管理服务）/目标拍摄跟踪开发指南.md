---
title: "目标拍摄跟踪开发指南"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/camera-tracking-guide"
menu_path:
  - "指南"
  - "系统"
  - "硬件"
  ...
  - "目标拍摄跟踪开发指南"
captured_at: "2026-04-17T01:35:59.434Z"
---
...
2.  获取已连接的机械体列表。
    ...
    //保存设备类型为GIMBAL\_DEVICE的设备的MechId
        if (device.mechDeviceType === mechanicManager.MechDeviceType.GIMBAL\_DEVICE) {
        savedMechanicIds.push(device.mechId);
        console.info(\`GIMBAL\_TYPE device saved ID: ${device.mechId}\`);
        } else {
...
2.  监听相机跟踪状态的变化。
    ...
    switch (eventInfo.event) {
        case mechanicManager.TrackingEvent.CAMERA\_TRACKING\_USER\_ENABLED:
        console.info('The user has enabled camera tracking');
        handleTrackingEnabled();
        break;
        case mechanicManager.TrackingEvent.CAMERA\_TRACKING\_USER\_DISABLED:
        console.info('The user has disabled camera tracking');
        handleTrackingDisabled();
        break;
        case mechanicManager.TrackingEvent.CAMERA\_TRACKING\_LAYOUT\_CHANGED:

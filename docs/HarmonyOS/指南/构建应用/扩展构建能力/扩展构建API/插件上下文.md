---
title: "插件上下文"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-build-expanding-context"
menu_path:
  - "指南"
  - "构建应用"
  - "扩展构建能力"
  - "扩展构建API"
  - "插件上下文"
captured_at: "2026-04-17T01:36:51.341Z"
---
...
hvigor.nodesEvaluated(() \=\> {
    const appContext \= hvigor.getRootNode().getContext(OhosPluginId.OHOS\_APP\_PLUGIN) as OhosAppContext;
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/0f/v3/ZB8wLW94RCqanN92kHWoTg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013652Z&HW-CC-Expire=86400&HW-CC-Sign=77CD244A80A199F0065567FA88667F79D0889C359597181845CB7AC290E981A2)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bb/v3/8hCTJJaOSTqQB0eigHeeiA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013652Z&HW-CC-Expire=86400&HW-CC-Sign=AF160B2AFEBA7D2F463F6F685634BD496C5E863AB22FA3C1A6A152F0BB20A767)
...
export function customPlugin(): HvigorPlugin {
    return {
        ...
        async apply(currentNode: HvigorNode): Promise<void\> {
            const rootNodeContext \= currentNode.getContext(OhosPluginId.OHOS\_APP\_PLUGIN);
            if (!rootNodeContext) {
                return;
            }
    ...
    currentNode.subNodes((node: HvigorNode) => {
        ...
        hapContext?.targets((target: Target) => {
            ...
            node.registerTask({
                ...
                // 任务执行逻辑主体函数
                run() {
                    const rootNodeContext = currentNode.getContext(OhosPluginId.OHOS\_APP\_PLUGIN) as OhosAppContext;
                    if (!rootNodeContext) {
                        return;
                    }
                    // 获取未签名的远程hsp相关信息
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ec/v3/VpbHDNyFQHCuCdGI0gtZeQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013652Z&HW-CC-Expire=86400&HW-CC-Sign=6365BA9DB8826885D25EE9FF999135E2A3F163E20443CD15D4D70AB44F79030F)
...
hvigor.nodesEvaluated(() \=\> {
    ...
    rootNode.subNodes((node: HvigorNode) \=\> {
        // 获得所有子节点
        const hapContext \= node.getContext(OhosPluginId.OHOS\_HAP\_PLUGIN) as OhosHapContext; // 仅对hap模块生效，hsp和har需要使用模块对应接口
        const moduleJsonOpt \= hapContext?.getModuleJsonOpt();
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ff/v3/Cs7Y3eDCR7uQyNZFPkkmZw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013652Z&HW-CC-Expire=86400&HW-CC-Sign=D55F27329DD173AF73B855D03AC27EEF2EE03824CF4C93D1ACDA895967CA7D4F)
    ...
    return {
        ...
        async apply(currentNode: HvigorNode): Promise<void\> {
            hvigor.nodesEvaluated(async () \=\> {
                currentNode.subNodes((node: HvigorNode) \=\> {
                    // 获取hap模块上下文信息
                    // 如果是HAR或HSP，使用OHOS\_HAR\_PLUGIN/OHOS\_HSP\_PLUGIN
                    const hapNodeContext \= node.getContext(OhosPluginId.OHOS\_HAP\_PLUGIN) as OhosHapContext;
                    if (!hapNodeContext) {
                        return;
                    }

...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file"
menu_path:
  - "指南"
  - "基础入门"
  - "开发基础知识"
  - "应用配置文件（Stage模型）"
  - "module.json5配置文件"
captured_at: "2026-04-17T01:35:30.926Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e1/v3/fQ0MMQbCT52o9DIgAp9V6w/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013531Z&HW-CC-Expire=86400&HW-CC-Sign=749D1BBDF410E6B707A0816679B86A40D2A18C06DE69624BDD903427893D899B)
...

{
  "module": {
    ...
    "pages": "$profile:main\_pages", // 资源配置，指向profile下面定义的配置文件main\_pages.json
    "appStartup": "$profile:app\_startup\_config",
    "metadata": \[
      {
        "name": "string",
        "value": "string",
        "resource": "$profile:distributionFilter\_config"
      },
      // ...
    \],
    "abilities": \[
      {
        ...
        "startWindow": "$profile:start\_window",
    ...
    "crossAppSharedConfig": "$profile:shared\_config",
    // ...
  }
}
...
标识当前Module的入口UIAbility名称，取值为长度不超过255字节的字符串。
...
\- false：当前HAP的.so文件会直接储存在libs目录中。
 ...
 | 布尔值 | 该标签可缺省，缺省值为false。 |
| [fileContextMenu](#filecontextmenu标签) | 
...
标识当前Module[启动框架](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-startup)配置路径，只允许entry类型模块配置。
...
标识应用间共享配置的配置文件名。取值为不超过255字节的字符串。用于发布配置给其他应用读取，在应用安装时生效，应用卸载时失效。详细使用方式见[共享配置使用](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/share-config#配置发布方)。
...
在[独立卡片包](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget-creation#方式二独立包方式创建卡片)中，应用包需要配置该标签，用来关联卡片包。取值为卡片包的模块名称，对应卡片包module.json5中的name标签。具体使用方式请参考[FormExtensionAbility配置](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget-configuration#formextensionability配置)。
...
在[独立卡片包](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget-creation#方式二独立包方式创建卡片)中，卡片包需要配置该标签，用来关联应用包。取值为应用包的模块名称，对应应用包module.json5中的name标签。具体使用方式请参考[独立卡片包配置](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget-configuration#独立卡片包配置)。
...
标识应用沙箱中分享目录的配置文件路径，用于为应用文件提供有安全保障的开放范围，保护应用资产。只允许entry类型模块配置，取值为长度不超过255字节的字符串。具体使用方式请参考[应用共享目录配置](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/share-app-file-configuration)。
...
从API version 23开始，支持该标签。
 ...
 | 字符串 | 该标签可缺省，缺省值为空。 |
| [requiredDeviceFeatures](#requireddevicefeatures标签) | 
  ...
  "module": {
    ...
    "metadata": \[
      // ...
      {
        ...
        "resource": "$profile:main\_pages" // 资源配置，指向profile下面定义的配置文件main\_pages.json
      }
    \],
    // ...
  }
...
| [launchType](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-launch-type) | 
...
\- false：只能由同应用或者具有ohos.permission.START\_INVISIBLE\_ABILITY权限（该权限仅系统应用支持申请）的应用拉起。
...
标识当前UIAbility组件能够接收的[Want](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/want-overview)特征集，为数组格式。
...
标识应用是否允许通过[App Linking](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-linking-startup)跳转自己。
...
\- follow\_recent：跟随背景窗口的旋转模式。

\- landscape\_inverted：反向横屏。

\- portrait\_inverted：反向竖屏。

\- auto\_rotation：随传感器旋转。

\- auto\_rotation\_landscape：传感器横屏旋转，包括横屏和反向横屏。

\- auto\_rotation\_portrait：传感器竖屏旋转，包括竖屏和反向竖屏。

\- auto\_rotation\_restricted：传感器开关打开，方向可随传感器旋转。

\- auto\_rotation\_landscape\_restricted：传感器开关打开，方向可随传感器旋转为横屏，包括横屏和反向横屏。

\- auto\_rotation\_portrait\_restricted：传感器开关打开，方向可随传感器旋转为竖屏，包括竖屏和反向竖屏。

\- locked：传感器开关关闭，方向锁定。

\- auto\_rotation\_unspecified：受开关控制和由系统判定的自动旋转模式。

\- follow\_desktop：跟随桌面的旋转模式。
...
\- landscape\_auto：多窗布局动态可变为横向，需要配合API enableLandScapeMultiWindow/disableLandScapeMultiWindow使用，建议视频类应用配置。
...
标识组件的进程名称。具体使用方式参考[进程模型定义](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/process-model-stage#其他进程类型)中的"静态指定进程"。
    ...
    "abilities": \[
      {
        ...
        "description": "$string:description\_main\_ability",
        "icon": "$media:layered\_image",
        "label": "$string:EntryAbility\_label",
        "permissions": \[\],
        "metadata": \[\],
...
标识是否开启[域名校验](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-linking-startup#section4452103365213)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/32/v3/xIaZHfYlRuGCt2SF_Skzcg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013531Z&HW-CC-Expire=86400&HW-CC-Sign=2F12B39E1BD31F216A016D015037BFBEF1A685A77B4FA5FEACCBF4B26EAAA99A)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a8/v3/DrJHwJ_iQcalasmjyqJhDA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013531Z&HW-CC-Expire=86400&HW-CC-Sign=B5F914166AD48D780C953BDE419744C653480858AED84890156AEEBE1521ACC9)
...
标识当前ExtensionAbility组件能够接收的[Want](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/want-overview)的特征集。
...
\- runWithMainProcess：表示该ExtensionAbility和应用主进程共进程，只有[状态栏开放服务](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/statusbar-extension-introduction)的ExtensionAbility可以配置runWithMainProcess。
...
标识组件的进程名称，只有type为embeddedUI时可以配置该标签。具体使用方式参考[进程模型定义](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/process-model-stage#其他进程类型)中的"静态指定进程"。
    ...
    "extensionAbilities": \[
      {
        ...
        "icon": "$media:icon",
        "label" : "$string:extension\_name",
        "description": "$string:form\_description",
        "type": "form",
        "permissions": \["ohos.permission.ACCESS\_BLUETOOTH"\],
        "exported": true,
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d3/v3/BybipIbwSLCmfnnwkPqWyA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013531Z&HW-CC-Expire=86400&HW-CC-Sign=D1CD08B0252684CECEFA6B8AC9F8812390287C758DF53963ED219EC2D5138F47)
            ...
            "metadata": \[
              {
                ...
                "resource": "$profile:shortcuts\_config"
              }
            \],
            // ...
          }
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d6/v3/u7ygVhKdRxWVGyAeTidaiw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013531Z&HW-CC-Expire=86400&HW-CC-Sign=BB90CB7B43391E1989959979C7F3FFC94BA5B8BFBABE06046DD901A7E6877DFB)
    ...
    "proxyData": \[
      {
        ...
        "requiredReadPermission": "ohos.permission.SYSTEM\_FLOAT\_WINDOW",
        "requiredWritePermission": "ohos.permission.SYSTEM\_FLOAT\_WINDOW",
        "metadata": {
          "name": "datashare\_metadata",
...
\- "REQUIRED\_SHOW"：强制显示启动页。不受[Ability管理服务（即StartOptions中hideStartWindow标签）](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions#startoptions)的影响。

\- "REQUIRED\_HIDE"：强制隐藏启动页。不受[Ability管理服务（即StartOptions中hideStartWindow标签）](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions#startoptions)的影响。

\- "OPTIONAL\_SHOW"：可选显示，默认行为为显示启动页，如果[Ability管理服务（即StartOptions中hideStartWindow标签）](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions#startoptions)设置隐藏启动页，则隐藏启动页。

\- 如未配置该标签，默认取值为"REQUIRED\_SHOW"，即强制显示启动页。

从API version 20开始支持该标签。

 | 字符串 | 可缺省，缺省值为REQUIRED\_SHOW。 |
| startWindowAppIcon | 
...
\- "FOLLOW\_SYSTEM"：启动页颜色模式跟随系统深浅色。

\- "FOLLOW\_APPLICATION"：启动页颜色模式跟随应用深浅色。

\- 如未配置该字段，默认取值为"FOLLOW\_SYSTEM"，即启动页颜色模式跟随系统深浅色。

从API version 20开始支持该字段。

 | 字符串 | 可缺省，缺省值为FOLLOW\_SYSTEM。 |
...

{
  "module": {
    // ...
    "systemTheme": "$profile:theme\_config", // 资源配置，指向profile下面定义的配置文件theme\_config.json
  }
}
...
\- $ohos:theme:ohos\_theme 系统默认的主题
...
\- large\_screen：表示设备需要支持[大屏横屏](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-device-screen-layout#section6493354468)。

...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/image-allocator-type"
menu_path:
  - "指南"
  - "媒体"
  - "Image Kit（图片处理服务）"
  ...
  - "图片解码"
  - "图片解码内存优化(ArkTS)"
captured_at: "2026-04-17T01:36:05.837Z"
---
...
2.  从ImageInfo对象中访问stride值：info.stride。
    ...
    async CreatePixelMapUsingAllocator(context: Context, type: image.AllocatorType): Promise<image.PixelMap | undefined> {
      ...
      try {
        const rawFile = await resourceMgr.getRawFileContent('99\_132.jpg'); // 测试图片为99\*132的jpg图。
        ...
        if (pixelmap != undefined) {
          ...
          // 用DMA\_ALLOC内存申请出的pixelmap的stride与SHARE\_MEMORY内存申请出的pixelmap的stride不同。

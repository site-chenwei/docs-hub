---
title: "标准化数据结构 (ArkTS)"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uniform-data-structure"
menu_path:
  - "指南"
  - "应用框架"
  - "ArkData（方舟数据管理）"
  - "标准化数据定义"
  - "标准化数据结构 (ArkTS)"
captured_at: "2026-04-17T01:35:33.347Z"
---
...
7.  遍历每条记录，判断该记录的数据类型，转换为子类对象并得到原数据记录。
    ...
    // 访问对象属性。
    hilog.info(0xFF00, '\[Sample\_Udmf\]', \`hyperlink.url = ${hyperlink.url}\`);
    ...
    let plainTextRecord =
      new unifiedDataChannel.UnifiedRecord(uniformTypeDescriptor.UniformDataType.PLAIN\_TEXT, plainText);
        ...
        switch (type) {
          case uniformTypeDescriptor.UniformDataType.HYPERLINK:
            Object.keys(record).forEach(key => {
              hilog.info(0xFF00, '\[Sample\_Udmf\]', \`show records: ${key} + , value: ${record\[key\]}\`);
            });
            break;

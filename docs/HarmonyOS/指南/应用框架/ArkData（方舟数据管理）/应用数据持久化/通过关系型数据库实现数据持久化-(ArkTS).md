...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/data-persistence-by-rdb-store"
menu_path:
  - "指南"
  - "应用框架"
  - "ArkData（方舟数据管理）"
...
captured_at: "2026-04-17T01:35:33.526Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2c/v3/8HFIuZJpSEq8Q3q6ZOmefg/zh-cn_image_0000002568898145.jpg?HW-CC-KV=V1&HW-CC-Date=20260417T013534Z&HW-CC-Expire=86400&HW-CC-Sign=E218C0F6A625D8F97B26BA95171DF008B197A9163B9BBBA931EE3B428590CA61)
...
1.  使用关系型数据库实现数据持久化，需要获取一个RdbStore，其中包括建库、建表、升降级等操作。推荐使用事务接口保证数据库升级流程原子性。
    ...
    let tokenType = relationalStore.Tokenizer.ICU\_TOKENIZER;
    let tokenTypeSupported = relationalStore.isTokenizerSupported(tokenType);
    if (!tokenTypeSupported) {
      tokenType = relationalStore.Tokenizer.NONE\_TOKENIZER;
      hilog.error(DOMAIN, 'rdbDataPersistence', \`ICU\_TOKENIZER is not supported on this platform.\`);
    }
    ...
    // ...
      ...
      if (store !== undefined) {
        ...
        let storeVersion = await transaction.execute('PRAGMA user\_version');
        // 当数据库创建时，数据库默认版本为0
        // 示例应用升级流程较短，所以使用单个事务。如果实际业务中升级逻辑较多，建议拆分多个独立事务串行执行。
        if (storeVersion === 0) {
          try {
            await transaction.execute(SQL\_CREATE\_TABLE); // 创建数据表，以便后续调用insert接口插入数据
            storeVersion = 1;
        ...
        if (storeVersion === 2) {
          ...
          try {
            ...
            storeVersion = 3;
            await transaction.execute('PRAGMA user\_version = 3');
    ...
    ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fb/v3/f403wpjTQsi_2NK5wrkDlA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013534Z&HW-CC-Expire=86400&HW-CC-Sign=69C4A1429803D23F2D2E2F8810A350283EF7D622E7CE1C6D3C3121BCA2BC1465)
...
2.  获取到RdbStore，完成数据表创建后，调用insert()接口插入数据。示例代码如下所示：
    ...
    ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/61/v3/oXjCcQa_Re20_k52KSULgA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013534Z&HW-CC-Expire=86400&HW-CC-Sign=3EE0031279A40096E34F78027A620B66FA69EB0A1F5F57D76202034DB12C65DE)
...
4.  根据谓词指定的查询条件查找数据。
    ...
    ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3f/v3/zP5Z3_maRIatu3HjgJGVow/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013534Z&HW-CC-Expire=86400&HW-CC-Sign=FD0001E39773441B0B5E972930B3A4F8A083FE8ECFB368BC7812C6D7A9E1FD8B)
    ...
    if (store !== undefined && tokenTypeSupported) {
      // 创建全文检索表
      const SQL\_CREATE\_TABLE = 'CREATE VIRTUAL TABLE IF NOT EXISTS example USING fts4(name, content, tokenize=icu zh\_CN)';
      try {
        await store.execute(SQL\_CREATE\_TABLE);
      ...
      try {
        ...
        try {
          ...
          const rowId = await transaction.insert(
            ...
            relationalStore.ConflictResolution.ON\_CONFLICT\_REPLACE
          );

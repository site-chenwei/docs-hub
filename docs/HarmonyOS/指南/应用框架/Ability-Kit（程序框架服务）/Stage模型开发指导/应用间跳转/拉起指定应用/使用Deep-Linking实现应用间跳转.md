...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/deep-linking-startup"
menu_path:
  - "指南"
  - "应用框架"
  - "Ability Kit（程序框架服务）"
...
captured_at: "2026-04-17T01:35:31.975Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6d/v3/XSTTEshkQjSq9Qk7NqVoMw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013532Z&HW-CC-Expire=86400&HW-CC-Sign=68B61D6CC43B8DD4D87C52CCBB353ABABDE01DA1D9623742678F2479D15F157A)
      ...
      .onClick(() => {
        ...
        try {
          context.openLink(link, openLinkOptions)
            .then(() => {
              hilog.info(DOMAIN\_NUMBER, TAG, 'openLink success.');
            }).catch((err: BusinessError) => {
            hilog.error(DOMAIN\_NUMBER, TAG, \`openLink failed. Code is ${err.code}, message is ${err.message}\`);
          });
        } catch (paramError) {
          hilog.error(DOMAIN\_NUMBER, TAG, \`Failed to start link. Code is ${paramError.code}, message is ${paramError.message}\`);
        }
      })
  }
}
      ...
      .onClick(() => {
        ...
        try {
          context.startAbility(want).then(() => {
            ...
            hilog.error(DOMAIN\_NUMBER, TAG, \`startAbility failed. Code is ${err.code}, message is ${err.message}\`);
          });
        } catch (paramError) {
          hilog.error(DOMAIN\_NUMBER, TAG, \`Failed to start ability. Code is ${paramError.code}, message is ${paramError.message}\`);
        }
      })
  }
}
...
const DOMAIN\_NUMBER = 0xF811;
const TAG = '\[Sample\_PullLinking\]';
        ...
        .onLoadIntercept((event) => {
          ...
          if (url === 'link://www.example.com') {
            (this.getUIContext().getHostContext() as common.UIAbilityContext).openLink(url)
              ...
              hilog.error(DOMAIN\_NUMBER, TAG, \`openLink failed, err: ${JSON.stringify(err)}.\`);
            });
            return true;
          }

...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-reusable"
menu_path:
  - "指南"
  - "应用框架"
  - "ArkUI（方舟UI框架）"
  ...
  - "自定义组件复用"
  - "@Reusable装饰器：V1组件复用"
captured_at: "2026-04-17T01:35:35.868Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a7/v3/oPeDx4EUR1OYCPOiE-FeMQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013536Z&HW-CC-Expire=86400&HW-CC-Sign=54FF028E0C238FFB4017AD362EC30D2D2CCC41CC5F6DD57CFA9D9ED97E96F5AB)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ff/v3/tyvR4au4SD2xGtCKljTBWQ/caution_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013536Z&HW-CC-Expire=86400&HW-CC-Sign=85EA41380BF026E32FBC4FD1A1B2C9C8C86988E19ECBE301EC3A50F1F4E9743B)
...
const TAG = '\[Sample\_ReusableComponent\]';
const DOMAIN = 0xF811;
const BUNDLE = 'ReusableComponent\_';
...
struct Index {
  ...
  aboutToAppear(): void {
    for (let i = 0; i < 20; i++) { // 循环20次
      let title = i + 1 + 'test\_if';
      // 请开发者自行在src/main/resources/base/media路径下添加app.media.app\_icon图片，否则运行时会因资源缺失而Image空白。
      this.dataSource.pushData(new FriendMoment(i.toString(), title, 'app.media.app\_icon'));
    }
...
struct ReusableChildComponent {
  ...
  build() {
    Column() {
      // 请开发者自行在src/main/resources/base/media路径下添加app.media.app\_icon图片，否则运行时会因资源缺失而报错。
    ...
    struct Index {
      ...
      aboutToAppear(): void {
        for (let i = 0; i < 1000; i++) { // 循环1000次
          let title = i + 1 + 'test\_swiper';
          let answers = \['test1', 'test2', 'test3', 'test4'\];
          // 请开发者自行在src/main/resources/base/media路径下添加app.media.app\_icon图片，否则运行时会因资源缺失而报错。
          this.dataSource.pushData(new Question(i.toString(), title, $r('app.media.app\_icon'), answers));
        }
      }
      ...
      build() {
        Column() {
          Text(this.itemData?.title)
            .fontSize(18)
            .fontColor($r('sys.color.ohos\_id\_color\_primary'))

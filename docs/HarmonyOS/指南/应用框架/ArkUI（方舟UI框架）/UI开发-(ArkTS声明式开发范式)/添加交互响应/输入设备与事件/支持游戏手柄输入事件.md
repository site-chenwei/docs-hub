---
title: "支持游戏手柄输入事件"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-interaction-development-guide-gamepad"
menu_path:
  - "指南"
  - "应用框架"
  - "ArkUI（方舟UI框架）"
  ...
  - "输入设备与事件"
  - "支持游戏手柄输入事件"
captured_at: "2026-04-17T01:35:38.562Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/48/v3/qwkczwDbTHyhexupX57mLw/zh-cn_image_0000002538178804.png?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=0258A391B18E558D979DC33AC760380D5C42ED9BCB0D2F20982E39A7DECD3E49)
          ...
          .onKeyEvent((event: KeyEvent) => {
            if (event && event.type === KeyType.Down) {
              switch (event.keyCode) {
                case KeyCode.KEYCODE\_BUTTON\_SELECT:
                  console.info('trigger BUTTON\_SELECT');
                  // 处理BUTTON\_SELECT按键的输入逻辑
                  break;
                case KeyCode.KEYCODE\_BUTTON\_START:
                  console.info('trigger BUTTON\_START');
                  // 处理BUTTON\_START按键的输入逻辑
                  break;
                default:
                  console.info('trigger BUTTON\_DEFAULT');
                  // 处理其他按键的输入逻辑
                  break;
              }
            }
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/84/v3/o73O4uTSRqySM2iXOHvNUw/zh-cn_image_0000002569018593.png?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=6F7CBFA54CAFA18DB50D0AA207B630645A803D19542D973D28EFA1280EB989AF)
        ...
        Column() {
          Button('JoyStick')
            ...
            .onKeyEvent((event: KeyEvent) => {
              if (event && event.type === KeyType.Down) {
                switch (event.keyCode) {
                  case KeyCode.KEYCODE\_DPAD\_UP:
                  case KeyCode.KEYCODE\_DPAD\_DOWN:
                  case KeyCode.KEYCODE\_DPAD\_LEFT:
                  case KeyCode.KEYCODE\_DPAD\_RIGHT:
    ...
    Column() {
      Button('JoyStick')
        ...
        .onFocusAxisEvent((event: FocusAxisEvent) => {
          let absX = event.axisMap.get(AxisModel.ABS\_X);
          let absY = event.axisMap.get(AxisModel.ABS\_Y);
          let absZ = event.axisMap.get(AxisModel.ABS\_Z);
          let absRZ = event.axisMap.get(AxisModel.ABS\_RZ);
          let absGas = event.axisMap.get(AxisModel.ABS\_GAS);
          let absBrake = event.axisMap.get(AxisModel.ABS\_BRAKE);
          // 处理操纵杆输入逻辑
      ...
      Column() {
        Button('JoyStick')
          ...
          .onFocusAxisEvent((event: FocusAxisEvent) => {
            ...
            let absGas = event.axisMap.get(AxisModel.ABS\_GAS);
            let absBrake = event.axisMap.get(AxisModel.ABS\_BRAKE);
            let absHat0X = event.axisMap.get(AxisModel.ABS\_HAT0X);
            let absHat0Y = event.axisMap.get(AxisModel.ABS\_HAT0Y);
            this.axisValue =
              'absX: ' + absX + '\\nabsY: ' + absY + '\\nabsZ: ' + absZ + '\\nabsRz: ' + absRz + '\\nabsGas: ' + absGas +
                '\\nabsBrake: ' + absBrake + '\\nabsHat0X: ' + absHat0X + '\\nabsHat0Y: ' + absHat0Y;
          })
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/21/v3/xTr8SpJ3S_iu9wkQq2PpEA/zh-cn_image_0000002568898583.gif?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=C5AFC388210F09FE9E4EC447A28002764F07FE6A72AB4BC0111664E917CA54F2)

---
title: "多层级手势事件"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-gesture-events-multi-level-gesture"
menu_path:
  - "指南"
  - "应用框架"
  - "ArkUI（方舟UI框架）"
  ...
  - "添加手势响应"
  - "多层级手势事件"
captured_at: "2026-04-17T01:35:38.617Z"
---
...
Stack() {
  ...
  .onTouch(() => {})
  .gesture(TapGesture({count: 1}))
  .hitTestBehavior(HitTestMode.BLOCK\_HIERARCHY)
}
.id('Stack A')
.onTouch(() => {})
.gesture(TapGesture({count: 1}))
...
Stack() {
  ...
  .onTouch(() => {})
  .gesture(TapGesture({count: 1}))
  .hitTestBehavior(HitTestMode.BLOCK\_DESCENDANTS)
}
.id('Stack A')
.onTouch(() => {})
.gesture(TapGesture({count: 1}))

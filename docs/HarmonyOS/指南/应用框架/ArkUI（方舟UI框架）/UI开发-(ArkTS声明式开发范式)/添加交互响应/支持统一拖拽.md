---
title: "支持统一拖拽"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-common-events-drag-event"
menu_path:
  - "指南"
  - "应用框架"
  - "ArkUI（方舟UI框架）"
  ...
  - "添加交互响应"
  - "支持统一拖拽"
captured_at: "2026-04-17T01:35:38.782Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/52/v3/_83aiY7_Qru9MrzshPNOPw/zh-cn_image_0000002568898593.png?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=6D77F08C7A13ECF783DECF67CCDD6F9725FD96F772931BE6F698C9B5142457A6)
...
1\. 设置DragResult.DROP\_ENABLED，组件允许落入。

2\. 设置DragResult.DROP\_DISABLED，组件不允许落入。
...
当用户在组件范围内释放拖拽操作时，此回调会被触发。开发者需在此回调中通过DragEvent的setResult方法来设置拖拽结果，否则在拖出方组件的onDragEnd方法中，通过getResult方法获取的将只是默认的处理结果DragResult.DRAG\_FAILED。

此回调是开发者干预系统默认拖入处理行为的关键点，系统会优先执行开发者定义的onDrop回调。通过在onDrop回调中调用setResult方法，开发者可以告知系统如何处理被拖拽的数据。

1\. 设置 DragResult.DRAG\_SUCCESSFUL，数据完全由开发者自己处理，系统不进行处理。

2\. 设置DragResult.DRAG\_FAILED，数据不再由系统继续处理。

3\. 设置DragResult.DRAG\_CANCELED，系统也不需要进行数据处理。

4\. 设置DragResult.DROP\_ENABLED或DragResult.DROP\_DISABLED会被忽略，等同于设置DragResult.DRAG\_SUCCESSFUL。
...
| [onPreDrag](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#onpredrag12) | 
...
1\. ACTION\_DETECTING\_STATUS：拖拽手势启动阶段。按下50ms时触发。

2\. READY\_TO\_TRIGGER\_DRAG\_ACTION：拖拽准备完成，可发起拖拽阶段。按下500ms时触发。

3\. PREVIEW\_LIFT\_STARTED：拖拽浮起动效发起阶段。按下800ms时触发。

4\. PREVIEW\_LIFT\_FINISHED：拖拽浮起动效结束阶段。浮起动效完全结束时触发。

5\. PREVIEW\_LANDING\_STARTED：拖拽落回动效发起阶段。落回动效发起时触发。

6\. PREVIEW\_LANDING\_FINISHED：拖拽落回动效结束阶段。落回动效结束时触发。

7\. ACTION\_CANCELED\_BEFORE\_DRAG：拖拽浮起落位动效中断。已满足READY\_TO\_TRIGGER\_DRAG\_ACTION状态后，未达到动效阶段，手指抬起时触发。

8\. PREPARING\_FOR\_DRAG\_DETECTION18+：拖拽准备完成，可发起拖拽阶段。按下350ms时触发。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/31/v3/A28vfNv1R7avHM0E00M-lw/zh-cn_image_0000002538018888.png?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=FF3A8CD26982509CDFEF94773A3139334ACA827906F846EB106FAECD06B00CA7)
...
1.  组件使能拖拽。
    ...
    Image($r('app.media.app\_icon'))
      ...
      .onDragStart((event) => {
        ...
        // 'resources/base/media/app\_icon.png'需要替换为开发者所需的图像资源文件
        data.imageUri = 'resources/base/media/app\_icon.png';
...
4.  拖拽过程显示角标样式。
    ...
    .onDragMove((event) => {
      event.setResult(DragResult.DROP\_ENABLED)
...
6.  拖拽发起方可以通过设置onDragEnd回调感知拖拽结果。
    ...
    .onDragEnd((event) => {
      ...
      if (event.getResult() === DragResult.DRAG\_SUCCESSFUL) {
        this.getUIContext().getPromptAction().showToast({ duration: 100, message: 'Drag Success' });
      } else if (event.getResult() === DragResult.DRAG\_FAILED) {
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a9/v3/uIv46mUFTJ-_jyX7Nhoohg/zh-cn_image_0000002538178816.gif?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=D5E9495BCB9A98F3C1C9F4020F5BA919F2864912BDA47924DE94F3F874B204C4)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7e/v3/0neoFzp3SXyT7gqkswNHZQ/zh-cn_image_0000002569018605.gif?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=674199826941D39E5B3DD3470C7F8EB79D96AC3E8C8DC2C6959995D46CBDAA6B)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/39/v3/XP4k-ulXSxiW1FWhf9hy8Q/zh-cn_image_0000002568898595.gif?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=6A7452870C4733F15193D6C48AACECA41CDBE4300064D705101CE58C03C6DF5C)
...
3.  拖拽数据提前准备。
    ...
    .onPreDrag((status: PreDragStatus) => {
      if (status == PreDragStatus.PREPARING\_FOR\_DRAG\_DETECTION) {
        this.loadData();
      }
    })
...
struct GridEts {
  ...
  build() {
    Column({ space: 5 }) {
      // 请将$r('app.string.Select\_All')替换为实际资源文件，在本示例中该资源文件的value值为"全选"
      Button($r('app.string.Select\_All'))
        .onClick(() => {
          for (let i = 0; i < this.isSelectedGrid.length; i++) {
        ...
        ForEach(this.numbers, (idx: number) => {
          ...
          .onPreDrag((status: PreDragStatus) => {
            ...
            if (status == PreDragStatus.PREPARING\_FOR\_DRAG\_DETECTION) {
              ...
              this.loadData();
            } else if (status == PreDragStatus.ACTION\_CANCELED\_BEFORE\_DRAG) {
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/09/v3/j4jsDH5kSAemA8DaqF3DKw/zh-cn_image_0000002538018890.gif?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=A62EEB416F4D8CA1F77E9C08090051510E2EFA7C9F72BF890799130AB5F8141B)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a3/v3/DLei63QqRgOfwWxmYXERdQ/zh-cn_image_0000002538178818.png?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=464DCA7A21B4AC9C3997628AA0ABC533B99DAB8756D0DEBC146CBB6876231A74)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4e/v3/AMdM-VZITl6aMKJPeQ2KLA/zh-cn_image_0000002569018607.png?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=BF4EEF188C3FEBC3753239457F3277D801DA72071714ABE5EBAF1D0174771FCA)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/91/v3/tZE-KQtAQNuON22aBjEgEA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=D3D8FF4994C6BC043E0FA963B26F89A0FCD2E373968464BB8EC6957032484D39)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/56/v3/WXQMzrj6SGKTTh8BV1bndw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=13E228341FB6D5B306945F17023E265143FB0A2EACB2175496064F4A71220213)
...
build() {
  Column() {
    // ...
      Column() {
        // 请将$r('app.string.DoubleClick\_Text')替换为实际资源文件，在本示例中该资源文件的value值为"双击文字选择后拖出: \\n     DeviceName"
        Text($r('app.string.DoubleClick\_Text'))
          .fontSize(30)
          .copyOption(CopyOptions.InApp) // 开启copyOption之后，文本组件即可支持选择内容进行拖拽
      }.padding({ bottom: 30 })

      // 请将$r('app.string.Search\_Device')替换为实际资源文件，在本示例中该资源文件的value值为"搜索设备"
      Button($r('app.string.Search\_Device'))
        .width('80%')
        .height('80vp')
        .fontSize(30)
        .bindSheet($$this.isShowSheet, this.SheetBuilder(), {
          ...
          // 请将$r('app.string.Search\_Device')替换为实际资源文件，在本示例中该资源文件的value值为"搜索设备"
          title: { title: $r('app.string.Search\_Device') },
        })
        // ...
...
SheetBuilder() {
  Column() {
    // 输入框
    // 请将$r('app.string.Push\_Here')替换为实际资源文件，在本示例中该资源文件的value值为"拖入此处"
    TextInput({ placeholder: $r('app.string.Push\_Here') })
      .width('80%')
      .borderWidth(1)
...
export struct SpringLoadingPage {
  ...
  // 请将$r('app.string.Select\_Result')替换为实际资源文件，在本示例中该资源文件的value值为"搜索结果：\\n  设备 1\\n  设备 2\\n  设备 3\\n  ... ..."
  private searchResult: string = this.context1?.resourceManager.getStringSync($r('app.string.Select\_Result').id)!;
    ...
    Column() {
      // ...
        ...
        Button($r('app.string.Search\_Device'))
          ...
          .allowDrop(\[uniformTypeDescriptor.UniformDataType.PLAIN\_TEXT\])
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5f/v3/Qeia6JmoTE669jqsPLs3Jg/zh-cn_image_0000002568898597.gif?HW-CC-KV=V1&HW-CC-Date=20260417T013539Z&HW-CC-Expire=86400&HW-CC-Sign=068378ED65E173C6E8E2A5A3EF5F6D8EEB76C1AFF87B70348713F76ACCC5B6C0)

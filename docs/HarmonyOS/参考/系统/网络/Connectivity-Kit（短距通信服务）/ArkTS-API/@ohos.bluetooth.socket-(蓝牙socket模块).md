...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-bluetooth-socket"
menu_path:
  - "参考"
  - "系统"
  - "网络"
...
captured_at: "2026-04-17T01:48:21.353Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/49/v3/pFxI7bA0S-ygW-xON8obaw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T014822Z&HW-CC-Expire=86400&HW-CC-Sign=61ACD6E60B2F94584CF4FB79C44C9FA93642EDCF8332CEBBD3067D5B68801A9D)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e2/v3/YmEvtsmjR-q-DtRLSsZB9Q/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T014822Z&HW-CC-Expire=86400&HW-CC-Sign=FF25E5457083600BE511638EE16D8FA6899BA1736DB591E60CF274603F683B5F)
...
该值是调用[socket.sppListen](#socketspplisten)接口后，通过其异步callback获取到的。
...
\- [SppType](#spptype)设置为SPP\_L2CAP\_BLE时，返回值的有效值范围为\[0x01, 0xFF\]。

\- [SppType](#spptype)设置为SPP\_L2CAP时，返回值的有效值范围为\[0x0000, 0xFFFF\]。

\- 服务端通道建立异常或[SppType](#spptype)非L2CAP链路类型时，返回-1。
...
该值是调用[socket.sppAccept](#socketsppaccept)或[socket.sppConnect](#socketsppconnect)接口后，通过其异步callback获取到的。
...
该值是调用[socket.sppListen](#socketspplisten)接口，通过其异步callback获取到的。
...
该值是调用[socket.sppAccept](#socketsppaccept)或[socket.sppConnect](#socketsppconnect)接口，通过其异步callback获取到的。
...
\- 建议开发者使用自定义的服务UUID（可通过工具函数[util.generateRandomUUID](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-util#utilgeneraterandomuuid9)生成），也可以使用标准协议定义的Serial Port UUID服务(00001101-0000-1000-8000-00805F9B34FB)。

\- SppType设置为SPP\_RFCOMM时该参数必选。

\- SppType设置为SPP\_L2CAP或SPP\_L2CAP\_BLE时设置为空字符串。
...
\- SppType设置为SPP\_RFCOMM时，该参数不填。

\- SppType设置为SPP\_L2CAP\_BLE或SPP\_L2CAP时，需和服务端的psm值保持一致。

对于服务端：

\- SppType设置为SPP\_RFCOMM时，该参数不填。

\- SppType设置为SPP\_L2CAP\_BLE时，psm值必须由系统自动分配，有效值范围为\[0x01, 0xFF\]。

\- SppType设置为SPP\_L2CAP时，psm值可以主动设置或蓝牙子系统分配，若为主动设置，其有效范围为\[0x00, 0xFFFF\]，并且需要满足低位字节最低位必须为1，高位字节最低位必须为0；若为蓝牙子系统分配，该参数不填，可以通过[socket.getL2capPsm](#socketgetl2cappsm20)接口获取psm值。

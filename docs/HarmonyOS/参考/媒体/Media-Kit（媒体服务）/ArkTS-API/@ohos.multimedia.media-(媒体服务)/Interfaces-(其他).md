---
title: "Interfaces (其他)"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-i"
menu_path:
  - "参考"
  - "媒体"
  - "Media Kit（媒体服务）"
  ...
  - "Interfaces (其他)"
captured_at: "2026-04-17T01:48:43.265Z"
---

# Interfaces (其他)

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/57/v3/iDxZd7XtRwimGDH127t4pg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T014845Z&HW-CC-Expire=86400&HW-CC-Sign=4A1EECE96E845F67B6A187A965209A9D29EC8852A4A8214209AD73B9D5B83FF4)
...
| callback | (buffer: ArrayBuffer, length: number, pos?: number) => number | 否 | 否 | 
用户设置的回调函数，用于填写数据。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| :-- | :-- | :-- | :-- | :-- |
| \[key: string\] | Object | 否 | 是 | 
该键值对支持的key取值范围，请参考[PlaybackInfoKey](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#playbackinfokey12)。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| :-- | :-- | :-- | :-- | :-- |
| audioSourceType | [AudioSourceType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#audiosourcetype9) | 否 | 是 | 
...
| profile | [AVRecorderProfile](#avrecorderprofile9) | 否 | 否 | 
...
录制输出URL：fd://xx (fd number) ![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5a/v3/qubeWyd5S0Oyx9FtFzQy3A/zh-cn_image_0000002569021519.png?HW-CC-KV=V1&HW-CC-Date=20260417T014845Z&HW-CC-Expire=86400&HW-CC-Sign=3A5D901FAFC10F1B631DF60D45146B87D93FE641A5D056289093AC9939366B49)，必要参数。
...
| rotation(deprecated) | number | 否 | 是 | 
...
如果同时设置两个值，将会采用[AVMetadata](#avmetadata11).videoOrientation。
 ...
 |
| location(deprecated) | [Location](#location) | 否 | 是 | 
...
\- AMR\_NB编码格式支持比特率范围\[4750, 5150, 5900, 6700, 7400, 7950, 10200, 12200\]。

\- AMR\_WB编码格式支持比特率范围\[6600, 8850, 12650, 14250, 15850, 18250, 19850, 23050, 23850\]。

**元服务API：** 从API version 12 开始，该接口支持在元服务中使用。

 |
| audioChannels | number | 否 | 是 | 
...
**元服务API：** 从API version 12 开始，该接口支持在元服务中使用。
 ...
 |
| audioCodec | [CodecMimeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#codecmimetype8) | 否 | 是 | 

音频编码格式，选择音频录制时必填。

当前支持AUDIO\_AAC、AUDIO\_MP3、AUDIO\_G711MU、AUDIO\_AMR\_NB和AUDIO\_AMR\_WB。

**元服务API：** 从API version 12 开始，该接口支持在元服务中使用。

 |
| aacProfile22+ | [AacProfile](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#aacprofile22) | 否 | 是 | 

音频编码扩展格式，默认为AAC\_LC格式。

当前支持类型：AAC\_LC、AAC\_HE和AAC\_HE\_V2。

**元服务API：** 从API version 22 开始，该接口支持在元服务中使用。

 |
| audioSampleRate | number | 否 | 是 | 
...
\- AMR\_NB编码支持采样率大小：8000。

\- AMR\_WB编码支持采样率大小：16000。
...
**元服务API：** 从API version 12 开始，该接口支持在元服务中使用。
 ...
 |
| fileFormat | [ContainerFormatType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#containerformattype8) | 否 | 否 | 

文件的容器格式，必要参数。当前支持MP4、M4A、MP3、WAV、AMR、AAC封装格式，当前AAC音频封装默认为ADTS帧头格式。不支持在MP4封装格式下使用AUDIO\_MP3编码格式。
...
| AUDIO\_MP3 | MP3 | \[8000, 11025, 12000, 16000, 22050, 24000, 32000, 44100, 48000\] | 
...
**元服务API：** 从API version 22开始，该接口支持在元服务中使用。
 ...
 |
| videoCodec | [CodecMimeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#codecmimetype8) | 否 | 是 | 
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| :-- | :-- | :-- | :-- | :-- |
| preferredWidth | number | 否 | 是 | 
播放策略首选宽度，设置范围为大于0的整数，如1080，单位为像素（px）。

**元服务API：** 从API version 12开始，该接口支持在元服务中使用。

 |
| preferredHeight | number | 否 | 是 | 
...
**元服务API：** 从API version 12开始，该接口支持在元服务中使用。
 ...
 |
| preferredBufferDuration | number | 否 | 是 | 
...
**元服务API：** 从API version 12开始，该接口支持在元服务中使用。
 ...
 |
| enableSuperResolution18+ | boolean | 否 | 是 | 
...
**元服务API：** 从API version 18开始，该接口支持在元服务中使用。
 ...
 |
| showFirstFrameOnPrepare17+ | boolean | 否 | 是 | 
...
**元服务API：** 从API version 17开始，该接口支持在元服务中使用。
 ...
 |
| mutedMediaType | [MediaType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#mediatype8) | 否 | 是 | 

关闭输出的媒体类型。

API version 12-19，仅支持设置MediaType.MEDIA\_TYPE\_AUD。API version 20起，增加支持MediaType.MEDIA\_TYPE\_VID。

 |
| preferredAudioLanguage13+ | string | 否 | 是 | 
...
**元服务API：** 从API version 13开始，该接口支持在元服务中使用。
 ...
 |
| preferredSubtitleLanguage13+ | string | 否 | 是 | 
...
**元服务API：** 从API version 13开始，该接口支持在元服务中使用。
 ...
 |
| preferredBufferDurationForPlaying18+ | number | 否 | 是 | 
...
**元服务API：** 从API version 18开始，该接口支持在元服务中使用。
 ...
 |
| thresholdForAutoQuickPlay18+ | number | 否 | 是 | 
...
**元服务API：** 从API version 18开始，该接口支持在元服务中使用。
 ...
 |
| keepDecodingOnMute20+ | boolean | 否 | 是 | 
...
| frameWidth | number | 否 | 是 | 
录屏的视频宽度。

默认屏幕宽度，根据不同屏幕默认值不同，单位为像素（px）。

 |
| frameHeight | number | 否 | 是 | 
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| :-- | :-- | :-- | :-- | :-- |
| event | [AVMetricsEventType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#avmetricseventtype23) | 否 | 否 | 
指标事件的类型。
...
| details | Record<string, Object> | 否 | 否 | 

事件的详细信息，不同指标事件类型包含的信息不同。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8f/v3/-59yPL4XQIOvHENF-dD0QA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T014845Z&HW-CC-Expire=86400&HW-CC-Sign=D6DD2463689CFF19BC1DC5AFEFFD8A95565A0B05A26727F78A8F5395F417D0D9)
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| :-- | :-- | :-- | :-- | :-- |
| audioEncoder | [AudioEncoder](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#audioencoderdeprecated) | 否 | 是 | 
音频编码格式，默认设置为AAC\_LC。

**说明：** 从API version 6开始支持，从API version 8开始废弃，建议使用audioEncoderMime替代。

 |
| audioEncodeBitRate | number | 否 | 是 | 
...
**说明：** 从API version 6开始支持，从API version 9开始废弃，建议使用[AVRecorderProfile](#avrecorderprofile9)中的audioChannels替代。
 ...
 |
| format | [AudioOutputFormat](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#audiooutputformatdeprecated) | 否 | 是 | 

音频输出封装格式，默认设置为MPEG\_4。

**说明：** 从API version 6开始支持，从API version 8开始废弃，建议使用fileFormat替代。

 |
| location | [Location](#location) | 否 | 是 | 
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8c/v3/XZs73QqsSsy0jZnmlPDOfw/zh-cn_image_0000002569021519.png?HW-CC-KV=V1&HW-CC-Date=20260417T014845Z&HW-CC-Expire=86400&HW-CC-Sign=AA02F3C93C3F022576C17A0B89523746030626009A64269060DF7E71271601D3)
...
**说明：** 从API version 6开始支持，从API version 9开始废弃，建议使用[AVRecorderConfig](#avrecorderconfig9)中的url替代。
 ...
 |
| audioEncoderMime8+ | [CodecMimeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#codecmimetype8) | 否 | 是 | 
...
**说明：** 从API version 8开始支持，从API version 9开始废弃，建议使用[AVRecorderProfile](#avrecorderprofile9)中的audioCodec替代。
 ...
 |
| fileFormat8+ | [ContainerFormatType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-e#containerformattype8) | 否 | 是 | 

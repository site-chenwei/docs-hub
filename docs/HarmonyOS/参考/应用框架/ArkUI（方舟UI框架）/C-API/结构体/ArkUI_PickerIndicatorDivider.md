...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pi-arkui-nativemodule-arkui-pickerindicatordivider"
menu_path:
  - "参考"
  - "应用框架"
  - "ArkUI（方舟UI框架）"
...
captured_at: "2026-04-17T01:48:10.793Z"
---
...
取值范围：\[0, 选中项高度的一半（即20vp）\]。strokeWidth小于0时设置分割线样式指示器的样式参数失败，strokeWidth大于选中项高度的一半时使用2.0px。不支持“百分比”类型。

 |
| uint32\_t dividerColor | 
...
取值范围：startMargin与endMargin之和不得超过Picker容器的宽度。设置小于0时设置分割线样式指示器的样式参数失败，startMargin与endMargin之和超过Picker容器的宽度时，使用默认值。不支持“百分比”类型。

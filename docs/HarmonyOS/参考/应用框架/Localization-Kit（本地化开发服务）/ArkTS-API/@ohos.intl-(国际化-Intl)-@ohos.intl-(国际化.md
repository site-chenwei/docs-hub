---
title: "@ohos.intl (国际化"
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-intl"
menu_path:
  - "参考"
  - "应用框架"
  ...
  - "ArkTS API"
  - "@ohos.intl (国际化-Intl)"
captured_at: "2026-04-17T01:48:16.305Z"
---
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7b/v3/aV7RuAodSbOcDeuYlbjQrw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T014816Z&HW-CC-Expire=86400&HW-CC-Sign=4C0EFB5A818BF816FAC437C3F95A86A96B248AA455988F87E29A242B20A06550)
...
"zhuyin"：注音排序，仅用于中文排序。
 ...
 |
| hourCycle | string | 否 | 否 | 

区域的时制信息，取值包括：

"h11"、"h12"、"h23"、"h24"。

不同取值的显示效果可参考[附录表5](#附录)。

 |
| numberingSystem | string | 否 | 否 | 
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8e/v3/H85x8YMHRcyUbWPTG-6XpA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T014816Z&HW-CC-Expire=86400&HW-CC-Sign=0DD336AA77938D705F1C0BFD299BA4B5D8B115BE50632580D9CC588E0BCE20B8)
...
| options | [DateTimeOptions](#datetimeoptionsdeprecated) | 否 | 
...
默认值：系统当前区域ID。
 ...
 |
| dateStyle | string | 否 | 是 | 

日期显示格式，取值包括：

"long", "short", "medium", "full", "auto"。

不同取值的显示效果请参考[附录表1](#附录)。

 |
| timeStyle | string | 否 | 是 | 

时间显示格式，取值包括：

"long", "short", "medium", "full", "auto"。

不同取值的显示效果请参考[附录表2](#附录)。
...
不设置dateStyle或timeStyle参数时的显示效果请参考[附录表5](#附录)。

设置dateStyle或timeStyle参数时的显示效果请参考[附录表6](#附录)。
...
"numeric", "2-digit"。
 ...
 |
| timeZoneName | string | 否 | 是 | 

时区名称的本地化表示，取值包括：

"long", "short", "auto"。

不同取值的显示效果请参考[附录表8](#附录)。

 |
| dayPeriod | string | 否 | 是 | 

时段的显示格式，取值包括：

"long", "short", "narrow", "auto"。

不同取值的显示效果请参考[附录表10](#附录)。

 |
| localeMatcher | string | 否 | 是 | 
...
货币单位， 取值符合[ISO-4217标准](https://www.iso.org/iso-4217-currency-codes.html)，如："EUR"，"CNY"，"USD"等。
...
不同取值的显示效果请参考[附录表19](#附录)。

 |
| currencyDisplay | string | 否 | 是 | 
...
不同取值的显示效果请参考[附录表21](#附录)。

 |
| unitUsage8+ | string | 否 | 是 | 
...
不同取值的显示效果请参考[附录表22](#附录)。

 |
| signDisplay | string | 否 | 是 | 
...
不同取值的显示效果请参考[附录表16](#附录)。

 |
| minimumIntegerDigits | number | 否 | 是 | 
...
不同取值的显示效果请参考[附录表11](#附录)。

 |
| minimumFractionDigits | number | 否 | 是 | 
...
不同取值的显示效果请参考[附录表12](#附录)。

 |
| maximumFractionDigits | number | 否 | 是 | 
...
不同取值的显示效果请参考[附录表13](#附录)。

 |
| minimumSignificantDigits | number | 否 | 是 | 
...
不同取值的显示效果请参考[附录表14](#附录)。

 |
| maximumSignificantDigits | number | 否 | 是 | 
...
不同取值的显示效果请参考[附录表15](#附录)。

 |
| roundingPriority18+ | string | 否 | 是 | 
...
**元服务API**：从API version 18开始，该接口支持在元服务中使用。
 ...
 |
| roundingIncrement18+ | number | 否 | 是 | 
...
不同取值的含义请参考[语言单复数规则](https://www.unicode.org/cldr/charts/45/supplemental/language_plural_rules.html)。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| :-- | :-- | :-- | :-- | :-- |
| localeMatcher(deprecated) | string | 否 | 是 | 
...
\- cardinal：基数词，ordinal：序数词。
 ...
 |
| minimumIntegerDigits(deprecated) | number | 否 | 是 | 
...
默认值：1。
 ...
 |
| minimumFractionDigits(deprecated) | number | 否 | 是 | 
...
默认值：0。
 ...
 |
| maximumFractionDigits(deprecated) | number | 否 | 是 | 
...
默认值：3。
 ...
 |
| minimumSignificantDigits(deprecated) | number | 否 | 是 | 
...
默认值：1。
 ...
 |
| maximumSignificantDigits(deprecated) | number | 否 | 是 | 
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/23/v3/nPvJA2fyQyC1UEpgoBkxeQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T014816Z&HW-CC-Expire=86400&HW-CC-Sign=1522DB03DB6F77D568A9774E5CCE1D9376673D3BD067CC104D476F91CFB9320F)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d6/v3/YW53oDSYSo6GtWQkeqw2Iw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260417T014816Z&HW-CC-Expire=86400&HW-CC-Sign=2C6C7B868F20DF3903E99142CB4A9C05C78D144486F80B7FCA18A8100C8CA7FF)

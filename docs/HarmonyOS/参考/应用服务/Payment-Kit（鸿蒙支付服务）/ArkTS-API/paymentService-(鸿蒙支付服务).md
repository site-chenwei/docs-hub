...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-paymentservice"
menu_path:
  - "参考"
  - "应用服务"
  - "Payment Kit（鸿蒙支付服务）"
...
captured_at: "2026-04-17T01:49:00.898Z"
---
...
| **名称** | **类型** | 只读 | 可选 | **说明** |
| :-- | :-- | :-- | :-- | :-- |
| selectedPaymentType | string | 否 | 是 | 
用户选择的支付方式。
...
| [extraInfo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-model#extrainfo) | string | 否 | 是 | 保留字段。json string格式。 |
| [payload](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-model#payload) | string | 否 | 是 | 
...
\- 传递后收银台可以转换成货币符号则显示货币符号（比如￥），转换不了则显示所传递的值。
 ...
 |
| [extraInfo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-model#extrainfo) | string | 否 | 是 | 

保留字段。json string格式。若未填写，默认为空。

**说明：** 商户可以通过保留字段指定支付方式。指定收银台支付方式列表传递内容示例为{"selectPayType":"wechat\_pay|xxx"}。
...
| [orderStr](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-model#orderstr) | string | 是 | 拉起收银台传入的订单信息，[orderStr](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-model#orderstr)是json字符串的格式。不传会报401参数错误。 |
...
| :-- | :-- | :-- | :-- |
| context | common.[UIAbilityContext](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext) | 是 | UIAbility上下文，不传会报401参数错误。 |
| [orderStr](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-model#orderstr) | string | 是 | 
拉起收银台或跳转三方支付传入的订单信息。

orderStr是json字符串的格式，不传会报401参数错误。

 |
| [payload](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-model#payload) | string | 是 | 
...
| 类型 | 说明 |
| :-- | :-- |
| Promise<[PayResult](#payresult)\> | 
Promise对象。带[PayResult](#payresult)返回结果的Promise对象。

**说明：** 华为支付场景下，[PayResult](#payresult)可能返回为空，支付结果以回调通知或查询结果为准。

...
source_url: "https://developer.huawei.com/consumer/cn/doc/harmonyos-references/graphics-accelerate-assetdownloadmanager"
menu_path:
  - "参考"
  - "图形"
...
captured_at: "2026-04-17T01:48:51.506Z"
---
...
\- 三方CDN：请前往AppGallery Connect查看已配置的CDN域名白名单。具体操作步骤请参见[创建下载任务](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/graphics-accelerate-assetdownload-release#创建下载任务)。

\- 华为CDN：域名白名单将基于资源包下载任务自动配置。

 |
| isEssential | boolean | 否 | 否 | 
...
下载资源的文件名。若输入文件名，系统下载的临时文件将以该文件名命名，否则，文件名将以任务ID命名。
...
| totalBytesWritten | number | 是 | 否 | 
待下载的资源总大小，单位：字节。

默认值：0。

 |
| totalExpectedBytes | number | 是 | 否 | 
...
\- 若资源托管至华为CDN，返回资源包下载清单manifestUrl。
...
| **名称** | **类型** | 只读 | 可选 | **说明** |
| :-- | :-- | :-- | :-- | :-- |
| totalBytesWritten | number | 否 | 否 | 
已下载的资源总大小，单位：字节。取值范围：大于等于0。

默认值：0。

 |
| totalExpectedBytes | number | 否 | 否 | 
...
| resourceType | [ResourceType](#resourcetype) | 否 | 是 | 

后台正在下载的资源类型，不同的资源类型决定了后台下载通知的内容样式。

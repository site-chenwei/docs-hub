"""查询 docs-hub 索引。与 local-doc skill 的调用契约对齐。

用法示例：
    python3 scripts/search_docs.py --hub-root . --list-docsets
    python3 scripts/search_docs.py --hub-root . 输入法 --top 10
    python3 scripts/search_docs.py --hub-root . 光标 跟随 --match all --docset harmonyos --top 5
    python3 scripts/search_docs.py --hub-root . pdd.mall.info.get --docset pinduoduo
    python3 scripts/search_docs.py --hub-root . --rebuild-stale 订单 --top 8
...
from _bootstrap import ensure_initialized  # noqa: E402
...
def ensure_index_ready(hub_root: Path, docset_id: str, rebuild_stale: bool) -> Path | None:
    ...
    if not db.exists() and not rebuild_stale:
        print(
            ...
            f"  使用 $local-doc refresh 或手动运行: "
            f"python3 scripts/build_docset_index.py --hub-root {hub_root} --docset {docset_id}",
            file=sys.stderr,
        )
        return None
    # refresh 模式：显式执行一次增量 build；即便索引已存在也要刷新变更文档。
    script = Path(__file__).parent / "build_docset_index.py"
    action = "增量刷新" if db.exists() else "先构建"
    print(f"[build] {docset_id} 索引{action}…", file=sys.stderr)
    r = subprocess.run(
        [sys.executable, str(script), "--hub-root", str(hub_root), "--docset", docset_id],
        check=False,
        capture_output=True,
        text=True,
    )
...
def search_one(
    ...
    try:
        ...
        if match_expr:
            if short_tokens and mode == "all":
                rows.extend(query_fts(conn, match_expr, section, top, include_nav, short_tokens, mode))
            else:
                rows.extend(query_fts(conn, match_expr, section, top, include_nav, [], mode))
    ...
    for r in rows:
        ...
        out.append({
            ...
            "snippet": snippet_clean(r["body"], 200),
        })
        if len(out) >= top:
            break
    return out
...
def main() -> None:
    ...
    ensure_initialized(hub_root, "查询文档")
    ...
    for ds in targets:
        db = ensure_index_ready(hub_root, ds["id"], args.rebuild_stale)
        if db is None:
            continue
        try:
            rows = search_one(db, match_expr, short_tokens, args.match, args.section, args.top, args.include_nav)
